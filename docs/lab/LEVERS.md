...
> **nothing** (the tune, config level ENHANCE, is the default since 2026-09-03; `PXA_ENHANCE=0` rolls it back, `PXA_REFERENCE=1` is the bit-exact baseline) and optionally **PXA_MODE=balance|max** (posture). Every other env below is an
...
| **DEFAULT** (level 1) | `PXA_ENHANCE=0` — **the rollback tier since 2026-09-03**, it used to be what you got with no env at all | the pre-2026-09-03 shipped defaults: §2's measured bit-exact winners ON + `PXA_VOLTA_CUBLAS_NE11=64` (sm_70). Behavior unchanged vs the 2026-07-21 ship. |
| **ENHANCE** (level 2) | **the default since 2026-09-03** (or `PXA_ENHANCE=1`, now a no-op) | default **+** the per-arch **measured** G3-class levers whose ship gates passed: `PXA_PXQ_INT8_PREFILL=1` on **sm_61 ONLY** (the cc==610 ship gate is unchanged; **+182% prefill** measured on the 1080 Ti, §4) and `PXA_SPEC_RELAXED=1` (spec lanes only, G3). **2026-07-29: plus the model-adaptive auto-set — see §0c** (`PXA_ROUTER_FUSE` gated pure-sm_70 × MoE, `PXA_PXQ_MMVQ` auto for PXQ4-bearing models on DP4A devices, `PXA_MTP_LAZY_WARMUP` default ON, server auto-samplers). |
...
Implementation: `ggml/src/ggml-cuda/pxa/pxa-enhance.cuh` (`common/sampling.cpp` mirrors the level
...
- **Explicit CLI always wins**: `-fa`/`-ub` (or `LLAMA_ARG_FLASH_ATTN`/`LLAMA_ARG_UBATCH`) on the
  ...
  `pxa_mode()` in `ggml/src/ggml-cuda/pxa/pxa-enhance.cuh`).
...
⚠ **HONESTY GATE — DISCHARGED 2026-09-11** for `PXA_FA_MASK_SKIP_TILE`. Its silicon A/B has now
run, and it ran on the **shipped artifact** rather than a development build: 4096-token prefill
co-resident with a 4096-token band, `-np 2`, cards 0,1,5,6 (sm_60 P100), gate `timings.prompt_ms`
— OFF 23133.8 ms vs ON 20747.2 ms, **+11.50 %**, noise floor 2.1 ms, output shas identical across
every arm. Pre-registered before the first arm. **Quote that figure for that cell and no other:**
it is a co-resident prefill-wall effect, and it measures neither decode nor a single-card seat.
The same run's `-np 1` discriminator is deliberately NOT quoted — it was sampled once per arm, and
the 1.94 % it shows sits under the 1.38 % drift the harness's own 1024-token scaling control
displayed across exactly those two boots, so it resolves nothing in either direction. Roll back
with `PXA_FA_MASK_SKIP_TILE=0`. `PXA_FA_PREFILL_SPLIT` was demoted to opt-in
...
The ENHANCE level is now adaptive to the **loaded model**, not just the device set. At model load the
...
| `PXA_SPEC_RELAXED` (+`_PMIN`, default 0.05) | **level default: ON at ENHANCE (the default level), OFF at DEFAULT/REFERENCE**; `=0`/`=1` always wins | relaxed speculative acceptance: accept a draft token that lands in the target's post-filter candidate set with p ≥ PMIN (instead of exact-match only). Auto-disabled for grammar/mirostat/temp≤0 **2026-09-06 correction:** this row (and the two below, and the code comment at `common/sampling.cpp:1200`) said "off" while `pxa_spec_relaxed_enabled()` has returned `ggml_pxa_config_level() >= 2` since ENHANCE became the default; only §0 line 69 was right. At temp 0 the exact-match path runs regardless (greedy gates unaffected); at temp>0 on defaults the output legitimately differs from the drafter-off run. The server now prints `PXA_SPEC_RELAXED: ON (pmin=…)` when a spec stage is armed and this is live; gate arms pin `PXA_SPEC_RELAXED=0` | never A/B'd — window item; G3-class by design (output legitimately changes at temp>0) | G3-class |
...
| `PXA_PXQ23_MMVQ` | `0` (off), and **off is the shipped default** — see the gate column. Bitmask: bit 0 = PXQ2, bit 1 = PXQ3, `3` = both. Separate from `PXA_PXQ_MMVQ` on purpose: "this device may use the q8_1 GEMV" and "the low tiers are among the tiers it may use" are different questions with different evidence, and **both must be on**. `REFERENCE` opts out. Declines loudly if `PXA_PXQ2_BOOK` / `PXA_PXQ3_BOOK` / `PXA_PXQ_CEIL_V2` / `PXA_PXQ2_V3` / `PXA_PXQ6_SUB` / `PXA_PXQ2_SUB` / `PXA_PXQ3_SUB` is set (this path holds frozen book copies and takes no runtime upload), and declines if a startup self-check cannot rederive the frozen int8 books from `PXQ2_BOOK_INIT` / `PXQ3_BOOK_INIT`. Only fires at cc ≥ `CC_VOLTA`: mode 2 of `PXA_PXQ_MMVQ` (an all-sm_61 fleet) carries PXQ4 on its own evidence, not these. | Lets **PXQ2 and PXQ3 decode ride the same q8_1 MMVQ kernel PXQ4 does** — the tiers the `uniform` quantisation profile produces, i.e. the case where the *smaller* file was stuck on the *slower* path. They fit because both carry PXQ6-core's scale machinery byte for byte; only the code load and the book gather differ (`pxq23-mmvq.h`). Same "not bit-exact, fidelity-gated" contract as `PXA_PXQ_MMVQ`: `q = rint(book·127/absmax)`, `absmax/127` folded into the fp16 row anchor. Decode window only (`ny <= MMVQ_MAX_BATCH_SIZE`). | **MEASURED 2026-09-10, 2×V100, Qwen3.8-27B, `-c 8192 -np 2 -fa`, `speed.sh` medians of 7, same binary and file per pair.** Decode t/s off→on, np1 and np2: uniform PXQ2 34.21→**46.65 (+36.4%)** / 42.21→**62.39 (+47.8%)**; uniform PXQ3 27.28→**37.20 (+36.4%)** / 33.39→**50.72 (+51.9%)**; PXQ2-attn4 37.81→42.88 (+13.4%) / 47.57→58.10 (+22.1%); PXQ3-balanced 31.45→37.60 (+19.6%) / 39.09→50.82 (+30.0%). The promoted profiles gain less because their off arm already has PXQ4 attention on MMVQ. Prefill moved +1.4…+9.5% and is **not claimed** — the lever only takes the decode window and the pf512 leg has a wide min–max. **Statically** (sm_70, plain `ny=1 ROWS=4 VDR=2`) PXQ2 is 488 instructions, *exactly* PXQ4's, while moving half the weight bytes — its 4-entry book fits one register so one PRMT covers 4 codes where the PX16 book needs three; PXQ3 is 584 (+19.7%) for −25% bytes. | **FAILS the pre-registered fidelity clause — that is why the default is OFF.** The rule was "mean KLD against this file's own exact path, no worse than PXQ4's existing fused kernel shows against its own". Measured in one session, wikitext-2, `-c 2048 --chunks 10 -b 8 -ub 8`: PXQ2 **0.004624 ± 0.000081** (PPL 23.2165→24.5898, +5.92%, same-top-p 96.452%), PXQ3 **0.002186 ± 0.000104** (PPL 5.5940→5.6137, +0.35%, same-top-p 98.074%), PXQ4 comparator **0.000951 ± 0.000097** (PPL 5.3393→5.3463, +0.13%, same-top-p 98.710%). PXQ2 is 4.9× the comparator and PXQ3 is 2.3×, so neither is "no worse". The comparison is confounded and cannot be unconfounded — the three tiers are three *files*, and the uniform-PXQ2 file sits at PPL 23.2, an operating point where the same-size perturbation converts into far more KLD; the host harness puts the weight snap at 0.0223% (PXQ2) and 0.0178% (PXQ3) of ‖w‖·‖x‖, both the same size as the q8_1 activation cost PXQ4 also pays. Needle answers byte-identical in every arm. ⚠⚠ **Gate this with `-b 8 -ub 8`, never the default batch**: `llama-perplexity` is pure prefill and at `-b 512` the `ne11 <= 8` dispatch gate never opens, so both arms return bit-identical logits — a false PASS on a run in which the kernel never fired. ⚠ A greedy-hash difference is **not** a usable engagement check either: 64 greedy tokens came out byte-identical across arms on three of the four files. Use the resolver ledger lines and the decode delta. Correctness is settled separately and exhaustively: `tests/test-pxq23-mmvq-snap` (CPU, 0.5 s) checks all 2¹⁶ PXQ2 and all 2²⁴ PXQ3 code patterns against the format and the addressing bit-for-bit against `pxa_pxq_dequant_row()`; `tests/test-pxq-mmvq-cols` runs the same patterns through the device's real PRMT — 562 cases, 135,010,816 values, 0 failed on a P100. | G3-class — different accumulation path; gate on KL divergence at `-b 8 -ub 8`, not on a sha |
| `PXA_PXQ_MMV_H2` | `3` (both low tiers) on `cc == 600` EXACTLY; `0` on every other arch. The auto verdict sets it at first dispatch (the `PXA_PXQ_MMVQ` pattern, §0c), so an unset environment gets the loop on a GP100 and the exact fp32 path everywhere else. ⚠ **The default was `0` until 2026-09-11** — it was flipped on the measurements and the fidelity pass in the two cells to the right, not on a projection. `PXA_PXQ_MMV_H2=0` restores the exact fp32 decode loop on a GP100. | **A half2 inner loop for the PXQ2/PXQ3 dense decode mmv.** The shipped `pxq6_dot32` issues the same 16 pair decodes per 32 weights for every tier — 2 shared-memory book gathers plus `FMUL + FFMA + FADD` per pair — so a 2-bit tier moves 39% fewer bytes than a 4-bit one and decodes at the same speed on a P100 (18.50 vs 18.01 t/s, Qwen3.8-27B, two cards, tg32 medians of 7): 177 GB/s per active card against ~550 achievable. That loop is ALU-bound, and the int8 route that fixed it on sm_70 needs DP4A, which GP100 does not have. GP100 does have full-rate `HFMA2`, and — the part that makes this different from the MMVQ row above — **the LM4/LM8 books are fp16-exact**, enforced by the `pxa_pxq23_book_ok()` self-check, so a half2 pair LUT carries **zero book error**: the entries are the same numbers, not roundings of them. Per pair the decode becomes one `LDS.32` (16 entries for PXQ2, keyed on the pair's own nibble; 64 for PXQ3, keyed exactly as `PXA_PXQ3_PAIRLUT`) plus one `HFMA2`. `x` is staged once per block as power-of-two-scaled `__half2`, and accumulation is half2 **inside the 16-element eff group only** — the eff scales, the `eff[0]*t[0] + eff[1]*t[1]` fold, the per-lane chain, the canonical per-chunk fold and the reducers are the incumbent's, unchanged. The scale is derived **per 32-element slab**, not per block: chunk boundaries are slab-aligned, so the scale is a function of those 32 `x` values alone and is identical in every block at every `S`, which keeps `PXQ_CANON_v1`'s split == unsplit bit-identity true inside the mode (a block-wide max would have made the logits depend on the launch geometry). Degenerate inputs are handled inside the scale rather than by a fallback arm: an all-zero slab contributes exactly `0.0f`, a non-finite `x` propagates through the `HFMA2` chain as it does in fp32, and the shift is clamped to +/-100 so `2^sh` and `2^-sh` stay inside fp32 normals. | **MEASURED 2026-09-11** on 27B low-tier files, 2x P100 (cc 6.0), `srv n256`, medians of 7. Every class was measured in BOTH arm orders and the effect is the order-cancelled mean, because a decode-only kernel cannot move the prefill control and whatever that control does IS the drift. Decode effect vs the exact fp32 decode loop: PXQ2 uniform **np1 +9.54%, np2 +11.34%**; PXQ3 uniform **np1 +14.42%, np2 +16.06%**; PXQ2-attn4 **np1 +4.27%, np2 +5.59%**; PXQ3-balanced **np1 +6.60%, np2 +7.32%**. The two orders agree to within 0.25 pp on every row, so the effect replicates across arm order rather than being a second-arm term. Net of the pp512 control's own movement it is **+2.34% to +13.33%** — still positive at np1 and np2 on all four classes. Absolute anchor: PXQ2 uniform tg32 **18.587 -> 20.385 t/s**. The reverse pass prints its order-cancelled column as `CANCELLED` in `logs/reverse-pass.out`; the estimate that survives the order term is `(d1+d2)/2`, which is the EFFECT only when both deltas are ON-relative-to-OFF — the same expression is the DRIFT under the opposite delta convention, so correct the deltas, never the formula. | **Not bit-exact against the fp32 arm, by construction** — `x` is rounded to fp16 once at stage time and the 16-element group accumulates in fp16 — so the gate is the same one the MMVQ row is held to: mean KL divergence against **this file's own exact path** no worse than the PXQ4 fused kernel shows against its own (`0.000951`), and same-top-p not below `98.710%`. ⚠⚠ **Gate it with `-b 8 -ub 8`, never the default batch.** `llama-perplexity` is pure prefill and the dense 2D decode driver only accepts `ny <= PXA_PXQ4_2D_MAX_NY` (8), so at `-b 512` it declines every node, both arms take the prefill GEMM and return bit-identical logits — a false PASS on a run in which the kernel never executed. ⚠ **A greedy hash is not a pass condition either**, in the other direction: the arithmetic differs by construction, so identical text would mean the kernel never engaged. Use the kernel's own first-fire banner (`PXA_PXQ_MMV_H2 dev<n>: ENGAGED`) as the engagement check and the decode delta as the effect. Correctness of the parts that ARE exact is settled by `tests/test-pxq23-mmv-h2` (LABEL `cuda`): the LUT is checked exhaustively against the fp16 image of the book at zero tolerance, split == unsplit is checked bitwise across the K1 and gen forms at every `S`, and the guard cases (fp16-overflowing outliers, all-zero `x`, non-finite `x`) are checked for silent swallowing. — **PASSED 2026-09-11.** Mean KLD **0.000027** (PXQ2 uniform, 35x under the bar), **0.000026** (PXQ3 uniform, 37x), **0.000032** (PXQ2-attn4, 30x); same-top-p **99.736 / 99.746 / 99.638%**, all >= 98.710%. ⚠⚠ **THE BAR IS A MEAN AND THE RUN LOG'S HEADLINE IS A MAXIMUM — they are 21x apart and only one of them is the gate.** `logs/kld-run.log` prints `Maximum KLD: 0.020244` (PXQ2-attn4) directly beside `PASS BAR: mean KLD <= 0.000951`, and that maximum EXCEEDS the bar on two of the three files (0.002857 PXQ3 uniform, 0.020244 PXQ2-attn4) while the mean is 30-37x under it. A reader who compares the two numbers sharing a line reports a catastrophic failure of a lever that passed by 1.5 orders of magnitude. Read the statistic the bar NAMES — `Mean KLD`, which is in the per-arm log (`p2u-kld-arm.log` and friends), never the run log's headline. | G3-class — different accumulation path; gate on KL divergence at `-b 8 -ub 8`, never on a sha |
...
- **⭐ STEADY-STATE BATTERY = the canonical-close V100 decode numbers (2026-07-22, merged canonical
  ...
  **The publishable V100 decode headline is 108.3 t/s (mtp n1 + lazy), +18.2% over the 91.6
  ...
  the ENHANCE level on top of the MTP config: statistically identical to the headline (108.1) and
...
| `PXA_AUTO_JINJA` | unset → on for the architectures listed in `common_chat_auto_jinja()` (today: `gemma4`, `gemma4_mtp`); `=0` declines and says so. An explicit `--jinja` or `--chat-template` decides in both directions and the rule never runs | **Turns the jinja chat path on for a model whose own template the built-in map cannot render** (2026-09-10). Gemma 4 ships a canonical template whose turns are `<|turn>` / `<turn|>` and whose channels are `<|channel>` / `<channel|>` — nothing like Gemma 3's `<start_of_turn>` — so the built-in `gemma` row is not merely absent, it would be WRONG, and a silent fallback would serve every chat user a prompt the model was never trained on. Without this, a stock `llama-server -m gemma-4-*.gguf` logged `chat template parsing error: this custom template is not supported` and chatted with someone else's template. Fires in `llama-server` and `llama-cli`, before the templates are built, so `example_format`, thinking support and `chat_params` all see one answer; prints value + reason + override in the house style. The list is explicit rather than "any template the built-in map rejects": flipping the default for every unknown template would change behaviour for models nobody has looked at. Verified: `/v1/chat/completions` on gemma-4-12B-it-qat-q4_0 returns `Paris` with the thinking trace in `reasoning_content` and `finish_reason` `stop` |
| `PXA_AUTO_SAMPLERS` | unset → engages at the ENHANCE level, which is the default (never under `PXA_REFERENCE=1`, and not at `PXA_ENHANCE=0`); `=0` forces off, `=1` forces on at any level | **Server-side model-family sampler DEFAULTS** (2026-07-29): fills only sampler fields the CLI left unset (per-field `--temp/--top-k/--top-p/--min-p` explicitness check; per-request params still win). Family table: `gpt-oss` → temp 1.0 / top_k **100** / top_p 1.0 (top_k=0 — the "official" gpt-oss setting — was measured to **HALVE decode**: 31 vs 65 t/s, full ~201k-vocab CPU sort per token, gpt-oss-120b 6-card cell 2026-07-04); `qwen35moe`/`qwen3next`/`qwen35` → the official Qwen no-think set temp 0.7 / top_k 20 / top_p 0.8 / min_p 0; `laguna` → the same Qwen-family set (HEURISTIC — no per-model sampler sweep yet, the log says so). Unknown arch → stock defaults kept, logged. Plus a generic guard: a resolved `top_k<=0` is clamped to 100 (loud). Every applied default logs value+reason+override. |
...
| `PXA_SPEC_RELAXED` (+`_PMIN`) | level default (ON at ENHANCE, OFF below); `=0` pins exact-match | relaxed draft-acceptance experiment (G3-class; not recommended) |
...
| `--imatrix` (behaviour note, not a lever) | ⚠ **The PXQ tiers IGNORE an offered importance matrix — default since 2026-08-24.** `pxq_imx_gate()` / `pxq_imx_optin_enabled()` sit in front of every tier (`src/pxq1-`, `pxq2-`, `pxq3-`, `pxq6-`, `pxq6r-quantize.inc.cpp`), and the MXFP4 backbone path does the same (`ggml/src/pxq-quants.cpp:546-557`). Passing `--imatrix` prints `PXQ tiers: imatrix IGNORED` once, and `examples/quantize/quantize.cpp` overrides the provenance KVs with `quantize.imatrix.ignored_by = pxq-tiers (PXA_PXQ_IMX unset)` so the artifact cannot advertise a consumption that did not happen — that rewrite also kills an inherited claim, since a requantize otherwise re-asserts its *source's* imatrix provenance (measured: a PXQ4 built from a community Q8_0 came out advertising an imatrix this tree never opened). **Why it is off, measured** (Ornith-9B, 580 chunks, wiki.test, clean A/B, P100, cross-card-verified; the numbers are in the source comment at `src/pxq6-quantize.inc.cpp:96-105`): every consumption path is net-negative for PXQ4 — no-imatrix **8.3076**, kqw+SMIN 8.3317, kqw 8.3420, kqw+anchor-fit 8.3491, raw 8.3542 — while the *same* imatrix file improves Q4_K_M by −0.0766. The PXQ lattice (fixed absmax anchor, 16-step sub-scale) cannot express per-column importance without clamping cold-column outliers. `--imatrix` on a **stock** target, including the requantize after `llama-pxq-export`, is unaffected and still worth having. **`PXA_PXQ_IMX=1` re-enables consumption** (KQW weighting `w = imx · sqrt(sigma2_row + x²)` by default, `PXA_PXQ_KQW=0` for the legacy raw-imx form) — a lab lever, not a quality setting. Under that opt-in the original behaviour applies: the anchor pick and the code/sub-scale selection both minimise a diagonal-weighted SSE (`err += w[i]·e²`), per-expert columns when `imx_size == K·E` and a shared column when `== K`, with the 2026-07-26 **dead-column guard** in front of it — an all-zero (or non-finite / negative) column would make *every* candidate score exactly `0.0`, silently collapsing the argmin into its tie-break, so such a column is fit **unweighted** instead and the total is reported at completion. That is the normal case for routed experts that never fired during calibration — expected on a 256-expert MoE — but a large count means the corpus is too thin for the expert count. Corpus guidance under the opt-in: ≥2000 chunks. (This row used to recommend computing the imatrix with `-sm graph` for a measured +163% prefill. An imatrix capture runs on the stock source file, which graph split does not refuse — but that figure predates the graph-split codec check and has not been re-taken, so it is no longer quoted.) |
...
| `PXA_SPEC_RELAXED_PMIN` | 0.05 | `common/sampling.cpp` | p_min floor for `PXA_SPEC_RELAXED`, which is ON by level default at ENHANCE (see the §4 row); only consulted at temp>0 |
...
| `PXA_FA_GQA_PACK` | **0 (off)**; NH ∈ {2,4,8}. ⚠ **BUT 4 IS ARMED WITH NO ENV SET on a multi-card sm_60 topology — the default ENHANCE level arms it. See the correction at the end of this row before reading this cell** | sm_60 D=256 vec flash-attention that hands one block NH Q heads sharing a KV head, so the K row is fetched once and dotted against NH query vectors. Load traffic and load-instruction count both fall by NH at unchanged FMA count. Gated on `ncols == 1`, `K == V == f16`, `Dk == Dv == 256`, no logit softcap, and NH dividing both the head count and the gqa_ratio; anything else keeps the stock kernel | Stock `flash_attn_vec_ext_f32` re-reads the whole K/V cache once per Q head — on qwen35moe-122B-A10B (32 Q heads over 2 KV heads) an nsys decode capture put it at 4.82 ms of a 36.14 ms token (13.4%), issuing ~291 MB of K+V loads per layer against 18.2 MB of unique cache. Measured 122B PXQU48-core, 4×P100, `-fa on`, fill 8881, n=3 medians, control in the same run: **NH=4 +6.9%**, NH=8 +6.9% (occupancy cancels the extra traffic cut); at fill 15 unchanged (31.83 → 31.94), prefill unchanged (300.5 t/s every arm — prefill takes `cols_per_block > 1`). ⚠ **Not bit-exact** vs the shipped `PXA_FA_VEC_ILP` V pass (1-accumulator vs 4-accumulator order). ⚠ **CORRECTED 2026-09-12 — this row advertised the wrong default.** It said "Default stays 0 until a coherence + ppl gate is recorded". That gate is still not recorded, but the code does not wait for it: `pxa_fa_gqa_pack_default()` (`ggml/src/ggml-cuda/pxa/pxa-enhance.cuh`) returns **4** whenever `pxa_config_level() >= 2`, and ENHANCE **is** the default level — `ggml_pxa_config_level()` (`ggml/src/ggml.c`) resolves an unset `PXA_ENHANCE` to 2. So with **no env set at all**, on a multi-card non-mixed sm_60 topology, this **not-bit-exact** kernel is armed. `docs/COOKBOOK.md` discloses the arming and adds "Do not quote a throughput number for either"; this row, that row and the source comment now agree. ⚠⚠ **UPDATED 2026-09-12 — the firing counter landed and ran, and it split the question in two.** **Engagement is settled: the default DOES reach the packed kernel** (see the row below for the counts). **Fidelity is not, and now has two failed instruments rather than one**: the server A/B's perplexity arm never dispatched the kernel, and the separate perplexity harness **voided its own control** — with `PXA_FA_GQA_PACK=0` the OFF arm still logged 5 `FIRED` lines on the `-b 1 -ub 1` decode path, so that instrument does not discriminate on-versus-off at all and no KLD taken with it says anything about this lever. **Read nothing into either KLD.** So what ships is a **not-bit-exact kernel, armed by default on a multi-card sm_60 topology with no env set, whose fidelity has never been measured by an instrument that worked.** That is the open item; the effect arm below is separately VOID. |
...
| lever | default | what it does | measured |
|---|---|---|---|
| `PXA_FA_MMA_VOLTA_Q8` | **ON** (`=0` restores the previous Q8 route) | On **sm_70 only**, admits matched Q8_0 K/V into the existing vendored Volta `m8n8k4` FlashAttention path for the measured **D=256, prefill (`Q rows > 8`)** case. Q8 K/V are staged through the CUDA pool to the same F16 MMA input used by the existing F16 route; F16, decode, other head sizes, and every other architecture keep their previous dispatch. | **Single V100 / Qwen3.8-27B / Q8 K/V:** interleaved same-binary `pp16000` OFF/ON blocks `743.66→903.97` and `739.15→905.23` tok/s (**+22.0% mean**), while `tg64` was unchanged. With one identical restored 100k-token slot, mirrored OFF/ON/ON/OFF process blocks for the 1k append were `149.30`, `305.04`, `300.73`, `149.52` tok/s: **149.41→302.89 (+102.7%)**; every arm generated the same control token. Four 512-token KL chunks vs the old route: PPL ratio `0.999840`, mean KLD `0.000024`, RMS Δp `0.130%`, same-top `100%`. |
...
| `PXA_ALLOW_GRAPH_SPLIT_HYBRID` | **off — and this is a correctness guard, not a performance lever** | ⚠ **Footgun. `=1` bypasses a measured-defect guard and can produce fully degenerate output.** `-sm graph` on the DeltaNet hybrid arches (`qwen35moe`, `qwen3next`, `qwen35`, `qwen4exp` — the guard keys on `llm_arch_is_hybrid()`, not on a hand-kept name list) falls back to `-sm layer` with a warning; this env removes the fallback, for debugging only. It does **not** touch the engine's separate refusal of `-sm graph` on a **PXQ file**, which is a property of the file's layout and holds on every architecture. On `qwen4exp` the override does not degenerate, it aborts: there is no split-device path for the hyper-connections (`src/llama-delta-net.cpp`) | **Root cause, corrected 2026-09-08 — it is the codec, not the reduce.** Bisected on hardware, two V100s, one variable at a time: the same binary and the same model are clean under `-sm graph` on a stock `Q4_K_M` quant and degenerate on the PXQ4 quant of it (`$` with a NaN logprob, then `!!!!`), and swapping only the split mode on the PXQ file makes it appear and disappear. Graph split cuts `wo` and `ffn_down`/`ffn_down_exps` along dim 0 = `K` (`src/llama-load-tensors.cpp:5611`, `:5733`, `:5761`); PXQ keeps one fp16 anchor per row in the 64-row panel header covering all of `K`, so a `K`-split would have to duplicate it and nothing does — and every assertion on that path is on *sizes*, which PXQ satisfies exactly, so the slices were read at wrong byte offsets in silence. The 2026-08-04 attribution (cross-device all-reduce not delivered) is **not needed to explain any of it** and stands unproven. ⚠ **The old speed row for this lever is VOID, not merely unverified:** `+64% prefill / −17% decode on 4×P100` was taken on a PXQ file under `-sm graph` — a configuration that was emitting NaN — so it describes nothing runnable; the 2×V100 (−31%) and 8×V100 (−4.6× decode) figures were taken before the codec check existed and are not a supported configuration either. **The guard still stands on a stock file, on its own terms:** graph mode there is not degenerate and matched `-sm layer`'s top-1 token on every prompt tried, but it is not bit-identical — the logits differ slightly and greedy text diverges after about a dozen tokens — so it is outside the determinism gate for these architectures |
| `PXA_REDUCE_NCCL` | **ON** where NCCL is available (`=0` takes the non-NCCL reduce route) | Transport selection for the cross-device reduce | Used as a bisection control in the graph-split investigation above; both routes were byte-identically degenerate, which was read at the time as ruling the transport out. ⚠ **It ruled out nothing** (noted 2026-09-08): once the logits are NaN, greedy decode emits the same token forever, so byte-identity between two arms proves only that both were broken. The failure was the codec, which neither arm changed. No throughput A/B on record |
| `PXA_REPLICATE_RECURRENT` | off (presence-tested) | Replicates the recurrent state across devices instead of splitting it | Same investigation, same caveat as the row above: byte-identically degenerate, which in an absorbing NaN state is uninformative rather than a null result about the recurrent carry. No throughput A/B |
...
|---|---|---|---|
| **`-sm graph`** (and `-sm attn`) | Rig-A 3-card, fill 8192 | **NULL — it never engages.** +0.0%/+0.2% prefill, +0.0%/+0.1% decode, byte-identical output. `LLM_ARCH_MUSE_GLIMMER` is **absent from `k_supported` in `is_model_split_supported()`** (`src/llama.cpp:3496`, still absent as of 2026-09-08), so the load path warns "not supported for this model" (`src/llama.cpp:3843`) and silently serves `-sm layer`. Engagement disproof: the graph arm prints `split_mode_graph_scheduling = 0` / `graph splits = 4`, byte-identical to the layer arm. Standing gate on top of the arch list, added 2026-09-08: on a **PXQ file** `-sm graph` is refused outright, so admitting an arch to `k_supported` would not by itself make this cell runnable | n/a (no-op) |
...
| **`PXA_FA_GQA_PACK=4`** on the single-P100 instance | that instance's card 5, fill 8352 | **DOES NOT REPRODUCE. NOT SHIPPED.** Measured **+1.7%** (MTP off, spreads 0.3/0.4%) and +2.7% (MTP on, vs a baseline band of 1.3%) — against the record's **~+6.9%**. `NH=8` is **+0.3% = NOISE** (the record's "+6.9% at NH=8" does not reproduce either). That instance satisfies every documented gate (D=256, K/V f16, gqa_ratio 4, sm_60) and sm_60 decode *does* route to `vec_f32` (`fattn.cu:137-166` via `pxq_use_sm60_vec_f32`), so it *could* fire | **⚠ UPDATED 2026-09-12 — ENGAGEMENT IS NOW PROVEN; THE FIDELITY GATE IS NOT, AND THE EFFECT ROW ABOVE IS STILL VOID.** The firing counter this cell asked for exists and ran, 4× P100 (sm_60), six boots: the default arm logged **5 FIRED** lines, the `PXA_FA_GQA_PACK=0` control **0**, and the geometric-rejection control **0 fired / 1 declined** — so the instrument discriminates, and the earlier objection (the `NH=4` banner proves only that the env was parsed) is answered. Reproducible: the arm against its own repeat agreed 12/12 on short coherence and 2/2 on needles. **What is still not measured is fidelity**: the perplexity/KLD arm **never logged a FIRED line**, so it evaluated prefill only and its KLD is a control arm's — **read nothing into it**. Speeds in that run: 24.008 / 24.829 (default, two boots), 23.636 / 23.837 (off), 23.916 (geometric reject) — inside each other's spread, which is why the effect row above stays VOID rather than being rewritten. **And the companion perplexity harness is worse, not merely silent**: its own verdict is `INVALID: CONTROL FAILED` — the `PXA_FA_GQA_PACK=0` arm logged **5 `FIRED` lines** on the `-b 1 -ub 1` decode path, so on that route the OFF state is not off and the instrument cannot discriminate at all. Note the two harnesses **disagree about the same control**: the server A/B's OFF arm fired 0 and the perplexity harness's fired 5, which is recorded as observed rather than diagnosed. Closing this needs a perplexity harness whose OFF arm is provably off **and** whose ON arm provably dispatches — both halves, since each has now failed once — or an explicit decision to accept the coherence and needle arms as the gate and say so in as many words |
| **`PXA_P100_FP16_GEMM`** | the single-P100 instance's card 5 (single sm_60), 40 chunks x 512 tok | ⚠ **FAILS THE STANDING QUALITY GATE: `Same top token` = 94.088 ± 0.234 %** (reject threshold is <98%). Median KLD 0.004266, max KLD 1.755914. **Auto-armed on ALL THREE live instances today** (every instance has a P100). I have left it armed for now; its benefit (+51% gpt-oss prefill) makes this a deliberate speed/fidelity trade | prefill/batched-path -> `llama-perplexity --kl-divergence`, reference = **lever OFF, same model** (not a Q8_0 parent). Engagement proven: `PXA_AUTO: P100_FP16_GEMM=off` vs `=on` in the two arms. Corpus `<engine-tree>` (**not** della). Perplexity NOT used |
...
on each. So a lever that fails my own quality bar is shipping ungated on my live instances today.
I have not disarmed it yet: the counterpart benefit is real and large (+51% gpt-oss prefill on the
record), so this is a speed-vs-fidelity trade I need to make knowingly, not a bug.
...
# 2026-08-16/17 — SWA-aware KV allocation (single-node)
...
- The D=256 story from the single-P100 investigation (`fattn-tile-f32` handles only D=64/128, so D=256 falls to
...
- Prebuilt libs kept for a zero-rebuild redeploy if I ever want the accuracy trade:
...
Not applied — a context reduction is a capability trade, not a tuning change.
...
⚠ **This corrects a claim in our own notes.** An earlier internal quant-ladder note (2026-08-01,
not published) attributes the profiled `concat_f32_non_cont` cost (3.8% of prefill over 9295 calls) to
...
## Updates — 2026-09-09 — rc3 levers

Every gated flag that lands in `v2026.09.09-rc3` and did not have a row above. Read as: every row
marked **unmeasured** is **default OFF** and stays off until a speed window says otherwise; the
two correctness guards are the exception and are on by default. Columns: default in the merged
tree, what the lever does, whether it has a measured number behind it yet, and which
architecture/card it actually matters on.

| lever | default | what it does | measured? | arch / card |
|---|---|---|---|---|
| `PXA_MOE_DEVICE_MAP` | **1 (on)** — but only on a multi-card, non-mixed, sm_60 topology at the ENHANCE level; **0** everywhere else. Modes: `0` off, `1` device-side mapping, `2` device-side + host cross-check (env-only at every level, including ENHANCE). An explicit setting wins in both directions | Moves the MoE expert-row mapping — the `rmap`/`tile` index arrays a `mul_mat_id` needs — off the host and onto the device, removing a D2H copy plus a `cudaStreamSynchronize` from inside graph compute. Armed by `pxa_moe_device_map_default()` (`ggml/src/ggml-cuda/pxa/pxa-enhance.cuh:805-817`); the only call site is the `mul_mat_id` row-mapping prep (`ggml/src/ggml-cuda/ggml-cuda.cu:6208-6228`). It **cannot** fire on a dense model or on a single-card/invalid/mixed topology — every one of those declines by construction, so the affected cell is narrower than the flag's name suggests | **ENGAGEMENT + CORRECTNESS: YES — settled env-only on the shipping artifact, no rebuild. THROUGHPUT: NO — no A/B on record.** `PXA_MOE_DEVICE_MAP=2` runs the device map's host cross-check, which rebuilds the expert map on the host and compares it element-by-element: **48/48 rows `-> identical`, 0 `-> MISMATCH`** on the 4× P100 (sm_60) cell. The control arm (`=0`) logs 0 `FIRING` lines and an unset arm logs 4, so the lever both arms by default and declines when told to. This settled without a build because the `FIRING` counter (`pxa/pxq-moemap.cuh:228`) and the `=2` VERIFY mode (`pxa/pxq-moemap.cuh:279-347`) are compiled into the shipping binary. **What is NOT established is the effect**: this row proves the mapping is *correct*, not that it is *faster* — the implementing comment (`pxa-enhance.cuh:807`) asserts a "measured basis … the same 4x P100 seat", and the engagement half of that is now verified while the number is not | multi-card sm_60 (the 4× P100 cell); declines on dense models, single-card, and mixed topologies |
| `PXA_MTP_HEAD_OUTPUT` | **1 (on)** | Correctness guard: marks the MTP head's feature-row tensor a graph output, so the allocator cannot hand its buffer to a later node before the next draft step reads it back. `=0` reproduces the pre-fix defect (a stale conditioning hidden on every multi-row commit) and exists only to prove the fix, never to ship. | Yes — `tests/test-mtp-head-output.cpp` (CPU, allocator-exact) and a GPU top-1-probability delta (0.036 → 0.227 mean at depth 2, on the affected path). | Any model with an MTP/NextN head. |
| `PXA_MTP_PMIN_TOPK` | **10** at the shipped config level, **0** at the bit-exact reference level | An MTP confidence floor compares a top-K-renormalised probability instead of a full-vocabulary softmax, so a floor of e.g. 0.75 means the same thing a draft-model floor does. | The renormalisation itself is a CPU-checked arithmetic change; inert on the shipping path today because the family default floor is `p_min = 0`, so no probability is computed to renormalise. | Any model with MTP armed and an explicit `p_min` set (CLI, per-request, or a future non-zero family default). |
| `PXA_MTP_ZERO_OUTPUT_COMMIT` | **0 (off)**, for every family the `PXA_AUTO` table knows | The commit decode that advances the MTP head's KV after a verify asks for no sampled output on every row instead of one, trading a full-graph pass (QKV, attention, output projection, a vocab-wide output-head matrix-vector product) for a KV-only one, plus one extra single-row draft decode to regenerate the free token that used to come from reading the commit's own output back. | **MEASURED, and it lost.** On the fixed binary (2×V100, Qwen3.8-27B PXQ4, decode 256, median of 3): depth 3 with the split 52.52 t/s at np1 and 63.63 aggregate at np2, against 54.42 / 71.89 without it. It led the earlier matrix only because it was working around the stale read-back row that `PXA_MTP_HEAD_OUTPUT` now fixes — with the cause gone, the workaround is just its own cost (~2.9 ms per cycle for the extra draft decode). Default off. | `qwen35` / `qwen35moe` family today; other MTP-capable families keep today's defaults. |
| `PXA_MTP_BATCH_SLOTS` | **0 (off)** | Shares one draft decode per step across every concurrently-drafting slot instead of running one independent draft chain per slot — the same idea the commit path already uses, extended to the draft loop. At `-np 2` this roughly halves the draft-decode count per cycle once it engages. | Landed without its own speed window. | Any MTP-capable model served at `-np` > 1. |
| `PXA_KV_SEQ_RM_INDEX` | **0 (off)** | Answers a KV-sequence-removal call by walking a small map instead of scanning the whole cache, with a per-cell tag epoch (not a hand-maintained call-site whitelist) deciding whether the index may answer at all. See `RELEASE-NOTES-2026-09-09.md` for the two defects this replaces and the whitelist failure mode it closes. | CPU: `tests/test-kv-seq-rm-index.cpp`, 4 arms × 60,000 steps. Live-server: a prompt-cache-eviction harness, clean in all three arms. **GPU window RAN: three aged boots, zero divergence, byte-identical to the index off, and speed a wash (+0.2% / +0.3% at `-c 32768`).** No measured win, so the default does not move: it ships as *available*, with its correctness established and its case unproven. | Any model using the unified KV cache; `PXA_KV_INDEX_CHECK=1` cross-checks the indexed answer against a full rebuild on every removal and names the exact disagreement if the two paths diverge — but it is an INSTRUMENT, not a gate, and it is **not bit-neutral**: two of the twelve gate prompts (`p128`, `p1536`) moved against the control with it on, while the on-vs-off comparison without it was exact. Use it to find drift, never in a run whose output you intend to compare. |
| `PXA_FUSE_SIBLINGS` (+ `PXA_SIB_MAX` cap, `PXA_FUSE_SIBLINGS_LOG=1` / `_CENSUS=1` instruments) | **0 (off)** | Merges a run of independent, same-op/same-shape/same-device graph nodes (copy nodes and row-normalisation nodes today) into one launch whose grid is the per-node grids concatenated. Nothing is eliminated — a runtime byte-range check proves no node in the run can see another's write — so this is a pure launch-count reduction, not a fusion in the elimination sense. | Bit-exact by construction; CPU parity proof (`tests/test-narrow-kernel-parity.cpp`) covers both ops. Whether a given decode graph even contains a mergeable run of either op is itself unmeasured — the census mode answers that without merging anything. | Architecture-independent; expected to matter most on graphs with many small per-layer copy/norm nodes (a wide multi-card layer split). |
| `PXA_CPY_ROWS` (`0` off / `1` one thread per row / `2` one warp per row, unconditional / `3` one warp per row, shape-gated) | **0 (off)**; `3` is the only mode with a shipping case behind it | Resolves a copy kernel's source/destination index once per **row** instead of once per element. Mode `3` only engages on narrow, numerous rows (`ne0 <= 256 && rows >= 256`) — the boundary a standalone microbenchmark found, not a guess: one thread per row loses everywhere (up to 140× slower at a wide row), and one warp per row is a loss on every shape *except* many narrow rows, where it measured 2.69×. | Microbenchmarked in isolation on one card, all four variants. Not yet measured inside a real decode graph — whether the shape-gate ever opens on a shipping model's graph is the first open question. | Architecture-independent; the gated mode only pays where narrow, numerous row copies exist (candidate: per-sequence state rows on a hybrid model). |
| `PXA_FA_F16_KV_CHUNK` (tokens per chunk; `PXA_FA_KV_CHUNK_GRAN` rounds the request) | **0 (off)** | On a card with no fp16 matrix-multiply hardware serving a **quantized** KV cache, converts that cache to fp16 in bounded chunks instead of one whole-tensor conversion sized for the graph's maximum reserved context, with an online-softmax merge folding the chunks back together. Caps the conversion scratch buffer at a fixed size regardless of context depth — a VRAM-headroom lever, not a speed lever; prefill is expected to give back a small amount at depth. | CPU: `tests/test-fa-chunk-merge`, 7 cases, bit-identical at one chunk, a bounded fp32 round-off argument above it. **Has no effect on a server running an uncompressed (f16) KV cache** — none of today's shipped serving recipes set a quantized KV cache, so this lever is presently a no-op on every seat until a deployment adds one at long context. | Pre-Turing cards without fp16 tensor cores (e.g. Pascal), only once `-ctk`/`-ctv` request a quantized KV cache. |
| query-time sparse attention: `PXA_QSA` (master), `PXA_QSA_GATHER` (1 = physical gather / 0 = reference mask), `PXA_QSA_GRID_INCR` (append-only candidate pool), `PXA_QSA_FAST_TOPK` (single-launch radix-select top-k), `PXA_QSA_MIN_FILL` (depth gate), `PXA_QSA_PROF` (instrument) | `PXA_QSA` **0 (off)**; `PXA_QSA_GRID_INCR` **1** when QSA is on; `PXA_QSA_FAST_TOPK` **0**; `PXA_QSA_MIN_FILL` **65,536** cells | Selects a subset of cached keys once the cache is deep enough that selection is cheaper than dense attention, instead of attending densely. This release's work is entirely in the selection machinery's own overhead — an append-only candidate pool instead of a full cache scan every token, one fused reduction instead of several separate ones, and a single-launch top-k instead of a full sort — not in the underlying selection idea, which already shipped off. `PXA_QSA_MIN_FILL` exists because below it dense attention measured cheaper than selecting; the lever is not meant to engage there and does not. | Identity-proven: the selected path reproduces the dense reference's output exactly on the fixtures gated so far, including the append-only pool and the fast top-k individually. Speed: mixed — milestone 1 found the *mechanism* correct but slower than dense at the fills tested; milestone 2's overhead work is the attempted fix and its own crossover point (if one exists at the fill depths this project runs) is `<GATE>`. | Long-context hybrid models with a full-attention component (the fill floor is what makes this a long-context-only question). |
| `PXA_RS_RING` | **0 (off)** | Expresses a rejected speculative draft's recurrent-state rollback as an index move into a small retained-state ring, instead of a device-side copy of the whole recurrent state per rejected token. | **MEASURED LOSS.** 27B PXQ4 on the V100 pair, on/off decode ratio 0.964 at np1 and 0.980 at np2 with `n_max=1`; 0.823 and 0.948 at `n_max=3`, where draft acceptance also falls from 0.825 to 0.405 — the ring is costing more than the copy it replaces, and costing most where the depth is deepest. INERT ON EMITTED TEXT: greedy output byte-identical off vs on, six fixed prompts, two boots. There is no refusal guard, so it will engage if you arm it; it ships off because it loses, not because it is unsafe | Any hybrid/recurrent model running speculative decoding. |
| `PXA_FUSE_DELTANET` (existing lever; shipped mask unchanged) | on, mask **53** (bits 0, 2, 4, 5) | One bit of this mask (bit 2 — folding a recurrent-state-row gather into the reset-mask multiply that immediately follows it) has been part of the shipped mask since early in this project but never actually executed: the same-device guard every fusion in this file shares used to read the first bytes of a host-resident input tensor as if they were a device id, which reliably declined bit 2's fusion (and, in principle, any other fusion whose window happened to include a host-side input view). That guard is now generic — it checks the buffer type before reading a device id from it — so bit 2 fires on real traffic for the first time since it was written. The mask value itself did not change; what it does, did. | Bit-exact by construction, CPU-proven on both destinations the fused kernel writes. GPU speed delta on the seat this fires on is `<GATE>` — expected small and positive, since the starting point is a kernel launch that was pure overhead. | Any gated-delta-net hybrid model (the fusion's pattern only matches that graph shape). |

**Housekeeping.** Two existing, unconditional (no env gate) mechanisms were traced to this
project's own work during this release's research and had no ledger record: the repetition-penalty
sampler already walks only the touched-token window instead of scanning the full vocabulary
(`src/llama-sampling.cpp`), and the top-k sampler prefilter already restructures the whole hot path
around a bounded top-K scan with a live shadow-verification fallback
(`common/sampling.cpp`). Both predate this release; they are noted here only so neither gets
rebuilt from scratch by a future pass over this file.

### PXQ codebook decode levers — measured, none earns a default (2026-09-09)

Four levers that change how a PXQ codebook is read in the decode kernel, measured on the four-card
seat, decode at one request, median of 7, with a parity check (207/207) and a greedy-identity check
(6/6) on every arm. All four are **bit-exact against the unmodified path**, so this is purely a
speed question — and none of them answers it well enough to move.

| lever | decode | verdict |
|---|---|---|
| *(none)* | 25.26 t/s | the baseline |
| `PXA_PXQ6_PAIRLUT` | 25.17 t/s | wash |
| `PXA_PXQ6_SHFL` | 25.17 t/s | wash |
| `PXA_PXQ6_PRMT` | 22.09 t/s | **−12.6%, a clear loss — stays off** |
| `PXA_PXQ3_PAIRLUT` | 25.58 t/s | +1.3%, noise-adjacent at n=7 |

The +1.3% one is the interesting case: it is the only arm that is faster, and I am still not
flipping it, because at n=7 a 1.3% difference is inside what I would expect from run-to-run spread
alone. A default that rests on a number I would not defend is worse than no default change.

### `PXA_SPEC_ADAPTIVE_LOAD` — **OFF by default** (was ON 2026-09-10 → reverted 2026-09-11)

Draft depth follows how many slots are actually decoding: full at one, about `n_max/S` at S, zero
at the cutoff (`PXA_SPEC_ADAPTIVE_LOAD_OFF_AT`). Depth only — acceptance is unchanged, so it cannot
move a token at any temperature. `PXA_SPEC_ADAPTIVE_LOAD=1` arms it.

| arm | aggregate decode |
|---|---|
| 4 clients, static depth | 57.62 t/s |
| 4 clients, adaptive | **78.93 t/s (+37%)**, bands disjoint |
| 1 client, static (base band) | 43.5–53.2 t/s |
| 1 client, adaptive (control) | 49.94 t/s — inside the base band, draft stats identical |

> **⚠ THE TABLE ABOVE IS WITHDRAWN — DO NOT QUOTE IT.** Recorded 2026-09-09; **reproduction
> FAILED 2026-09-11**. The same binary (commit `6caeb014db`), the same arms, the same cards (2,4)
> and the same arm order, 29 h later, did not reproduce the +37%, and the flip to default-ON was
> reverted. The reason the original number was believable at all is the part worth keeping: the
> 4-client arm's own **control drifts 16.7% run-to-run** and the server-TTFT arms **20.3%**, so a
> delta of this size is not resolvable at n=5 — and it was read as signal. The single-client
> control was not the load-bearing half it was written up as; a control that wanders as much as
> the effect cannot license a default either way.

The mechanism stays, the lever stays, and OFF is what every release before 2026-09-10 shipped —
so the revert returns to a measured state rather than choosing a new unmeasured one. To re-open it
as a default the harness has to come first: per-rep interleaving with a co-measured control
published beside every delta, and a pre-registered gate that VOIDs the run when the control's own
drift is as large as the effect claimed.

### Measured and left OFF (2026-09-09/10)

Recorded because "we tried it" is worth as much as "it worked", and the next person deserves not to
re-run these:

| lever | verdict |
|---|---|
| `PXA_VOLTA_CUBLAS_TILE` | a **loss** |
| `PXA_MTP_DRAFT_CACHE_ONLY` | wash |
| `PXA_MTP_BATCH_SLOTS` | wash — its crash fix ships regardless, the lever does not |
| `PXA_SLOT_FORK` | a win, but without a green determinism gate behind it |
| `PXA_CKPT_EVICT=value` | wash |
| hybrid `-sm graph` | not possible on a PXQ file at all |

---

## Update — 2026-09-10 — `PXA_PXQ_SSM_OUT` (the DeltaNet out-proj floor)

| lever | default | what it does | measured | gate |
|---|---|---|---|---|
| `PXA_PXQ_SSM_OUT` | unset (= `q8_0`, the constant `PXA_PXQ_SSM_OUT_PROTECT` in `src/pxa-pxq-policy.h`) | Quantizer-side. `blk.N.ssm_out.weight` — the DeltaNet output projection — is floored at `q8_0` on every PXQ level and under every policy. This env names a different type (`q8_0`/`pxq6`/`pxq4hq`/`pxq4`/`pxq3`/`pxq2`) or `off` to drop the floor and reproduce the pre-2026-09-10 allocation for an A/B. It is a **floor**: an explicit `--custom-q 'ssm_out\.weight=…'`, a PXQU map entry, the geometry demote, the MTP companion's pin and a deliberately flat-MXFP4 backbone (`PXA_PXQ_BACKBONE=legacy`/`=lite`) all still decide. | **Yes, and this is the one row here that changed a default.** Qwen3.8-27B, mean KLD vs its own `Q8_0` source (wikitext-2, `-c 2048`, 10 chunks, one class changed): `pxq4` **0.433** → `q8_0` **0.042**; `mxfp4` 0.060, `pxq4hq` 0.049, `pxq6` 0.044. Same-top-p 93.05% → 93.26%. File +5.10%. Decode unmoved: two P100s, `-c 8192`, np1 medians of 7 — 18.29 / 18.10 / 17.83 / 17.81 t/s for `pxq4` / `mxfp4` / `q8_0` / `pxq6`, a 2.7% spread with no separation. | Changes the artifact, not the kernel. `q8_0` and not the 4x cheaper `pxq6` **only** because `pxq6`/`pxq4hq` have no vLLM decoder and a file is routed by its level, not its contents — see the cap note in `src/pxa-pxq-policy.h`. Read `pxa.pxq.backbone_map` in the output to see which type a given file actually got. |

**Why this needed a lever at all.** `ssm_out` was moved onto the PXQ backbone on 2026-08-25 on a
byte-parity argument, with the source comment recording in as many words that it was *not*
measured on a paired perplexity. Every PXQ level shipped it at four bits from that day: the dense
branch hands it the named level, and `balanced`/`attn4` buy it up only as far as the `pxq4` cap,
which reads like protection and is the same allocation. The 7x KLD gap above is what that cost.
The lever exists so the next person who wants to move this class has to name a type and can A/B
it against `off`, rather than discovering the allocation from a file's byte count.

## Update — 2026-09-12 — the six default-ON gates that had no row

Six levers ship **ON** in the merged default and appeared nowhere in this file; none of them printed
a startup line either. A census of every `PXA_*` the engine reads found them: each occurred in the
tree only as a `getenv()` argument plus the comment around it. This section is the documentation
half of that debt, closed. The other half — a boot line per gate — is **outstanding** and stated at
the end of this section rather than left implied.

**Read the `measured?` column literally.** Two of the six are correctness guards whose ON state *is*
the fix, so there is no lever effect to measure; the question is whether the defect is gone, and for
one of them the answer is that it is unexercised rather than proven. The rest are performance
defaults whose mechanism is backed by the evidence named in the row but which have **no paired A/B
of the gate itself**. Do not quote a gain for a lever whose row says unmeasured.

| lever | default | what it does | measured? | arch / card |
|---|---|---|---|---|
| `PXA_DN_REDUCE_VIEWFIX` | **1 (on)** — `=0` restores the pre-fix shape | Correctness guard, graph-split path (`src/llama-delta-net.cpp:19`). A `GGML_OP_REDUCE` partial can collapse to a **view** once the graph is split, and the reduce's consumers then address rows as if it were still a materialised tensor — so they read the wrong rows. ON keeps the delta-net partial a real reduce result; `=0` leaves it a view and reproduces the defect. | No paired A/B, and one is not constructible from this lever alone — see the note below the table. **Not gated, and no gate of this lever exists to run:** the separating experiment would compare the fixed binary with itself. | Any multi-device topology that splits the graph. |
| `PXA_DN_SPLIT_INPUT_FIX` | **1 (on)** — `=0` omits the marker | Correctness guard (`src/llama-delta-net.cpp:23`). Emits the per-device `0xff` split-boundary marker on the delta-net input, so a device can tell a split boundary from real data. `=0` omits the marker and reproduces the old behaviour. | No paired A/B, same reason as the row above. **Not gated**, same reason. | Any multi-device topology that splits the graph. |
| `PXA_SM_GRAPH_REDUCE_CONSUMER` | **1 (on)** — `=0` restores the pre-fix test | The **consumer-side** half of the same defect family (`ggml/src/ggml-backend.cpp:1562`, `src/llama-delta-net.cpp:36`, `src/llama-build-context.cpp:765` — all three read the same variable). `=0` restores the pointer-identity-only test in `get_input_tensor_sm_graph`, its twin in the build-context path, and the `REDUCE` pinning that shares the variable. This is the lever that actually repairs delivery: the two rows above gate only the delta **call site**, so a run with both of them off is still a *fixed* engine — measuring the first two against a both-off baseline compares the fixed binary with itself. | **CPU-proven and UNEXERCISED.** `tests/test-reduce-split.cpp` — the first regression test `GGML_OP_REDUCE` has ever had — passes 11/11 with the lever on **and** with it off, because it tests the ggml primitive rather than the scheduler; both states passing is the expected result and is therefore not evidence either way. The hardware runs that followed showed the reduce was **not** the cause of the observed graph-split failure, so the fix is not disproven, it is unexercised, and no claim should rest on it having fixed an observed failure. **It has no measurement route on the hardware we have:** one family's models have no split-device linear path at all, and a PXQ file refuses a dim-0 K-split at load. Both are missing *features*, not missing measurements. Keep the test. | Graph-split path, all cards; presently reachable only where a split actually forms. |
| `PXA_F16_GEMV_WIDE` | **1 (on)** — `=0` restores the old dispatch | A small-R F16 decode GEMV that keeps the arithmetic in fp32 (`ggml/src/ggml-cuda.cu:3901`): 8 warps per row with the K extent capped at 512, weights widened to fp32 **before** the FMA, and `x` never rounded to f16. Restricted to the small-R window, so it cannot intercept a real dense projection. The chain it replaces rounds both `x` and the result through f16, so this path is **strictly tighter** — and for that reason **not bit-identical by construction**, which is why it is gauntlet-gated rather than sha-gated. `PXA_F16_GEMV_WIDE_RPW` pins rows-per-warp for an A/B (honoured only when it also tiles R exactly). | Checked standalone against an **fp64 CPU reference** at the live shape: max **absolute** error **6.05e-07** against a max \|y\| of 6.71, i.e. **9e-08 relative** — plain fp32 round-off. ⚠ Do not read the per-row max **relative** error: a row near zero makes it meaningless. No paired A/B of the gate on a serving cell. Gate: behavior gauntlet. | Pre-Turing cards; engages wherever the small-R f16 GEMV shape appears. |
| `PXA_SWA_HINT` | **1 (on)** — `=0` restores the suppression | Passes the sliding-window hint again for sliding-cache layers (`src/graphs/build_muse_glimmer.cpp:112`). The suppression it removes existed because the CPU attention path sliced K/V/mask **by index**; that slice is now mask-driven, which is what made the old suppression unnecessary rather than merely wrong. `=0` restores the suppression without a rebuild, as a fallback. | **No A/B on record.** The row exists because a fallback that needs no rebuild should be discoverable rather than silent. **Not gated:** the gate it would need is a behavior gauntlet on an SWA-interleave architecture, and none has been run, so none is claimed. | Architectures with a full/SWA layer interleave. |
| `PXA_VOLTA_F16_GEMM` | **1 (on) for cc == 7.0 exactly**, off elsewhere — `=0` restores the old dispatch on Volta | Takes the f16-input, fp32-accumulate, **F32-output** `GemmEx` form for every dense GEMM on Volta, matching what mainline does for this arch (`ggml/src/ggml-cuda.cu:2283`). It closes two defects at once: **(a)** an `op_params[0]` alias — a fused up/gate node stores the *unary op id* in the slot the generic branch reads as a precision, sees non-default and vetoes the entire f16 branch, falling to dequantize + `SGEMM`; on the MXFP4 twin of Qwable-27B that was `volta_sgemm_128x128_tn` at **68.5 s = 51.2 % of all GPU time** against 14.1 s of real tensor-core GEMM, and is why that file's prefill sat at 373/317 t/s where mainline gets 940/1195 on the identical file; **(b)** the fp16-accumulate round trip — the old path computed in fp16 into a pool-allocated f16 destination and converted back (`convert_unary<half,float>`, **1.29 s / 10846 calls = 2.1 %** of prefill GPU time at 20801 tokens) plus a pool allocation per call. It also fixes a latent buffer-sizing bug: the old f16 branch sized its pool buffer `row_diff*ncols` but let cuBLAS write with leading dimension `ldc`, which is `ne0` (and `> row_diff`) on the main device of a split tensor. | **Yes** — nvprof on the 2×V100 pair, Qwable-27B, 20801-token prefill, 2 reps, 2026-09-02; the two figures above are from that run. Not bit-exact against *either* old path (fp32 vs fp16 accumulator; f16 vs f32 operands) — the accumulator is fp32 either way, which is the numeric class mainline already ships for these models. Gate: behavior gauntlet. | **Volta only.** Every other arch keeps its exact incumbent behaviour, so this is not a global default change. |

**Why the first two rows have no A/B, said plainly.** `PXA_DN_REDUCE_VIEWFIX` and
`PXA_DN_SPLIT_INPUT_FIX` gate the delta **call site**; the consumer-side repair is
`PXA_SM_GRAPH_REDUCE_CONSUMER`. So the separating experiment ("does the split path deliver the right
rows?") cannot be built out of the first two levers — turning them off leaves the delivery fix in
place, and the comparison degenerates to the fixed binary against itself. One binary, three
isolable hypotheses, and only the third one is the delivery fix.

### Still outstanding: 11 default-ON gates print no boot line

This section closes the **docs** half of that debt for six gates. The **startup-line** half is not
closed, and it applies to eleven of them, not six. The distinction to know as a reader:
`PXA_AUTO:` decision-ledger lines (§0c) cover every **auto-decided** lever — a lever armed by the
(model × device) profile always says so, including when it declines. A lever that is a plain
compiled-in `static const bool` default is not part of that ledger and prints nothing at all, so
"is this gate on?" is not answerable from a boot log for any of them. `PXA_ENHANCE_DBG=1` narrows
that a little — it prints the topology and the chosen auto-set values, plus the tile-f32 FA gate's
resolved state — but it is a partial view, not an enumeration. Treat a gate not named at boot as
**unverified at runtime**, never as off.

---

## Update — 2026-09-13 — every lever that came in with this cut

Nine new `PXA_*` names since `5167e7453a`, the tree the `v2026.09.11-rc3` payload was compiled from.
A tenth name appears in the prose below — `PXA_ALLOW_GRAPH_SPLIT_HYBRID` — and is **not** new: it has
had its own row since 2026-08-09 (the split-mode table above), and that row's footgun warning stands
unchanged. It is named here only because the attention-split admission below is what replaces it for
one model.

**The provenance convention this section uses, because every number in it depends on it.** A *window*
is one lock on one card set, with a no-drafter bracket arm at each end. If the two bracket readings
differ by more than **1.50%** the window is **VOID** and its numbers are shapes, not results — the
box was doing something else (a compile, another job) while the arms ran. Every figure below is
**np1, temperature 0, `REPS 5`**, taken on the **2× V100 PCIe 16 GB pair (cc 7.0, both cards on x4
links)** of the development box. Two prompt classes are reported throughout and they do not move
together: **control** (a repetitive, quotable class where an n-gram table predicts well) and **prose**
(free generation, where it mostly does not).

⚠ **Two files, and which one a row uses decides what the row can claim.** This engine's arms serve
**an abliterated fine-tune of Qwen3.8-27B, in PXQ4** (the seat model on this box) unless a row says
otherwise; a mainline arm serves **stock Qwen3.8-27B in `UD-Q4_K_S`** on `fdb2c11c70`. So **a row
that quotes a mainline number crosses codec AND weights** and is an engine-plus-file comparison, not
a codec one — the stock-PXQ4 windows that would isolate either variable have not been run. **A row
that compares this engine with itself** (every lever A/B here) runs the same file in both arms and is
unaffected by any of that.

| window | when (UTC) | bracket drift | usable as |
|---|---|---|---|
| `casc-h2h` | 2026-09-13 07:12–07:26 | 0.11% | a result |
| `casc2-fair` | 2026-09-13 07:30–07:44 | 0.10% | a result |
| `casc3-nmax` | 2026-09-13 07:47–08:01 | 3.50% | **VOID** — shapes only |
| `casc5-ckpt` | 2026-09-13 08:18–08:32 | 0.00% | a result |
| `reduce-pinned-spd1` | 2026-09-13 08:42–08:58 | 1.88% | **VOID** — sign only |
| `replanfix2-b1` | 2026-09-13 09:35–09:49 | 0.53% | a result |
| `replanfix2-c1` | 2026-09-13 10:14–10:27 | 0.24% | a result |
| `prefill2-v100-w1s` | 2026-09-13 13:45–13:56 | 0.99% | a result for the three long prompt classes; its 512-token class moved −1.81% and is **not reported** |

Builds referenced: `b46d4848b3` (the base this cut merges onto), `698efc92c0` and `39f6afe1d6` (the
reserve/checkpoint work), `116ea9fe22` (the reduce route), `a6796424a6` (the attention-split
admission). Each default below was read off its `getenv` site in this tree, not recalled.

| lever | default (and where it is decided) | what it does | measured? — with the window it came from | gate class / arch |
|---|---|---|---|---|
| `PXA_MTP_STABLE_OUT_IDS` | **1 (on)**; `=0` restores the old shape. `src/graphs/build_qwen35.cpp:22-28` — unset resolves ON | Keeps the MTP head's graph **shape** constant across the two batches a speculative cycle alternates between. The head decodes an N-row update pass (fewer output rows than rows, so it carries an `out_ids` row-slice) and then a 1-row draft pass (one row out of one, so the slice used to be skipped). Two node counts on one context means the allocator's single plan can never cover both, so it re-plans — a full backend drain plus a compute-buffer reallocation — twice per accepted token. Building the slice unconditionally costs two extra `get_rows` on the 1-row pass, where at one row out of one **the slice is the identity**, so no value changes | **Yes, and it is the whole mechanism.** Four arms of ONE binary (`698efc92c0`), separated only by the env, default cascade at `-sm layer`, `-c 32768`: reserve calls / coverage failures / allocator re-plans over a whole boot including prefill — **242 / 174 / 176** with everything off, **7 / 5 / 11** with this lever alone, **7 / 5 / 11** with it and `PXA_RESERVE_SHAPE` together. The 229 width-5 re-reserves per bench (the two MTP passes evicting each other) become **one**. Speed, window `replanfix2-b1`: `casc-l` **86.38** control / **83.30** prose against the same arm on `b46d4848b3` at **83.00 / 82.13** (`casc5-ckpt`) and **85.65 / 81.92** (`casc-h2h`) — +1…+4% control, +1.4% prose | **Bit-exact in the measured cell**: greedy probe sha `a5467dd43144` and draft counters `1013/1030` and `953/1158` identical to the pre-fix arm, and the 512-token control generation identical character for character. The identity argument (at `n_outputs == n_tokens` the slice is the identity) is what makes that the expected result rather than a lucky one. Any model with an MTP/NextN head |
| `PXA_RESERVE_SHAPE` | **1 (on)**; `=0` restores the synthesised-token-batch reserve. `src/llama.cpp:8782-8789` — unset resolves ON | Builds the compute-buffer reserve from the same batch **kind** the context really decodes, instead of a synthesised token batch that asks for `n_outputs == n_tokens`. The reserve is what the graph allocator sizes its single plan from; when its shape does not match the shape the context actually runs, the coverage test fails and the buffers are reallocated | **Measured a wash on every counter, and shipped ON anyway — read both halves.** On top of the stabiliser it adds nothing: the stabiliser-only and both-on arms are identical on reserve calls, coverage failures and re-plans (`7 / 5 / 11`). Alone it cuts reserves 242 → 32 but leaves 121 re-plans — it stops the re-*reserving* without stopping the re-*planning*. In `replanfix2-b1` the both-on arm read **86.38 / 83.30** against **84.94 / 81.71** with `PXA_RESERVE_SHAPE=0` (+1.7% / +1.9%), which is **at the edge of the 1.50% band, not outside it**. It ships as built because there is no fidelity difference and nothing to gain from a rebuild to flip it — not because 1.7% was believed | Bit-exact: same probe sha and same counters in both arms of `replanfix2-b1`. Architecture-independent (it is a scheduler-reserve shape, not a kernel) |
| `PXA_RESERVE_GUARD` | **1 (on)**; `=0` disables the thrash guard. `src/llama.cpp:8792-8798` — unset resolves ON | A latch: once a context has re-reserved `PXA_RESERVE_MAX` times, the mechanism turns itself off **for that context** and says so, rather than paying a drain and a reallocation on every ubatch whose shape it cannot cover. It also blacklists a shape after one failed try, which is why a residual mismatch costs a handful of events per boot instead of one per cycle | **Yes, as a bound rather than a speed.** With the stabiliser on, a whole boot shows **5** coverage failures and **11** allocator re-plans — per boot, not per cycle — and that ceiling is the guard's doing. The warning it prints names the node count and the width it could not cover (`src/llama.cpp:7916`, `:7922`) | Bit-exact — it changes when buffers are reallocated, never what is computed. Architecture-independent |
| `PXA_RESERVE_MAX` | **24**. `src/llama.cpp:8801-8806` — unset resolves to 24 | How many re-reserves one context may take before `PXA_RESERVE_GUARD` latches it off | **Not swept.** 24 is a bound chosen so a healthy boot (5 failures) never reaches it and a pathological one stops early; no arm has measured a different value | As above |
| `PXA_RESERVE_DEBUG` | **0 (off)**. `src/llama.cpp:8773-8779` — unset resolves OFF | **Diagnostic.** On a coverage failure, prints the node-by-node difference between the reserved graph and the real one, plus the device-side buffer picture. A node count is not a diagnosis; the two nodes that differ are | **It is the instrument that named the residual, and it needed two fixes before it could.** First it read the real graph *after* the reserve had rebuilt it in the same buffer, so all 174 of its diffs compared the reserve with itself. Second, its node signature carried dimensions — and the reserve is built at width 2048 while the graph it is compared with is 2 to 55 rows wide, so every one of 3078 nodes differed and buried the two that mattered. With both fixed (`39f6afe1d6`) the answer is identical at every failing width: the two extra reserved nodes are `GET_ROWS attn_out-63` and `GET_ROWS sainp_get_rows-63` — the same `out_ids` row-slice, this time in the main model's last layer | Diagnostic; costs log volume, changes nothing. Architecture-independent |
| `PXA_CKPT_BUDGET` | **1 (on)**; `=0` disables the clamp. `src/llama.cpp:11718-11724` — unset resolves ON | A recurrent (DeltaNet/GDN) target checkpoints every speculation window, and the **per-step** checkpoint buffers scale **linearly with the draft length**. Auto mode used to claim them whenever the allocation merely *succeeded* — and fitting is the wrong test, because the CUDA VMM compute pool grows afterwards. The budget asks the question the allocator does not: after these buffers are claimed, does every participating device still hold the compute buffers it has already reserved, plus `PXA_CKPT_BUDGET_MARGIN_MB`? The largest draft length for which the answer is yes becomes the capacity, and **every drafter stage is clamped to it** (`common/speculative.cpp:1618-1665`). It allocates nothing to decide — the per-step layout is arithmetic over the layers. It applies only to `auto` and `per-step`; `gpu-fallback` and `cpu` keep small shadows whatever the draft length is | **Yes — and the failure it prevents is measured too.** In window `replanfix2-c1` a 64/48 n-gram stage at `-sm layer` ran to completion at `-c 32768` on these 16 GB cards with **exact** per-step checkpoints and no fallback: the boot line read `this context can checkpoint 53`, drafts were clamped to 52, and the arm posted **185.88** control against mainline's **140.47** in the same window (cross-engine, so different weights as well as a different codec — see the scope note). The same request at `-sm attn` resolved to capacity 20 (drafts ≤ 19) — the split leaves less headroom — and the clamp is why that arm's control is modest rather than why it crashed. Per-step buffer cost on this pair, read from the allocator's own lines: `max_tokens=5` **377.58 + 226.55 MiB**, `max_tokens=33` **2996.02 + 1797.61**, `max_tokens=65` **5988.52 + 3593.11**. At 65 the first card's budget is weights 7587.75 + KV 1245.52 + compute 432.93 + checkpoint 5988.52 = **15254 of 16384 MiB**, leaving about 0.7 GB for the pool — and the server dies there about a dozen speculative cycles in. That is the whole reason a 64-token n-gram draft would not run at `-c 32768` on a 16 GB card, and it is a **startup** allocation, so no scheduler escape hatch avoids it | Changes draft *depth*, never arithmetic: a clamp lowers `n_max`, and acceptance is unchanged. Any recurrent/hybrid model under speculation on a VRAM-tight card |
| `PXA_CKPT_BUDGET_MARGIN_MB` | **256**. `src/llama.cpp` — unset resolves to 256 | The room the budget leaves free on every device **on top of** the compute buffers already reserved there. It exists because the pool grows during decode — a graph re-plan reallocates the compute buffers *before* it frees the old ones — so "exactly enough" is not enough | **Swept 2026-09-14** on the 16 GB V100 pair at `-c 32768 -np 2 --kv-unified` (Qwen3.8-27B PXQ4), which is the shape where the margin is paid once per state slot: margin **1024** -> capacity 14/chain, effective `n_max` 13, control decode **129.90 t/s**; **512** -> 16, 15, **144.75**; **256** -> 18, 17, **161.96**; **128** -> 19 then re-fitted down to 18, 17, **158.90**. All four survived a lone request and two concurrent ones. 128 buys no rows over 256 and is the only arm that had to re-fit, so **256 is the shipped default** (+24.7% control decode at two slots against the old 1024) and 128 is the floor you should not go below. At `-np 1` the same window shows the margin is paid once rather than not at all: capacity goes 49 -> **57** and effective `n_max` 48 -> **56**, so a one-slot server drafts longer on the new default too. Safety was proved before the default moved, at both `-np 2` and `-np 1` on the 16 GB pair: the largest prefill this context can ask for (20,801 tokens), the largest concurrent prefill pair the unified ring holds, a 512-token decode on both slots, an 8k two-fact needle, and lone plus concurrent requests, with both cards' free VRAM read while the server was live and the server log clean of out-of-memory text. `-np 1` is the tighter of the two for headroom (867 MiB free on the first card with the server live) and it still took that prefill. **128 is the floor**: it is the first value that had to re-fit its own capacity downward on the next boot line, which is the margin failing at its job | As above |
| `PXA_REDUCE_PINNED` | **0 (off)**. `ggml/src/ggml-cuda/reduce.cu:196` — set-and-non-zero required | A third route for the cross-device reduce the split modes need: one kernel staging the partials through **pinned host memory** with an in-kernel spin on a host-memory arrival token, instead of the in-tree peer-BAR copy path. It is the shape a two-card allreduce takes when there is no peer path at all. The boot prints which route resolved; a shape the predicate refuses falls back to the ring path | **MEASURED LOSS — it ships off, and the reason is now a number rather than a guess.** Per reduce, with `PXA_REDUCE_TIME`, `-sm attn` + MTP on `Q4_K_S`, 256-token decode, the dominant `ne1=4` bucket (n=10972 samples): in-tree **GPU span 46.87 µs / host enqueue 41.42 µs**; pinned **66.83 / 13.08**. The host saving is **2.95–4.18×** in every bucket and the device cost **1.30–1.45×** in every bucket. Speed, window `reduce-pinned-spd1` — **VOID at 1.88% drift, so this is a sign and not a size**, and the drift biases the pinned arms (which ran second) *downward* by about 2% while the deficit is 6–18%: `a-mtp-q4` 48.51/51.04 → **42.80/42.13** with it on; `casc-a-x` 89.71/71.09 → **83.07/66.82**. Draft counters identical between each arm and its pinned twin (`785/1460`, `765/1510`, `1010/1030`, `929/1212`) and probe shas identical, so it is rate-only. **Why it loses:** on a pair with working peer access the decode loop is *device*-bound between splits, so 28 µs of enqueue saved per reduce buys nothing against 20 µs of extra span across the ~130 reduces of a verify pass | **Bit-exact, proven on the card**: `tests/test-reduce-pinned.cu` (label `cuda`), **32/32**, zero differing elements on both devices at `[5120,1]` (decode), `[5120,4]` (the MTP verify batch), `[37,3]` (vector tail) and `[5120,64]`, against a real single-device sum. `[5120,64]` is refused by the predicate on purpose and falls back, still bit-identical. Two-card CUDA topologies; the companion scheduler-drain skip it carries is **inert on two backends** and live only on a >2-backend async graph split |
| `PXA_REDUCE_TIME` | **0 (off)**. `ggml/src/ggml-cuda/reduce.cu:451` and `ggml/src/ggml-backend.cpp:1578` — unset resolves to 0 | **Diagnostic.** `=<n>` prints a per-route table every `<n>` completed reduce samples: GPU span and host enqueue, bucketed by `ne1`, plus a row for the scheduler's per-reduce backend drain. The first instrument this op has ever had on a card | **It is how the row above got its answer**, and it also retired a claim: the per-reduce backend drain that the pinned route was partly built to remove **never ran** in the two-card configuration — its row is empty in both arms, because the drain sits behind `n_backends > 2 && split_mode_graph && has_reduce` (`ggml/src/ggml-backend.cpp:2781`) and a two-card server does not satisfy it | Diagnostic; costs a timer per reduce and some log volume. Any CUDA topology that forms cross-device reduces |
| `PXA_ATTN_SPLIT_QWEN38` | **0 (off)**. `src/llama.cpp:3993` — set-and-non-zero required, **and** the model must match. Admission is the conjunction | **Admission only.** The load-time guard demotes `-sm attn` (and `-sm graph`) to `-sm layer` on every DeltaNet hybrid architecture. This lever admits `-sm attn` for **exactly one model shape** — `qwen35` with the Qwen3.8-27B hparam fingerprint (`n_layer 65`, `n_embd 5120`, `n_ctx_train 262144`, `n_head 24`, `n_head_kv 4`, `n_ff 17408`, head k/v 256, `n_rot 64`, ssm `conv 4 / inner 6144 / state 128 / dt_rank 48 / group 16`, `nextn 1`, rope sections `{11,11,10,0}`) — and leaves every other file demoted exactly as before. Nothing here changes arithmetic: no kernel, no reduce, no `-ts` handling. `-sm graph` is **not** admitted and stays demoted. When the lever is on and a `qwen35` file does not match, the boot prints the observed fingerprint field by field against the wanted one, so the failure mode is never a silent no-op | **Yes, and it is the largest single decode lever in this cut on this model.** Window `casc-h2h`: the default cascade at `-sm attn` reads **103.87** control / **77.56** prose against **85.65 / 81.92** at `-sm layer` — **+21.3% control, −5.3% prose**. Window `replanfix2-b1`: the MTP head alone reads **66.14 / 41.53** at `attn` against **58.56 / 37.63** at `layer` — **+12.9% / +10.4%**, and 66.14 is level with mainline's own MTP-only control (66.31 in `casc-h2h`, 64.71 in `casc2-fair`) — a cross-engine cell, so different weights; the lever's own delta is the `attn`-versus-`layer` pair, which is one file. Both arms above also carried `PXA_REDUCE_NCCL=0`, which is a route choice and is quoted as part of the configuration, not attributed to this lever | **MEASURED 2026-09-13, and it stays default-OFF because the criterion was pre-registered.** The plain-graph fidelity gate scoped itself *out* of the MTP verify batch — the path these numbers run on — so a new instrument was built for it: `llama-perplexity` at the speculative verify width, 12 chunks × 512, 2× V100, **stock** PXQ4. Against the layer split: **mean KLD 0.000684 prose / 0.0106 repetitive**, against **0.000689 / 0.0098** for the flash-attention on/off toggle on the same instrument — i.e. the same level as a toggle that ships on. The **drafted rows are no worse than the accepted row**; greedy text and draft counters are **byte-identical** to the layer split on the harness (the probe sha `a5467dd43144` match is consistent with that, and was never the gate). **What it misses** is the pre-registered default-on criterion on two tail statistics: top-1 agreement short by **2 rows in 3060**, and one wider worst-case logit. Registered before the numbers, so not renegotiated after them — it ships as a measured option. Default-ON is deferred until the **rejection and checkpoint-restore path** has an instrument of its own, that being what a tail statistic most likely reports. `qwen35` / Qwen3.8-27B only, on a two-card CUDA topology |

**The PXA attention split, and what that name does and does not claim.** The `-sm attn` split mode
originated in `ik_llama.cpp` (`ikawrakow`, 2025-12); **the PXA attention split** is this project's
version of it — the PXQ panel-aware cut, the hybrid (delta-net) reduce-delivery fixes and demotion,
the reduce route control, the model-exclusive admission and the headroom balance: the parts that make
it run on hybrid models, on the PXQ codec, on Pascal and Volta. The mode, the split builder,
`ggml_reduce` and its transports, the recurrent-layer split and the `qwen35` support behind it
entered this line with the 2026-07-17 import. `PXA_ATTN_SPLIT_QWEN38` does not implement a split — it
**admits** one, which is why its row says admission only. Spelled out, because the rows above depend
on it, the parts that make the mode runnable on this codec and this architecture class are the **panel-aware split granularity** (a PXQ
tensor holds one fp16 anchor per 64-row panel header covering all of `K`, so it can only be cut on
panel boundaries; a cut anywhere else reads the slices at the wrong byte offsets in silence, because
every assertion on that path is about sizes and PXQ satisfies those exactly), the **hybrid delta-net
reduce-delivery fixes** with their demotion guard and consumer-side fallback (`PXA_DN_REDUCE_VIEWFIX`,
`PXA_DN_SPLIT_INPUT_FIX`, `PXA_SM_GRAPH_REDUCE_CONSUMER`, their rows above), the **reduce route
control surface** (`PXA_REDUCE_NCCL`, `PXA_REDUCE_PINNED`, `PXA_REDUCE_TIME`), and the
**model-exclusive admission lever and its fingerprint**. Upstream also ships `-sm graph` and MTP
self-speculation of its own, so any head-to-head against it has to arm those — measuring it at its
defaults measures the wrong thing.

**`PXA_ALLOW_GRAPH_SPLIT_HYBRID` is not the same thing and must not be used as if it were.** It is a
blanket bypass that admits `qwen3next` / `qwen35moe` / `qwen35` / `qwen4exp` alike, its own row says
in as many words that it must not serve traffic, and the row's old speed figures are **VOID** because
they were taken on a configuration that was emitting NaN. The admission above is a whitelist of one
model shape; the bypass is the absence of a guard. Both exist; only one of them is a feature.

### `PXA_FA_TILE_512` — ⚠ NOT IN THIS CUT'S BINARY, and it carries a known depth defect

Recorded here because the finding is worth more than the flag, and because the next person to reach
for it deserves to meet the defect in the documentation rather than in their output.

**It is not in this tree.** A grep of the cut returns nothing; the kernel lives on a branch that has
not merged. No row in this file may be read as describing a flag the shipped binary contains — that
mistake has its own entry above (`PXA_PXQ_KV`), and this note exists so it is not repeated.

**What it is.** A flash-attention tile kernel for head dimension 512, written because Gemma 4's eight
global layers have no pre-Turing flash-attention path and fall to the CPU backend today.

**The defect, measured on the card.** Its accumulator is **f16 and saturates with depth**: normalised
mean-squared error **2.8e-1 at a 8,192-token KV** and **2.8 at 20,480** — that is, the error does not
creep, it grows with cache depth until the result is meaningless. It is therefore **a lever with a
known defect, not a lever awaiting a measurement**, and it stays that way until the fp32-accumulator
rebuild lands and measures.

**No speed number is quoted for it, here or anywhere.** A throughput figure taken through a
saturating accumulator describes a kernel nobody would ship. The Gemma 4 sections carry no speed
sentence for this path for the same reason, and that is deliberate rather than an omission.

### Volta prefill against mainline — one confound fewer than the decode rows, not zero

Every decode number in this section runs an abliterated fine-tune on this side against stock weights
on the other. **This one runs stock on both sides** — this engine on the stock PXQ4 (md5 `11d6dd2a`,
build `39f6afe1d6`) against mainline `fdb2c11c70` on `UD-Q4_K_S` (md5 `b3853896`) — so the weights
confound is gone. **What remains is PXQ4 against `Q4_K_S`: codec and engine together, neither
isolated.** The equal-codec decomposition, this engine driving the same `Q4_K_S` file, has not run
**on this pair**; on four P100s it has, and it reverses the sign — mainline is 9.4% faster there at
equal codec (198.24 against 218.81 at 3,121 tokens, window `lb-shake1`), so that card set's +7.3%
headline is the codec carrying an engine deficit. Published in `bench/LEADERBOARD.md`. Window `prefill2-v100-w1s`, 2× V100 (cards 2 and 4), `REPS 3`,
prompt-processing tok/s:

| prompt tokens | this engine, PXQ4 | mainline, `Q4_K_S`, best prefill split (layer) | delta |
|---|---|---|---|
| 3,121 | 1,348 | 956 | **+41.1%** |
| 8,192 | 1,404 | 1,017 | **+38.0%** |
| 20,801 | 1,324 | 964 | **+37.3%** |

**The 512-token class is not reported.** Its bracket moved **−1.81%**, outside the band that voids a
class, and mainline's own 512-token arm spread 21% across repetitions; an unstable number is dropped,
not discounted. Mainline is quoted at its **layer** split because that is its faster prefill shape —
its tensor split, which wins its decode cells, is slower here. **The Pascal cell is measured** — on
stock files, window `lb-shake1` — and lives in `bench/LEADERBOARD.md` rather than here, because a
picker constant that moves it is still in flight and its competitor revisions are pinned; read it
there with its provisional marker.

### The n-gram stage's draft length, and the line that tells you what you got

The default speculation for a file that carries an MTP head is a **cascade**: an n-gram stage in front
of the trained head (`examples/server/server.cpp:1287-1294`). The n-gram stage is built from the bare
`ngram` **alias**, and an alias fills its own measured defaults
(`common_speculative_apply_alias_defaults`, `common/speculative.cpp:222-232`): the stage drafts
**`n_max = 4`, `n_min = 2`**, with a hash width of `ngram_size_n = 16` and `ngram_size_m = 48`,
`ngram_min_hits = 1`.

⚠ **Two different 16s sit a foot apart here, and confusing them is easy.** `speculative.n_max = 16`
(`common/common.h:236`) is the global draft length for a stage that carries **no** override — and the
alias always leaves one, so it never applies to this stage (`common/common.cpp:116` and
`common/speculative.cpp:1493` both resolve `stage.has_n_max_override() ? stage.n_max : params.n_max`).
`ngram_size_n = 16` is the **hash width**, a quality knob, not a draft length. The default cascade
drafts **four** n-gram tokens, and two boot lines say so independently: the stage line
(`PXA_SPEC: self-spec stage ngram_mod: n_max=4, n_min=2, ngram_size_n=16, ngram_size_m=48, ngram_min_hits=1`)
and the checkpoint budget line (`the chain drafts up to 4 tokens (capacity 5)`, which is
`get_max_stage_n_max() + 1`).

Longer drafts are a per-stage option, through the same parser the auto layer uses
(`common_speculative_stage_from_arg`, `common/common.cpp:1026`):

```
--spec-type ngram:n_max=64,n_min=48
--spec-type ngram:n_max=64,n_min=48 --spec-type mtp:n_max=3      # the two stages, explicitly
```

**Read the boot line before believing the length you asked for.** `PXA_CKPT_BUDGET` prints one line
on every recurrent model, clamp or no clamp, because which capacity a run actually got is not visible
anywhere else — and two arms differing only in it have been compared as if they were like for like:
...
and, when the request does not fit:
...
A third line warns when even a two-token per-step checkpoint cannot be afforded, in which case the
checkpoint mode is left to resolve itself.

**What a longer stage buys and what it costs, on the two classes.** Window `replanfix2-c1`, every arm
on exact per-step checkpoints under the budget, against mainline's own n-gram+MTP cascade at its tensor
split in the same window. ⚠ The mainline row is **stock Qwen3.8-27B in `UD-Q4_K_S`**; every other row is
**the abliterated fine-tune in PXQ4**:

| arm | control | prose |
|---|---|---|
| mainline, `ngram-mod` + MTP, tensor split | 140.47 | 73.35 |
| this engine, default cascade (n-gram stage at **4**), `-sm layer` | 86.73 | **83.83** |
| this engine, n-gram stage at 32, `-sm layer` | 128.45 | 57.43 |
| this engine, n-gram stage at 64/48, `-sm layer` (budget: capacity 53, drafts ≤ 52) | **185.88** | 65.62 |
| this engine, n-gram stage at 64/48, `-sm attn` (budget: capacity 20, drafts ≤ 19) | 109.52 | **106.99** |

Read it as one shape and not five numbers: **control rises with draft length and prose falls with it**,
on both engines, and mainline's fixed 64/48 makes exactly the same trade — it buys the repetitive class
and pays for it in free generation. A longer fixed length is therefore not a better default; it is a
different bet on the workload. The mechanism that takes both classes is an **acceptance-adaptive**
length — grow toward the ceiling on a high-acceptance streak, fall back toward the default 4 on misses.
That mechanism now **exists and is measured, and it is not the default of this release**: at one slot
it is −25.5% against mainline's cascade on repetitive text (37.36, barely above the 36.11 of no
drafter at all), and at two slots it falls **below running no speculation** on prose (36.26 against
49.61) with acceptance halved (0.473 against 0.858) and a bimodal spread that a median hides. Both
classes are open. The release notes carry the full np1 and np2 tables; this row will name the lever
and its cell constants when the branch that carries it lands in the cut — **it is deliberately unnamed
here, because a lever row that names an environment variable the shipped binary does not contain is
the `PXA_PXQ_KV` defect above, repeated.** The `attn` row is the same point from the other side: the budget clamps it to
19-token drafts on this card pair, which is why its control is modest and its prose is the highest of
any arm measured.
