...
class Gemma4Model(Gemma4BaseModel):
    ...
    def set_gguf_parameters(self):
        ...
        # The sparse variant spells its routing hyper-parameters "num_experts" and
        # "top_k_experts". The shared base only recognises "num_local_experts" and
        # "num_experts_per_tok", and this class does not chain to it, so without these two
        # lines a converted sparse checkpoint carries expert_count = 0: the runtime then
        # builds a dense block and rejects the expert tensors it finds in the file.
        n_experts = hparams.get("num_experts") or hparams.get("num_local_experts")
        if n_experts:
            self.gguf_writer.add_expert_count(int(n_experts))
            n_experts_used = hparams.get("top_k_experts") or hparams.get("num_experts_per_tok")
            if n_experts_used:
                self.gguf_writer.add_expert_used_count(int(n_experts_used))
    ...
    # The sparse variant ships its routed experts already stacked, under HF names that end in
    # "gate_up_proj" / "down_proj" rather than ".weight". prepare_tensors reads that suffix off
    # the SOURCE name and forces anything without it to F32, so the two largest tensors in the
    # model -- 22.8 G parameters of expert stacks, 89% of the checkpoint -- would be written at
    # 4 bytes each whatever --outtype asked for, turning a 51 GB conversion into a 95 GB one.
    # Returning True marks them "quantize as the file type says", which is what every other
    # 2-D weight in the file already gets.
    def tensor_force_quant(self, name: str, new_name: str, bid: int | None, n_dims: int) -> gguf.GGMLQuantizationType | bool:
        del name, bid
        if n_dims >= 2 and new_name.endswith("_exps.weight"):
            return True
        return False
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        # The checkpoint packs each block's routed experts as ONE tensor, gate rows first:
        # experts.gate_up_proj is [n_expert, 2*n_ff_exp, n_embd]. Both halves are whole rows, so
        # writing them as two tensors is a re-slicing and nothing else -- every row keeps its own
        # quantisation scale either way. It is worth doing because the two layouts do not reach
        # the same code: the fused expert-stacked decode and prefill drivers take a separate gate
        # and up operand, and a merged tensor arrives at them with the gate operand null and is
        # declined, leaving the experts on the generic per-expert loop. Set
        # PXA_GEMMA4_MERGED_EXPERTS=1 to write the single merged tensor instead.
        if bid is not None and name.endswith("experts.gate_up_proj"):
            if os.environ.get("PXA_GEMMA4_MERGED_EXPERTS", "") not in ("", "0"):
                return [(self.format_tensor_name(gguf.MODEL_TENSOR.FFN_GATE_UP_EXP, bid), data_torch)]
            n_ff_exp = data_torch.shape[1] // 2
            return [
