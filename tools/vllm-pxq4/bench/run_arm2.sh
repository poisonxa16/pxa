...
if [ "$ARM" = awq ]; then
  ...
  EXTRA_ENV="-e VLLM_SM70_QUANT_BACKEND=marlin -e PYTHONPATH=$MODELS_DIR/pxa-int-v5/site"
  QARG="--quantization pxq4"
elif [ "$ARM" = pxq4v6 ]; then
  MODEL=$MODELS_DIR/pxa-models/Qwen3.8-27B-PXQ4-vllm-p2a-nf
  EXTRA_ENV="-e VLLM_SM70_QUANT_BACKEND=marlin -e PYTHONPATH=$MODELS_DIR/pxa-int-v6/site"
  QARG="--quantization pxq4"
else
...
docker run -d --name vllm-bench \
  ...
  $EXTRA_ENV \
  --entrypoint python ${VLLM_IMAGE:-vllm/vllm-openai:latest} \
...
for i in $(seq 1 132); do
  ...
  if ! docker ps --format '{{.Names}}' | grep -q '^vllm-bench$'; then
    echo "[$(date -u +%FT%TZ)] CONTAINER DIED"; docker logs vllm-bench 2>&1 | tail -60
    docker rm -f vllm-bench >/dev/null 2>&1; touch $D/results/$NAME.FAIL; exit 1
  fi
  if docker exec vllm-bench python -c "import urllib.request,sys; urllib.request.urlopen('http://127.0.0.1:$PORT/health', timeout=3); print('ok')" 2>/dev/null | grep -q ok; then
    UP=1; break
  fi
done
if [ "$UP" != 1 ]; then
  echo "[$(date -u +%FT%TZ)] TIMEOUT waiting for health"; docker logs vllm-bench 2>&1 | tail -80
  docker rm -f vllm-bench; touch $D/results/$NAME.FAIL; exit 1
fi
if [ "${LOCKGC:-0}" = 1 ]; then
...
( while docker ps --format '{{.Names}}' | grep -q '^vllm-bench$'; do
