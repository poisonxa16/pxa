...
FORMAT, restated so this file is self-contained:
    ...
    dequant, PARITY-LOCKED (ggml/src/pxq-cpu.c:135-158, ggml/src/ggml-cuda/pxa/pxq6.cuh:326-331):
...
def check_tables(book: np.ndarray = BOOK, sub: np.ndarray = SUB) -> None:
    """The engine's own startup self-check (ggml/src/ggml-cuda/pxa/pxq6.cuh:98-108), replicated so a
...
def canon_nfix(kslabs: int, cmax: int = CANON_CMAX) -> int:
    """pxq6_canon_nfix, ggml/src/ggml-cuda/pxa/pxq6.cuh:826-833. Shape-only by construction — that
