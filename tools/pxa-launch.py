...
DESIGN RULE - NEVER MAGIC
  ...
  debugging session about the launcher.
    ...
    --sm                 auto (default) | layer | tensor | graph | attn. 'auto' picks
                         the PXA tensor split on two identical cards with an admitted
                         file - faster decode, slower prefill - and layer otherwise.
...
# ---------------------------------------------------------------------------
# GEMMA 4 - one table, because these are facts, not logic
# ---------------------------------------------------------------------------
# Everything this launcher knows about Gemma 4 lives here. The rows underneath are
# generated from it, so correcting a number is an edit to one line of data rather
# than a hunt through the decision code.
#
# KV BYTES PER TOKEN, read off the engine's own "KV self size" boot line - the
# allocator's number, not arithmetic over the header:
#   sparse 26B-A4B  220 KiB/token   5,280.00 MiB at -c 24,576
#                                   (30 layers: 25 sliding at 8 kv heads x 256,
#                                    5 full at 2 x 512; f16 K and V)
#   dense 12B       336 KiB/token   5,376.00 MiB at -c 16,384
#                                   (48 layers: 40 sliding, 8 full)
#
# Both are with the per-layer sliding-window KV cache OFF, which is how this
# release ships. On the sparse file the sliding cache is a measured decode loss
# and buys nothing anyone needs: the full cache is 3.4 GiB at 16k on a 32 GiB
# pair. It stays a lever (PXA_GEMMA4_ISWA), and the launcher does not set it.
#
# BOTH FILES DECLARE A 262,144-TOKEN TRAINED WINDOW. That is 55 GiB of KV on the
# sparse file and 84 GiB on the dense one, so it is not a context anything on a
# 16 GiB card is going to hold. Every -c below is a number that was booted, or
# arithmetic that says so and is labelled.
GEMMA4_KV_BYTES_PER_TOKEN = {"moe": 220 * 1024, "dense": 336 * 1024}
GEMMA4_TRAINED_CTX = 262144

# THE SPARSE 26B-A4B COMES IN THREE FILES and they do not behave alike, so the
# rows below are keyed on the PXQ tier as well as the cards:
#   Google's QAT q4_0   14.4 GB   the reference file. Two cards, or one at 4k.
#   PXQ3 (ours)         11.0 GB   THE ONE-CARD FILE. 16k of context on a single
#                                 16 GiB card, and at a 2.4k prompt one card beats
#                                 two: the layer-split hop between the cards costs
#                                 more than the second card gives back.
#   PXQ4 (ours)         13.9 GB   same size class as the QAT file. See the TODO
#                                 under GEMMA4_PXQ4_GELU_TODO before quoting it.
#
# TODO(GEMMA4_PXQ4_GELU_TODO): the PXQ4 file decodes at 61 t/s as it stands and at
# 105 t/s with PXA_PXQ_MOE_GELU=1, which is a fused activation that is NOT on by
# default yet. This launcher does not set it, because a launcher that turns on a
# lever the engine has not adopted is a launcher that prints a command nobody else
# can reproduce. When the engine makes it the default, delete this note and add the
# PXQ4 rows; until then the PXQ4 file gets no row of its own and falls through to
# the tier-agnostic q4_0 rows, whose flags are right and whose numbers are not
# quoted for it.
#
# The 11.0 GB file is named PXQ3 and is BUILT as a mixed map - most tensors PXQ3,
# a few kept wider - so tier_from_tensors() reads it as PXQ_UNIVERSAL, which is the
# honest answer about what is inside it. The rows below have to be keyed on what the
# file reports or they never match the file they were measured on. The numbers in
# them came from gemma-4-26B-A4B-it-PXQ3.gguf and no other universal map; the
# launcher's own PXQ_UNIVERSAL acknowledgement still applies and is not weakened
# here.
GEMMA4_PXQ3_TIERS = ("PXQ3", "PXQ_UNIVERSAL")

# ncards, card class, model class, PXQ tier ((): any), -c, status, numbers, source, notes
GEMMA4_CELLS = [
    # -- the sparse 26B-A4B, our PXQ3 file: one card is the headline ----------
    (1, 70, "moe", GEMMA4_PXQ3_TIERS, 16384, "MEASURED",
     "decode 105 / 95 / 84 t/s at 14 / 2,370 / 6,363 prompt tokens",
     "measured 2026-09-20 on ONE V100, our 11.0 GB PXQ3 file, -ngl 99 -fa -b 2048 "
     "-ub 512, np1",
     ["This is the file to reach for when you have one 16 GiB card: 11.0 GiB of "
      "weights leaves room for 16,384 tokens of context, which the 14.4 GB QAT "
      "file does not.",
      "-c 20,480 does NOT fit on one card - it is the compute buffer that runs "
      "out, not the KV cache, so the error names a buffer and not a context. "
      "16,384 is the ceiling that was booted."]),
    (2, 70, "moe", GEMMA4_PXQ3_TIERS, 32768, "INFERRED",
     "no number: the pair was not booted on the PXQ3 file",
     "arithmetic from the one-card row: 11.0 GiB of weights over two cards plus "
     "7.0 GiB of KV at -c 32,768 across 32 GiB",
     ["ON THIS FILE, TWO CARDS ARE FOR CONTEXT, NOT FOR SPEED. At a 2,370-token "
      "prompt one V100 measured 95 t/s on this file; splitting it over two cards "
      "adds a hop between them for every layer, and on a model this sparse that "
      "hop costs more than the second card gives back. Use the pair when you want "
      "more than 16k of context, and one card when you want the tokens per second."]),
    (1, 60, "moe", GEMMA4_PXQ3_TIERS, 16384, "INFERRED",
     "no number: one P100 was not booted on the PXQ3 file",
     "the same arithmetic as the one-V100 row, which was measured: 11.0 GiB of "
     "weights and 3.5 GiB of KV at -c 16,384 on a 16 GiB card",
     ["A P100 is slower than a V100 on this file but it is the same 16 GiB, so "
      "what fits is the same. The 2x P100 numbers on the QAT file are the closest "
      "measured thing to expect from."]),
    (2, 60, "moe", GEMMA4_PXQ3_TIERS, 32768, "INFERRED",
     "no number: the P100 pair was not booted on the PXQ3 file",
     "11.0 GiB of weights over two cards plus 7.0 GiB of KV at -c 32,768",
     []),
    # -- the sparse 26B-A4B, any other file (Google's QAT q4_0 is the reference)
    (2, 70, "moe", (), 32768, "MEASURED",
     "decode 111.5 / 107.6 / 103.0 / 98.4 t/s at 14 / 2,370 / 6,363 / 12,700 "
     "prompt tokens  |  prefill about 2,500 t/s  |  needle PASS at 3k, 8k and 20k, "
     "raw and through the template",
     "measured 2026-09-20 on 2x V100, the Google QAT q4_0 file, -ngl 99 -sm layer "
     "-fa -b 2048 -ub 512, np1, 3 reps, medians. Both -c 16,384 and -c 32,768 were "
     "booted and the decode numbers are the same; the head-512 attention kernel "
     "this release turns on by default for V100-class cards is in these numbers",
     []),
    (2, 60, "moe", (), 16384, "MEASURED",
     "decode 58.3 / 51.1 / 43.3 t/s at 14 / 2,370 / 6,363 prompt tokens  |  prefill "
     "about 590 t/s  |  needle PASS at 8k",
     "measured 2026-09-20 on 2x P100, the Google QAT q4_0 file, -ngl 99 -sm layer -fa "
     "-b 2048 -ub 512, np1, 3 reps, medians. -c 16,384 was not pushed higher",
     []),
    (4, 60, "moe", (), 32768, "INFERRED",
     "no number: the quad has booted this file but nothing was timed on it",
     "arithmetic from the pair row: 13.4 GiB of weights spread over 4 cards plus "
     "7.0 GiB of KV at -c 32,768 is comfortable on 64 GiB. The flags are the pair's",
     ["Four cards buy context and headroom here, not speed: the pair already holds "
      "the weights, so the extra hop between four cards is paid for nothing unless "
      "you need the context."]),
    (1, 70, "moe", (), 4096, "MEASURED",
     "decode 114 / 102 t/s at 14 / 2,370 prompt tokens",
     "measured 2026-09-20 on ONE V100, the Google QAT q4_0 file, -ngl 99 -fa "
     "-b 2048 -ub 512, np1. -c 4,096 is the only context that booted",
     ["4,096 tokens and no more: 13.4 GiB of weights on a 16 GiB card leaves about "
      "2.6 GiB, and -c 4,096 asks 0.86 GiB of it. This is the tightest cell in the "
      "table.",
      "For more context on ONE card, use our PXQ3 file (11.0 GB) instead - it "
      "holds 16,384 tokens on the same card. Two cards is the other way out."]),
    (1, 60, "moe", (), 4096, "INFERRED",
     "no number: one P100 was not booted on this file",
     "the same arithmetic as the one-V100 cell, which was measured: 13.4 GiB of "
     "weights, 0.86 GiB of KV, about 16 GiB of card",
     ["For more context on one card, our PXQ3 file (11.0 GB) holds 16,384 tokens "
      "where this one holds 4,096."]),
    # -- the dense 12B --------------------------------------------------------
    (1, 60, "dense", (), 16384, "INFERRED",
     "no number taken on this launcher's cells",
     "6.5 GiB of weights plus 5.3 GiB of KV at -c 16,384 on a 16 GiB card. The "
     "published ceiling for the 12B on one 16 GiB card is around 24k tokens "
     "[docs/KNOWN-ISSUES.md], so this leaves room rather than sitting on it",
     []),
    (1, 70, "dense", (), 16384, "INFERRED",
     "no number taken on this launcher's cells",
     "same arithmetic as the 1x P100 dense cell",
     []),
    (2, 60, "dense", (), 32768, "INFERRED",
     "no number taken on this launcher's cells",
     "6.5 GiB of weights plus 10.5 GiB of KV at -c 32,768 across 32 GiB",
     []),
    (2, 70, "dense", (), 32768, "INFERRED",
     "no number taken on this launcher's cells",
     "same arithmetic as the 2x P100 dense cell",
     []),
]

# The flags are the same in every cell on purpose. -b 2048 -ub 512 is what both
# measured pairs ran, -fa is required by the sliding-window attention this
# architecture uses on most of its layers, and -sm layer is the only split mode
# the engine will accept here (see GEMMA4_SPLIT_REFUSED).
GEMMA4_B, GEMMA4_UB = 2048, 512

# -sm tensor and -sm graph are both refused on this architecture by the engine
# itself. Named here so the launcher refuses first, with the reason, instead of
# printing a command that dies a minute later.
GEMMA4_ARCHES = {"gemma4", "gemma4_mtp"}


# ===========================================================================================
# '--sm auto' - THE DEFAULT SPLIT, PICKED BY THE LAUNCHER (2026-09-21)
#
# The engine's own default is still '-sm layer' and no binary changed for this. What changed is
# who chooses: on a pair of identical cards, with a file the engine's tensor-split capability
# check admits, the launcher now asks for the PXA tensor split, because that is where the
# measured decode speed is.
#
# THE EVIDENCE, all of it taken on the packaged v2026.09.20 binary, a V100 pair (cards 2,4),
# Qwen3.8-27B PXQ4, and none of it extrapolated:
#   decode, plain, tensor+fused vs layer, by prompt length
#       11 tok    48.2 vs 38.8 t/s        6,360 tok  47.2 vs 37.7 t/s
#    2,370 tok    45.3 vs 37.1 t/s       12,710 tok  44.4 vs 35.7 t/s
#   15,600 tok    43.7 vs 35.2 t/s
#   gate on the tensor path, -c 32768: PASS=13 FAIL=0 SKIP=0, two-slot concurrent needle 0/8 bad
#   prefill at 12.7k: 752 t/s split, 773 with PXA_TSPLIT_REDUCE_PREFILL=1, 849 with that plus
#       -b 2048 -ub 2048, against 1,135 on OUR layer split - SLOWER, and that is the trade. It is
#       still faster than stock llama.cpp's own tensor split in the same window (817/422/102 t/s
#       against 590/408/36 on long / repetition / prose).
#   host sensitivity: both cards launch every step together, so a loaded host costs decode -
#       an identical cell read 42.9 t/s quiet and 26.4 t/s while other GPU jobs ran.
#
# WHICH FILES. Only an arch the engine's own table carries EVIDENCE for, which today is exactly
# one: qwen35. The table is src/llama.cpp pxa_tsplit_arch_lookup() - every other row there has
# `evidence == nullptr` and is refused at load unless PXA_TSPLIT_UNPROVEN_ARCH=1, and Gemma 4 is
# not in the table at all (it needs PXA_TSPLIT_GEMMA4=1, and it is measured faster on a P100 pair
# and SLOWER on a V100 pair - so it is not something to pick for anyone). A default that emitted
# a mode the engine then refuses is a default that wastes a model load, so the launcher's admission
# list is deliberately the engine's evidence list and nothing more.
TSPLIT_AUTO_ARCHES = {"qwen35"}

# The codec, for the same reason: PXQ4 is what the gate and every decode number above were taken
# on. A K-split of a panel-addressed tensor is only defined for PXQ4 with the ladder off
...
TSPLIT_TRADE = (
    "decode is faster at every context length measured (48.2/45.3/47.2/44.4 t/s against "
    "38.8/37.1/37.7/35.7 at 11 / 2,370 / 6,360 / 12,710 prompt tokens, and 43.7 against 35.2 at "
    "15.6k); PREFILL is slower than the layer split on narrow PCIe links like this box's x4 "
    "risers (849 t/s at 12.7k at best, against 1,135 on layer); and it is sensitive to other work "
...
def resolve_auto_split(engine, sel, prof, forced_ts, recipe, fa="on"):
    """'--sm auto' -> ('tensor'|'layer', why). PURE: no I/O, no globals but the tables above,
    so the selftest can drive it over a case table instead of a box.

    Every clause is a REASON, and the reason is what --explain prints. A default that cannot
    say why it chose is the magic this launcher exists not to do."""
    if engine != "llama":
        return "layer", (f"the engine that wins this seat is {engine} - the PXA tensor split is a "
                         f"llama.cpp-side mode, and vLLM has its own parallelism")
    n = len(sel or [])
    if n < 2:
    ...
    if len(models) > 1:
        return "layer", (f"the selected cards are not the same model ({', '.join(models)}). An "
                         ...
                         f"sets the pace of every single step; a layer split can at least be "
                         f"weighted by capacity")
    arch = prof.get("arch") or "unknown"
    if arch in GEMMA4_ARCHES:
        return "layer", (f"arch '{arch}': the engine refuses the tensor split on Gemma 4 unless "
                         f"PXA_TSPLIT_GEMMA4=1, and it is measured FASTER on a P100 pair and "
                         f"SLOWER on a V100 pair - a coin flip is not a default. Ask for it by "
                         f"hand with --sm tensor and that lever if you want it")
    if arch not in TSPLIT_AUTO_ARCHES:
        return "layer", (f"arch '{arch}' is not one the engine's tensor-split table carries "
                         f"EVIDENCE for (only {'/'.join(sorted(TSPLIT_AUTO_ARCHES))} is, today). "
    ...
    if nkv and int(nkv) < n:
        return "layer", (f"this file has {nkv} KV heads and {n} cards are selected: attention "
                         f"cannot divide more ways than it has KV heads, so {n - int(nkv)} card(s) "
                         f"would do no attention work (the engine refuses this at load)")
    if fa != "on":
        # --workload longdoc picks -fa off deliberately (the cold-prefill regime). The split
        # attention builder has no correct non-FA path, so the engine would turn FA back ON
        # for this context - quietly undoing the one setting the operator chose with their own
        # head. Declining here keeps the workload the workload.
        return "layer", (f"-fa {fa}: this workload runs flash attention OFF, and the split "
                         f"attention builder has no non-FA path - the engine would force FA "
                         f"back on and you would not be serving the regime you picked")
    if forced_ts:
        return "layer", (f"--ts {forced_ts} was forced. A forced split is a capacity split over "
                         f"LAYERS; the tensor split wants an even one and would throw yours away, "
                         f"so the layer split you asked for is what you get")
    if recipe is not None and recipe.ts and recipe.sm != "tensor":
        return "layer", (f"recipe row '{recipe.key}' was MEASURED at '-sm {recipe.sm}' with "
                         f"-ts {recipe.ts}, a LAYER split. A measured row beats a default: the "
                         f"row's own split is emitted, unchanged")
    return "tensor", (f"{n}x {models[0]}, arch '{arch}', tier {tier} - the engine's tensor-split "
                      f"capability check admits this file and the split is where the decode speed "
                      f"is on this pair")


def _gemma4_recipes():
    out = []
    for n, cap, cls, tiers, ctx, status, numbers, source, notes in GEMMA4_CELLS:
        fam = ("moe", "hybrid-moe") if cls == "moe" else ("dense", "hybrid")
        what = "sparse 26B-A4B" if cls == "moe" else "dense 12B"
        kvb = GEMMA4_KV_BYTES_PER_TOKEN[cls]
        tag = "-pxq3" if tiers else ""
        out.append(Recipe(
            f"{n}x{'v100' if cap == 70 else 'p100'}-gemma4-{cls}{tag}",
            f"{n}x {CARD_CLASS[cap]}, Gemma 4 {what}"
            + (", PXQ3 (a PXQ_UNIVERSAL map)" if tiers else ""), n, {cap}, fam, tiers,
            b=GEMMA4_B, ub=GEMMA4_UB, ctx=ctx, env={}, arch="gemma4",
            numbers=numbers, source=source, status=status,
            notes=list(notes) +
                  [f"-c {ctx} and not the {GEMMA4_TRAINED_CTX:,}-token window this file "
                   f"declares: Gemma 4 keeps {kvb // 1024} KiB of KV per token, so the "
                   f"trained window alone is {GEMMA4_TRAINED_CTX * kvb / BYTES_PER_GIB:.0f} "
                   f"GiB. Ask for more with -c N and the launcher will pass it through.",
                   "-fa is not optional on this architecture: most of its layers use "
                   "sliding-window attention and the non-flash path does not build them.",
                   "-sm layer only. The tensor-split and graph-split builders exist for "
                   "this architecture but have never been run on a real file: the engine "
                   "refuses -sm tensor outright and quietly demotes -sm graph and -sm attn "
                   "back to layer, so layer is what runs either way.",
                   "Two levers stay OFF here and this launcher does not set either: "
                   "PXA_GEMMA4_ISWA=1 (the per-layer sliding-window KV cache - it "
                   "measured slower and the full cache is small enough not to need it) "
                   "and the MTP assistant drafter, which wins on short prompts and "
                   "loses on everything longer.",
                   "This model answers directly. There is no thinking pass unless you "
                   "ask for one - --reasoning on at the server, or "
                   "chat_template_kwargs {\"enable_thinking\": true} on the request."]))
    return out


# ---- the rows, most specific first ----------------------------------------
RECIPES = _gemma4_recipes() + [
...
def model_profile(path, kind):
    ...
    p = {"arch": None, "n_expert": 0, "is_moe": False, "tier": None, "tier_kv": None,
         ...
         "tier_src": "not inspected", "ftype": None, "n_ctx_train": None, "n_head_kv": None,
    ...
    if kind in ("gguf", "gguf_broken"):
        ...
        # The tensor split's unit is one KV head, so this is the hard ceiling on how many
        # ways attention can divide (src/llama.cpp clause 6). Read here so '--sm auto' can
        # decline a 6-card split of a 4-KV-head file without paying for the model load.
        _nkv = kv.get(f"{arch}.attention.head_count_kv")
        p["n_head_kv"] = _nkv if isinstance(_nkv, int) else None
...
def kv_bytes_per_token(kv, arch):
    ...
    # Gemma 4 has a measured number, and a measured number beats arithmetic. The
    # formula below prices every layer at the widest of the TWO attention geometries
    # this architecture interleaves (a sliding layer has 8 kv heads at 256, a full
    # layer 2 at 512) and comes out about 10% high on the sparse file and 14% high
    # on the dense one - enough to call a context tight that is not.
    if arch in GEMMA4_ARCHES:
        cls = "moe" if (kv.get(f"{arch}.expert_count") or 0) else "dense"
        b = GEMMA4_KV_BYTES_PER_TOKEN[cls]
        return b, (f"MEASURED: {b // 1024} KiB/token, read off the engine's own 'KV self size' "
                   f"boot line for the Gemma 4 {'sparse 26B-A4B' if cls == 'moe' else 'dense 12B'} "
                   f"with f16 K and V and the sliding-window KV cache off (how this release "
                   f"ships). Turning that lever on with PXA_GEMMA4_ISWA=1 changes the shape, not "
                   f"just the number: the cost becomes a smaller per-token part plus a fixed "
                   f"ring, so this figure no longer applies.")
    try:
...
R = {
 ...
 "R-30": ("REFUSING: -sm {sm} on arch '{arch}'. The engine has a {sm}-split builder for this "
          "architecture and it has never been run on a real file, so the engine refuses it "
          "after the model load rather than produce output nobody has checked - it says "
          "\"'-sm tensor' refused: this arch has a tensor-split builder that has never been "
          "run on a real file here\". Refusing here saves you the load. Use -sm layer - it is "
          "this architecture's measured shape, and the recipe rows in this table were all "
          "taken on it."),
...
class Plan(object):
    def __init__(self):
        ...
        self.sm_auto = False    # True when '-sm ...' was CHOSEN here, not typed by the operator
...
def build_llama_cmd(plan, a, sel, prof, ctx, ub_expect, mmproj, explain=False):
    ...
    R = plan.recipe
    if a.sm == "auto":
        # Belt and braces. 'auto' is resolved in the decision above and cannot reach here
        # through any path that exists today; if a future one is added, it must not put the
        # word 'auto' on a llama-server command line, which llama.cpp would reject.
        a.sm = "layer"
    ...
    elif a.sm == "tensor" and sel and len(sel) > 1:
        # THE TENSOR SPLIT TAKES AN EVEN -ts, NOT A CAPACITY ONE. '-sm layer' partitions the
        # model by BYTES, so a card with more free VRAM should hold more of it; '-sm tensor'
        # cuts every weight and every attention head the same way on every device and the two
        # halves run in lockstep, so an uneven ratio does not give one card less work - it
        # gives one card work the other waits for, every step.
        _ts = ",".join(["1"] * len(sel))
        cmd += ["-ts", _ts]
        print(f"  -ts {_ts}: an EVEN split, because -sm tensor cuts the WEIGHTS and the "
              f"attention heads rather than handing out whole layers. The capacity-weighted "
              f"split the layer mode uses is deliberately NOT reused here: both devices run "
              f"the same step at the same time, so an uneven ratio only adds wait.")
    ...
    if R is not None and R.env:
        ...
        for k, v in R.env.items():
            print(f"      {k}={v}")
    # ---- the tensor split's own environment -------------------------------
    # Emitted whenever '-sm tensor' is what goes on the command line, however it got there,
    # so that the line printed here, the line in --serve-name's script and the line in the
    # tutorials are the same line and mean the same thing. setdefault, not assignment: a
    # recipe row that names one of these was measured with its own value and keeps it.
    if a.sm == "tensor":
        ...
        if plan.sm_auto:
            ...
            # for the split by name wants to know it did not happen. A split this launcher
            # picked is a different promise - it must never be the reason a seat fails to
            # start - so the auto path, and only the auto path, hands the engine permission
            # to demote itself back to '-sm layer'.
            for k, v in TSPLIT_AUTO_FALLBACK_ENV.items():
                env.setdefault(k, v)
            print(f"  PXA_TSPLIT_FALLBACK=1: the split was CHOSEN here, not asked for, so if "
...
#
# A name in this list that the engine has no built-in row for is listed in
# _NO_BUILTIN_ROW below, so the launcher can recognise the family WITHOUT implying
# that --chat-template will take the name.
_TEMPLATE_FINGERPRINTS = [
    # Gemma 4 is FIRST and deliberately not spelt like Gemma 3: its turns are
    # <|turn> / <turn|> and its channels <|channel> / <channel|>, so the "gemma" row
    # below - which keys on <start_of_turn> - never matched it and the launcher used
    # to report "an unrecognised family" on a file it ships a recipe for. Do not be
    # tempted to answer that with --chat-template gemma: the built-in Gemma row would
    # format every turn wrongly.
    ("gemma4",   lambda t: "<|turn>" in t or "<turn|>" in t),
...
# Families the engine recognises but has NO built-in template row for. The file's
# own template is the only correct way to render them, which is why the engine turns
# the jinja path on by itself when it loads one and says so on the boot line.
_NO_BUILTIN_ROW = {"gemma4"}
...
def chat_defaults(prof, a, engine_dir=None):
    ...
    if a.chat_template_file:
        ...
        c.add("chat template", f"embedded in the GGUF ({len(tmpl)} chars"
              ...
              + (f" There is NO built-in '{fam}' row to substitute - the engine renders this "
                 f"file's own template and turns --jinja on by itself. Do not pass a "
                 f"--chat-template here; every built-in row would format the turns wrongly."
                 if fam in _NO_BUILTIN_ROW else ""))
    else:
        ...
        if (prof.get("arch") or "") in GEMMA4_ARCHES:
            c.warnings.append(
                "AND ON THIS ARCHITECTURE IT SHOWS IMMEDIATELY. Gemma 4 writes its turns as "
                "<|turn> / <turn|> and its channels as <|channel> / <channel|>; no built-in "
                "template produces those, so the model answers into a shape the server cannot "
                "parse and the markup arrives in the content field - a first reply that reads "
                "'<|channel>\\nthought\\n<channel|>The three primary colours are...'. It is not "
                "a broken model and it is not the cards. Serve a Gemma 4 file that still "
                "carries its template, or pass --chat-template-file with the template from the "
                "original GGUF or the model's own repository. There is no --chat-template name "
...
def selftest(gpus):
    ...
    for r in RECIPES:
        ...
        # A row that accepts ANY tier is probed with a tier no tier-keyed row claims,
        # so that "is this row reachable" is a question about the row and not about
        # which tier the probe happened to pick. 'none' - a file with no PXQ tensors
        # at all, which is what Google's q4_0 files are - is that tier: the only row
        # that names it is the last one in the table.
        tier = sorted(r.tiers)[0] if r.tiers else "none"
    ...
    # A12 (2026-09-21): '--sm auto'. The default split mode is now a DECISION, so every
    #     clause of it is exercised here rather than on a box - the box only has the two
    #     pairs, and the rule has to be right on the pairs it will meet at a user's desk.
    #     The table reads as the rule reads: one row per reason to decline, one to accept.
    def _c(cc, name, n):
        return [(i, name, cc, 16384, 0, f"U{i}") for i in range(n)]
    V100, P100 = "Tesla V100-PCIE-16GB", "Tesla P100-PCIE-16GB"
    qwen = {"arch": "qwen35", "tier": "PXQ4", "n_head_kv": 4}
    a12cases = [
        ("V100 pair, qwen35 PXQ4",        "llama", _c(70, V100, 2), qwen, "", None, "tensor"),
        ("P100 pair, qwen35 PXQ4",        "llama", _c(60, P100, 2), qwen, "", None, "tensor"),
        ("one card",                      "llama", _c(70, V100, 1), qwen, "", None, "layer"),
        ("mixed P100+V100",               "llama",
         _c(60, P100, 1) + [(1, V100, 70, 16384, 0, "U1")], qwen, "", None, "layer"),
        ("gemma4 26B",                    "llama", _c(70, V100, 2),
         {"arch": "gemma4", "tier": "PXQ4", "n_head_kv": 8}, "", None, "layer"),
        ("qwen35moe (no evidence row)",   "llama", _c(70, V100, 2),
    ...
    # the -fa clause, driven by the regime map so it cannot drift away from --workload
    a12fa = [("workload longdoc (-fa off)", "llama", _c(70, V100, 2), qwen, "", None,
              FA_BY_WORKLOAD["longdoc"], "layer"),
             ("workload chat (-fa on)", "llama", _c(70, V100, 2), qwen, "", None,
              FA_BY_WORKLOAD["chat"], "tensor")]
    # ...and the recipe clause, driven by a REAL row so it cannot rot: the first table row
    # that carries a layer -ts must keep its own split rather than be overridden by auto.
    _lrow = next((r for r in RECIPES if r.ts and r.sm != "tensor"), None)
    if _lrow is not None:
        a12cases.append((f"recipe row '{_lrow.key}' measured at -sm {_lrow.sm} with -ts",
                         "llama", _c(70, V100, 2), qwen, "", _lrow, "layer"))
    a12bad = []
    a12all = [(l, e, s, p, f, r, "on", w) for l, e, s, p, f, r, w in a12cases] + a12fa
    for label, eng, sel_, prof_, fts, rec, fa_, want in a12all:
        got, why = resolve_auto_split(eng, sel_, prof_, fts, rec, fa_)
        # 'auto' must never survive the resolver: llama-server has no such value.
        if got != want or got == "auto" or not why:
            a12bad.append(f"{label}: got {got} want {want}")
    a12 = not a12bad
    ok_all &= a12
...
def plan_and_build(a, gpus):
    ...
    # ---- '--sm auto' resolves HERE, and only here -------------------------
    # Above the Gemma 4 block and above the vLLM R-11 check on purpose: everything below
    ...
    if plan.sm_auto:
        ...
        if a.sm == "tensor":
            ...
            if plan.recipe is not None and plan.recipe.sm != "tensor":
                row = (f" Say so out loud: recipe row '{plan.recipe.key}' was measured at "
                       f"'-sm {plan.recipe.sm}', so the -b/-ub/-c cells below come from a "
                       f"LAYER-split measurement and are carried over, not re-measured under "
                       f"the tensor split. The row named no -ts, so nothing of it is thrown away.")
            plan.notes.append(
                f"--sm auto -> -sm tensor (the PXA panel-aware tensor split). Why: {why}. "
                f"The trade, measured on a V100 pair on the packaged build: {TSPLIT_TRADE}{row}")
        else:
            plan.notes.append(
    ...
    # load. '-sm graph' and '-sm attn' it DEMOTES - it prints "Split mode 'graph' is not
    # supported for Gemma 4 (unmeasured on this arch) => changing split mode to 'layer'"
    # and serves. Refusing those would be this launcher stopping a command the engine
    # handles perfectly well, so it says what will happen and emits the split mode that
    # is going to be used either way. Decided here, above the notes, so the note prints.
    if plan.engine == "llama" and prof.get("arch") in GEMMA4_ARCHES:
        if a.sm == "tensor":
            plan.refuse("R-30", code=3, sm=a.sm, arch=prof["arch"])
        elif a.sm in ("graph", "attn"):
            plan.notes.append(
                f"-sm {a.sm} was asked for and -sm layer is what will run. The engine does "
                f"not support the {a.sm} split on this architecture - it is unmeasured here, "
                f"not broken - and demotes it to layer at startup with a message saying so. "
                f"The command below says 'layer' so that what you read is what runs; every "
                f"Gemma 4 row in this table was measured there.")
            a.sm = "layer"
...
def main():
    ...
    ap.add_argument("--sm", default="auto",
                    help="split mode: auto (default) | layer | tensor | graph | attn. 'auto' "
                         "picks the PXA tensor split on two identical cards holding a file the "
                         "engine's capability check admits, and 'layer' everywhere else; "
