...
#: Every slab stride any PXQ tier can legitimately allocate (576 pxq2 / 832 pxq3 / 1088
#: pxq4 / 1152 pxq4hq -- derived from the tier table, so a new tier needs no edit here).
#: PXQ4SlabParameter used to hard-assert exactly 1088 -- correct back when this class
#: only ever served PXQ4 LINEAR modules, wrong now that create_weights allocates at
#: ``tier.slab_bytes`` for a tiered module. The per-module tier (and therefore the ONE correct
#: stride for that instance) was already asserted against the checkpoint's own tensor shape by
#: the caller and by the C++ op; this is only the class invariant "is this a stride some known
#: PXQ tier actually uses", not "is this the PXQ4 stride".
_VALID_SLAB_BYTES = frozenset(t.slab_bytes for t in _TIERS.values())
...
class PXQ4SlabParameter(PackedvLLMParameter):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        assert self.data.dim() == 3, "pxq4_slabs must be [panels, kslabs, slab_bytes]"
        assert self.data.shape[2] in _VALID_SLAB_BYTES, (
            f"pxq4_slabs last dim must be a known PXQ tier stride "
            f"{sorted(_VALID_SLAB_BYTES)}, got {self.data.shape[2]}"
        )
