...
def _register_fakes() -> None:
    ...
    # ---- fused MoE decode block (pxq_moe_fused.cuh) ------------------------
    # Same rule as above: these mutate argument 0 and return nothing, so the fakes are pure
    # shape checks. They are registered per NAME, and every registration is guarded on the op
    # actually being present, because a v13 library built before the fused kernels landed is
    # a legitimate library that this sidecar must still load.
    def _fake_gateup(name):
        @torch.library.register_fake("pxq4::" + name)
        def _meta(act, x, ids, slabs, anchor, top_k):  # noqa: ANN001, ANN202
            torch._check(slabs.dim() == 4 and anchor.dim() == 3)
            torch._check(x.dim() == 2 and act.dim() == 2 and ids.dim() == 1)
            torch._check(x.shape[1] == slabs.shape[2] * 32)
            torch._check(act.shape[0] == ids.shape[0])
            # w13 holds gate then up on the panel axis, so act is HALF the weight's N.
            torch._check(act.shape[1] * 2 == slabs.shape[1] * 64)
            return None

    def _fake_down_fold(name):
        @torch.library.register_fake("pxq4::" + name)
        def _meta(out, act, ids, wts, slabs, anchor, top_k):  # noqa: ANN001, ANN202
            torch._check(slabs.dim() == 4 and anchor.dim() == 3)
            torch._check(act.dim() == 2 and out.dim() == 2 and ids.dim() == 1)
            torch._check(act.shape[0] == ids.shape[0])
            torch._check(act.shape[1] == slabs.shape[2] * 32)
            torch._check(out.shape[1] == slabs.shape[1] * 64)
            return None

    def _fake_down_part(name):
        @torch.library.register_fake("pxq4::" + name)
        def _meta(dn, act, ids, slabs, anchor):  # noqa: ANN001, ANN202
            torch._check(slabs.dim() == 4 and anchor.dim() == 3)
            torch._check(act.dim() == 2 and dn.dim() == 2 and ids.dim() == 1)
            torch._check(dn.shape[0] == ids.shape[0])
            torch._check(act.shape[1] == slabs.shape[2] * 32)
            torch._check(dn.shape[1] == slabs.shape[1] * 64)
            return None

    for _n in ("moe_gateup_glu_out", "pxq2_moe_gateup_glu_out", "pxq3_moe_gateup_glu_out"):
        if hasattr(torch.ops.pxq4, _n):
            _fake_gateup(_n)
    for _n in ("moe_down_fold_out", "pxq2_moe_down_fold_out", "pxq3_moe_down_fold_out"):
        if hasattr(torch.ops.pxq4, _n):
            _fake_down_fold(_n)
    for _n in ("moe_down_part_out", "pxq2_moe_down_part_out", "pxq3_moe_down_part_out"):
        if hasattr(torch.ops.pxq4, _n):
            _fake_down_part(_n)

    if hasattr(torch.ops.pxq4, "moe_slot_fold_out"):
        @torch.library.register_fake("pxq4::moe_slot_fold_out")
        def _slot_fold_meta(out, dn, wts, top_k):  # noqa: ANN001, ANN202
            torch._check(dn.dim() == 2 and out.dim() == 2)
            torch._check(out.shape[1] == dn.shape[1])
            return None
...
def upload_tables(book, sub) -> None:
    ...
    # NO-OP WHEN THERE IS NOTHING TO CHANGE, and that is not just an optimisation.
    #
    # set_tables fans out to pxq4_mma_upload_tables (pxq4_kernel_torch.cpp), which
    # cudaMemcpyToSymbol's into the TENSOR-CORE translation unit. That TU is compiled sm_70
    # only -- it uses wmma m16n16k16, which does not exist on sm_60 and will not compile for
    # it -- so on a Pascal card the symbol has no device image and the copy fails with
    # "no kernel image is available for execution on the device", killing model load AFTER
    # the weights are in. (The pre-v13 sm_60 libraries never hit this because they predate
    # the mma TU entirely.)
    #
    # Reading the tables back first costs one 128-byte D2H and touches only pxq4_kernel.cu's
    # symbols, which do have an sm_60 image. If the checkpoint's tables are the ones already
    # on the device -- the overwhelmingly common case, since the converter asserts a PXQ4
    # checkpoint's tables equal the frozen ones -- there is nothing to upload and the hazard
    # never arises. If they genuinely differ on a device that cannot take them, say so
    # clearly instead of dying inside a memcpy.
    if hasattr(torch.ops.pxq4, "get_tables"):
        try:
            cur = torch.ops.pxq4.get_tables()
            if torch.equal(cur[0], b) and torch.equal(cur[1], s):
                logger.info("pxq4: device tables already match the checkpoint's; no upload")
                return
        except Exception as exc:  # noqa: BLE001 - a readback failure must not mask the upload
            logger.warning("pxq4: could not read back the device tables (%s); uploading", exc)
    try:
        torch.ops.pxq4.set_tables(b, s)
    except RuntimeError as exc:
        raise RuntimeError(
            "pxq4: this checkpoint records book/sub tables that differ from the compiled-in "
            "ones, and uploading them failed on this device. On sm_60 that is expected with a "
            "v13 library: the tensor-core translation unit it carries is sm_70-only, so its "
            "table symbols have no Pascal device image. Serve this checkpoint on sm_70, or "
            "rebuild the library with pxq4_mma_upload_tables() guarded by pxq4_mma_arch_ok(). "
...
def _stream_is_capturing() -> bool:
    """True iff the current CUDA stream is capturing a graph.

    Wrapped because the predicate is not present on every torch build and
    because it raises when CUDA is not initialised; either way the safe answer
    for the callers below is "no capture in progress, act eagerly".
    """
    try:
...
class PXQ4Workspace:
    ...
    that baked in the old pointer, so ``reserve`` raises once frozen AND a view
    has already been handed out (see ``_viewed`` below).
    """

    _reserved_dequant: int = 0
    _reserved_act: int = 0
    _frozen: bool = False
    # 2026-09-07 (ported 2026-09-08); the first boot of this build hit the exact
    # bug it fixes: --spec-model loads a SECOND
    # model -- the DFlash drafter -- after the target's process_weights_after_
    # loading has already frozen the arena. True once dequant_view() has handed
    # a pointer into the arena to a caller; until then the arena can still be
    # grown safely because nothing has baked its address into a graph or a
    # compiled kernel. This is what lets the drafter's wider `fc` layer
    # (25600x5120 = 131,072,000 fp16 elems, against a target whose widest
    # layer was 17408x5120 = 89,128,960) contribute after the target froze.
    _viewed: bool = False
    ...
    def reserve(cls, *, dequant_elems: int, act_elems: int = 0) -> None:
        ...
        if cls._frozen and (
            ...
            # The original contract assumed ONE model: every create_weights runs
            # before the first process_weights_after_loading. With --spec-model
            # that is false -- the drafter is built and loaded after the target
            # has finished. Growing is unsafe only once a pointer INTO the arena
            # has escaped (dequant_view was called) or a graph capture is
            # running; until then the buffer can simply be reallocated larger.
            if cls._viewed or _stream_is_capturing():
                raise RuntimeError(
                    "pxq4: workspace reservation grew after the arena was in "
                    f"use (want dequant={dequant_elems}, have "
                    f"{cls._reserved_dequant}). A layer cannot enlarge the "
                    "dequant arena once a view of it has been handed out or a "
                    "graph capture is running; see ops.py PXQ4Workspace."
                )
            logger.info(
                "pxq4: dequant workspace grows %d -> %d fp16 elems before first "
                "use (second model loaded after the first froze the arena)",
                cls._reserved_dequant,
                max(cls._reserved_dequant, int(dequant_elems)),
            )
            cls._frozen = False
            for _arena in cls._arenas.values():
                _arena.dequant = None
    ...
    def materialize(cls, device: torch.device) -> None:
        ...
        if (
            arena.dequant is not None
            and arena.dequant.numel() < cls._reserved_dequant
            and not cls._viewed
        ):
            # reserve() grew the requirement for a second model; drop the
            # too-small buffer from the first model's materialize() so the
            # block below allocates the new, larger size.
            arena.dequant = None
        ...
        if arena.dequant is None or arena.dequant.numel() < need:
            if _stream_is_capturing():
                raise RuntimeError(
                    ...
                    "and a CUDA graph capture is in progress, so it cannot be "
                    "allocated now. PXQ4LinearMethod.process_weights_after_"
                    "loading must run before the first forward."
                )
            # Outside capture this is recoverable. It is reached when a layer
            # skipped the reservation because it expected to take the
            # torch.ops.pxq4.linear_out path (which owns its own arena in C++)
            # and something routed it here instead. Say so loudly -- an
            # unreserved arena is a policy bug, not a normal path -- and
            # allocate rather than kill the boot.
            logger.warning(
                "pxq4: dequant workspace short (need %d fp16 elems, have %d); "
                "allocating eagerly outside capture. A PXQ4 layer reached the "
                "Python dequant+GEMM path without reserving for it.",
                need,
                0 if arena.dequant is None else arena.dequant.numel(),
            )
            arena.dequant = torch.empty(need, dtype=torch.float16, device=device)
            cls._reserved_dequant = max(cls._reserved_dequant, need)
        cls._viewed = True
