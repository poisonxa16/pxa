...
class PXQ4Workspace:
    ...
    _reserved_act: int = 0
    _frozen: bool = False
    # 2026-09-07: True once dequant_view() has handed a
    # pointer into the arena to a caller.  Until that happens the arena can
    # still be grown safely -- nothing has baked its address into a graph or a
    # compiled kernel.  This is what lets a SECOND model (the speculative
    # drafter, loaded after the target's process_weights_after_loading has
    # already frozen the class) contribute a wider layer.
    _viewed: bool = False
    ...
    def reserve(cls, *, dequant_elems: int, act_elems: int = 0) -> None:
        ...
        if cls._frozen and (
            ...
            # 2026-09-07.  The original contract assumed ONE
            # model: every create_weights runs before the first
            # process_weights_after_loading.  With --spec-model that is false --
            # the drafter is built and loaded after the target has finished, so
            # a drafter layer wider than anything in the target (the DFlash2
            # `fc`, 25600x5120 = 131,072,000 fp16 elems, against the target's
            # widest 17408x5120 = 89,128,960) reaches this branch legitimately.
            # Growing is unsafe only once a pointer INTO the arena has escaped;
            # until then the buffer can simply be reallocated larger.  Refuse
            # exactly in the case the original assert was defending: a view has
            # already been handed out (or we are inside a graph capture).
    ...
    def materialize(cls, device: torch.device) -> None:
        ...
        if (
            ...
            # Grew in reserve() above; drop the small buffer and take a new one.
            arena.dequant = None
    ...
    def dequant_view(cls, N: int, K: int, device: "torch.device | None" = None) -> torch.Tensor:
        ...
        if arena.dequant is None or arena.dequant.numel() < need:
            ...
            # 2026-09-07: outside capture this is recoverable.
            # It is reached when a layer skipped the reservation because it
            # expected to take the v7 torch.ops.pxq4.linear_out path (which owns
            # its own arena in C++) and something routed it here instead. Say so
            # loudly -- an unreserved arena is a policy bug, not a normal path --
            # and allocate rather than kill the boot.
            logger.warning(
