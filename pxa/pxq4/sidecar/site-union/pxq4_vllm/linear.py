...
_PXQ4_SMALLM_M = 16
_PXQ4_SMALLM = os.environ.get("PXA_PXQ4_SMALLM", "1") not in ("0", "")
...
# 2026-09-07: see the workspace note in create_weights.
_WS_RESERVE_ALWAYS = os.getenv("PXQ4_WS_RESERVE_ALWAYS", "0") == "1"
...
# ---------------------------------------------------------------------------
# Route trace (diagnostic, default OFF, PXQ4_TRACE_M=1)
# ---------------------------------------------------------------------------
# Two images running the same kernel library can still differ in SPEED if the
# runner hands this method different shapes -- a different prefill chunk width,
# an extra graph split, a compile range that lands on the other side of the
# mmv/dequant boundary. None of the engine-level arms can see that, because they
# all vary the engine and none of them record what the sidecar actually receives.
# This records exactly that: per (M, route) call counts, printed periodically.
# It allocates nothing on the device, takes no lock, and does no host sync.
_TRACE_M = os.getenv("PXQ4_TRACE_M", "0") == "1"
_TRACE_EVERY = int(os.getenv("PXQ4_TRACE_EVERY", "200"))
_trace_counts: dict = {}
_trace_calls = 0


def _is_compiling() -> bool:
    """True while Dynamo is tracing this function into a graph."""
    try:
        return bool(torch.compiler.is_compiling())
    except Exception:      # very old torch, or the predicate moved
        return False


def _trace_route(layer, M: int, N: int) -> None:
    global _trace_calls
    # PXA: NEVER touch M on the compiled path. M is x2.shape[0], which is a
    # SymInt while Dynamo traces, and the two lines below both force int(M):
    # `key = (M, route)` hashes it into a dict key, and the f-string formats
    # it. Either one SPECIALISES the dynamic batch dimension, and the engine
    # then dies at init with
    #   Constraints violated (L['inputs_embeds'].size()[0],
    #                         L['positions'].size()[1])
    # or, one pass earlier, with "Tried to erase Node size but it still had 2
    # users in the graph: {core_attn_out, setitem}" - that `setitem` being this
    # function's own dict write left behind in the graph.
    #
    # This cost two full compiled-boot failures of the sm70 v1.5.0 rebase on
    # 2026-09-05 and nearly cost the release: the harness set PXQ4_TRACE_M=1 on
    # a COMPILED arm, and the engine was blamed for a diagnostic's side effect.
    # An earlier version of this comment argued against guarding here, on the
    # grounds that the predicate is true at run time and the trace would then
    # report nothing under compile. That is the correct outcome: this is an
    # eager-arm diagnostic, and a diagnostic must never be able to stop the
    # server from booting. It reports nothing compiled, by design.
    if _is_compiling() or not isinstance(M, int):
        return
    ...
    _trace_calls += 1
    if _trace_calls <= 8 or _trace_calls % _TRACE_EVERY == 0:
        items = sorted(_trace_counts.items(), key=lambda kv: -kv[1])[:24]
        msg = "PXQ4_TRACE_M calls=%d routes=%s" % (
            _trace_calls,
            " ".join(f"M{m}:{r}={c}" for (m, r), c in items))
        logger.warning(msg)
...
class PXQ4LinearMethod(LinearMethodBase):
    ...
    def create_weights(
        ...
        **extra_weight_attrs: Any,
        ...
        #
        # 2026-09-07: reserve ONLY for layers that can
        # actually reach the Python dequant+GEMM branch of apply().  When the
        # loaded .so exposes ``linear_out`` (v7 dispatcher, op version >= 7),
        # apply() returns at the ``pxq4_linear_op`` branch above the elif/else
        # chain and NEVER calls PXQ4Workspace.dequant_view -- the mmv-vs-dequant
        # policy and its scratch both live in C++, sized by the warm call at the
        # end of process_weights_after_loading.  The Python arena was therefore
        # 178 MB/rank of dead weight on this build (target's widest layer,
        # 17408x5120 fp16), which on a 16 GB V100 is the difference between k=7
        # at MML 2048 and k=7 at a usable context.  PXQ4_WS_RESERVE_ALWAYS=1
        # restores the unconditional reservation.  If the policy is ever wrong,
        # dequant_view says so loudly and allocates outside capture rather than
        # failing the boot.
        if not layer.pxq4_linear_op or _WS_RESERVE_ALWAYS:
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        if layer.pxq4_use_mmv and layer.pxq4_mmv_max_m > 0:
            m = _PXQ4_SMALLM_M if _PXQ4_SMALLM else int(layer.pxq4_mmv_max_m)
        ...
        if getattr(layer, "pxq4_linear_op", False):
            m = (_PXQ4_SMALLM_M if _PXQ4_SMALLM else int(layer.pxq4_mmv_max_m or 8)) + 1
            dev = layer.pxq4_slabs.device
            x_warm = torch.zeros((m, layer.pxq4_K), dtype=torch.float16, device=dev)
            out_warm = torch.empty((m, layer.pxq4_N), dtype=torch.float16, device=dev)
    ...
    def apply(
        ...
        if _TRACE_M:
            # Deliberately NOT wrapped in try/except and NOT guarded on
            # is_compiling(): a swallowed exception here is how the first version
            # of this trace reported nothing for two windows while looking
            # healthy, and the compiling predicate is true at run time under the
            # compiled path. The traced arms run --enforce-eager.
            _trace_route(layer, M, N)
        if M == 0:
        ...
        elif layer.pxq4_use_mmv and M <= (
            _PXQ4_SMALLM_M if _PXQ4_SMALLM else layer.pxq4_mmv_max_m
        ):
