...
# v14 SMALL-M SWITCH (2026-09-07), the Python mirror of the C++
# PXA_PXQ4_SMALLM in pxq4_kernel_torch.cpp.  It governs the FALLBACK branch of
# apply() -- the one taken when the library is too old to carry pxq4.linear_out
# -- and the arena warmup, which must cover the largest M the C++ policy can
# serve from mmv_out.  Both engines must read the same variable or the warmup
# sizes one ceiling and the serve asks for another.
_PXQ4_SMALLM_M = 16
...
class PXQ4LinearMethod(LinearMethodBase):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        if layer.pxq4_use_mmv and layer.pxq4_mmv_max_m > 0:
            # v14 (2026-09-07): warm at the SMALL-M CEILING, not at
            # mmv_max_m.  With PXA_PXQ4_SMALLM on, the C++ policy serves a whole
            # verify batch of up to 16 tokens from one mmv_out launch, so 16 is
            # the largest partials/counter request the arena will ever see from
            # this path -- and the arena refuses to grow after the first capture.
            # Warming at 8 and then meeting an M=16 verify at serve time is
            # exactly the hard error the freeze contract raises.
