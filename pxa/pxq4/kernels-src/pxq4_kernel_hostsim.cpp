...
// Block-wide OR-reduction barrier (k_pxq4_mmv_fused's winner-flag broadcast). Three barriers
// rather than one, because correctness beats speed in a test artifact: contribute, read, reset.
namespace { std::atomic<unsigned> g_bar_or{0}; }

int __syncthreads_or(int pred) {
    if (pred) g_bar_or.store(1u, std::memory_order_relaxed);
    __syncthreads();
    const int r = (int)g_bar_or.load(std::memory_order_relaxed);
    __syncthreads();
    if (threadIdx.x == 0 && threadIdx.y == 0 && threadIdx.z == 0)
        g_bar_or.store(0u, std::memory_order_relaxed);
    __syncthreads();
    return r;
}
