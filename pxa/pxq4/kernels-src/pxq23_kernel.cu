...
bool pxq23_mmv_supported(int kslabs) {
    ...
    // Static: 2*16 table floats + the KSEG*BM reduce tile + the multi-token kernel's `last`
    // flag. Budget for the LARGEST of the family so a shape can never be admitted here that
    // the v16 split kernels cannot run -- the dispatcher treats this predicate as the single
    // admission test for all of them.
...
int pxq23_mmv_nfix(int kslabs) {
    return pxq23_canon_nfix(kslabs, PXQ4_CANON_CMAX);
}

int pxq23_mmv_mt_smem_bytes(int kslabs, int M) {
    return pxq23_canon_max_chunk(kslabs) * PXQ4_QK * (int)sizeof(float) * M;
}

bool pxq23_mmv_mt_supported(int kslabs, int M) {
    // 1..16 are instantiated (see PXQ23_MT_CASE). The smem test is the real gate: it scales
    // linearly in M, so a wide K simply declines the arm and the caller falls back to the
    // per-token path exactly as before.
    if (M < 1 || M > 16) return false;
    const int stat_smem = (int)(2 * 16 * sizeof(float) + PXQ4_MMV_KSEG * PXQ4_BM * sizeof(float)
                                + 16);
    return pxq23_mmv_mt_smem_bytes(kslabs, M) + stat_smem <= 48 * 1024;
}
...
// ------------------------------------------------------------------- v16 split mmv launchers
//
// Every one of these carries the SAME nfix to every kernel it launches and asserts it against
// pxq23_canon_nfix. The pxq4 twin records why: pre-swap, the part kernel read nfix off gridDim
// while the reduce took it as an argument, with nothing asserting the two agreed, and a
// mismatch produced silently wrong output rather than a fault.
template <class POL>
static void pxq23_mmv_split_tpl(const uint8_t * slabs, const void * anchor, const void * x,
                                float * part, void * out, int M, int panels, int kslabs,
                                bool vecx, cudaStream_t stream) {
    const int    R    = panels * PXQ4_BM;
    const int    K    = kslabs * PXQ4_QK;
    const int    nfix = pxq23_mmv_nfix(kslabs);
    const size_t smem = (size_t)pxq23_mmv_smem_bytes(kslabs);
    const dim3   grid((unsigned)nfix, (unsigned)panels, (unsigned)M);   // c fastest
    if (vecx) {
        k_pxq23_mmv_part<POL, true><<<grid, 256, smem, stream>>>(
            slabs, (const __half *)anchor, (const __half *)x, part, K, nfix, panels);
    } else {
        k_pxq23_mmv_part<POL, false><<<grid, 256, smem, stream>>>(
            slabs, (const __half *)anchor, (const __half *)x, part, K, nfix, panels);
    }
    PXQ23_CUDA_CHECK(cudaGetLastError());
    const dim3 rgrid((unsigned)panels, (unsigned)M, 1u);
    k_pxq23_mmv_reduce<<<rgrid, PXQ4_BM, 0, stream>>>(part, (__half *)out, nfix, R);
    PXQ23_CUDA_CHECK(cudaGetLastError());
}

// Shape checks shared by the three split entry points. `need2` demands nfix >= 2, which the
// two fused forms require for their arrival barrier to be non-degenerate.
static void pxq23_split_shape_check(const char * what, int nfix, int kslabs, int panels, int M,
                                    bool need2) {
    if (nfix != pxq23_canon_nfix(kslabs, PXQ4_CANON_CMAX) || nfix < 1 || panels < 1) {
        fprintf(stderr, "pxq23: %s nfix/panels inconsistent (nfix=%d panels=%d kslabs=%d)\n",
                what, nfix, panels, kslabs);
        abort();
    }
    if (need2 && nfix < 2) {
        ...
        fprintf(stderr, "pxq23: %s requires nfix >= 2 (got %d)\n", what, nfix);
        abort();
    }
    if (panels > 65535 || M > 65535) {
        fprintf(stderr, "pxq23: %s grid limit exceeded: panels=%d M=%d\n", what, panels, M);
        abort();
    }
}
...
void pxq23_launch_mmv_split_f16(int tier, const uint8_t * slabs, const void * anchor,
    ...
    pxq23_split_shape_check("split mmv", pxq23_mmv_nfix(kslabs), kslabs, panels, M, false);
    switch (tier) {
        case PXQ_TIER_PXQ2: pxq23_mmv_split_tpl<pxq2_pol>(slabs, anchor, x, part, out, M, panels, kslabs, vecx, stream); return;
        case PXQ_TIER_PXQ3: pxq23_mmv_split_tpl<pxq3_pol>(slabs, anchor, x, part, out, M, panels, kslabs, vecx, stream); return;
        default: pxq23_bad_tier(tier, "pxq23_launch_mmv_split_f16");
    }
}

template <class POL>
static void pxq23_mmv_fused_tpl(const uint8_t * slabs, const void * anchor, const void * x,
    ...
    const dim3   grid((unsigned)nfix, (unsigned)panels, (unsigned)M);   // c fastest
    if (vecx) {
        k_pxq23_mmv_fused<POL, true><<<grid, 256, smem, stream>>>(
            slabs, (const __half *)anchor, (const __half *)x, part, ctr, (__half *)out,
            R, K, nfix, panels);
    } else {
        k_pxq23_mmv_fused<POL, false><<<grid, 256, smem, stream>>>(
            slabs, (const __half *)anchor, (const __half *)x, part, ctr, (__half *)out,
            R, K, nfix, panels);
    }
    PXQ23_CUDA_CHECK(cudaGetLastError());
...
void pxq23_launch_mmv_fused_f16(int tier, const uint8_t * slabs, const void * anchor,
                                ...
                                cudaStream_t stream) {
    pxq23_split_shape_check("fused mmv", pxq23_mmv_nfix(kslabs), kslabs, panels, M, true);
    switch (tier) {
        case PXQ_TIER_PXQ2: pxq23_mmv_fused_tpl<pxq2_pol>(slabs, anchor, x, part, ctr, out, M, panels, kslabs, vecx, stream); return;
        case PXQ_TIER_PXQ3: pxq23_mmv_fused_tpl<pxq3_pol>(slabs, anchor, x, part, ctr, out, M, panels, kslabs, vecx, stream); return;
        default: pxq23_bad_tier(tier, "pxq23_launch_mmv_fused_f16");
    }
}

// The multi-token form dispatches M to an exact-MT template so every inner loop fully unrolls.
template <class POL, bool VECX>
static void pxq23_dispatch_mmv_fused_mt(const uint8_t * slabs, const __half * anchor,
    ...
    switch (M) {
#define PXQ23_MT_CASE(MTV)                                                                    \
        case MTV:                                                                             \
            k_pxq23_mmv_fused_mt<POL, VECX, MTV><<<grid, 256, smem, stream>>>(                \
                slabs, anchor, x, part, ctr, out, R, K, nfix, panels);                        \
            break;
        PXQ23_MT_CASE(1)  PXQ23_MT_CASE(2)  PXQ23_MT_CASE(3)  PXQ23_MT_CASE(4)
        PXQ23_MT_CASE(5)  PXQ23_MT_CASE(6)  PXQ23_MT_CASE(7)  PXQ23_MT_CASE(8)
        PXQ23_MT_CASE(9)  PXQ23_MT_CASE(10) PXQ23_MT_CASE(11) PXQ23_MT_CASE(12)
        PXQ23_MT_CASE(13) PXQ23_MT_CASE(14) PXQ23_MT_CASE(15) PXQ23_MT_CASE(16)
#undef PXQ23_MT_CASE
        default:
            fprintf(stderr, "pxq23: fused mt mmv M out of range: %d\n", M);
            abort();
    }
}

template <class POL>
static void pxq23_mmv_fused_mt_tpl(const uint8_t * slabs, const void * anchor, const void * x,
                                   float * part, unsigned * ctr, void * out, int M, int panels,
                                   int kslabs, bool vecx, cudaStream_t stream) {
    const int    R    = panels * PXQ4_BM;
    const int    K    = kslabs * PXQ4_QK;
    const int    nfix = pxq23_mmv_nfix(kslabs);
    const size_t smem = (size_t)pxq23_mmv_mt_smem_bytes(kslabs, M);
    if (vecx) {
        pxq23_dispatch_mmv_fused_mt<POL, true>(slabs, (const __half *)anchor, (const __half *)x,
                                               part, ctr, (__half *)out, M, R, K, nfix, panels,
                                               smem, stream);
    } else {
        pxq23_dispatch_mmv_fused_mt<POL, false>(slabs, (const __half *)anchor, (const __half *)x,
                                                part, ctr, (__half *)out, M, R, K, nfix, panels,
                                                smem, stream);
    }
    PXQ23_CUDA_CHECK(cudaGetLastError());
...
void pxq23_launch_mmv_fused_mt_f16(int tier, const uint8_t * slabs, const void * anchor,
                                   ...
                                   int M, int panels, int kslabs, bool vecx,
                                   cudaStream_t stream) {
    pxq23_split_shape_check("fused mt mmv", pxq23_mmv_nfix(kslabs), kslabs, panels, M, true);
    if (!pxq23_mmv_mt_supported(kslabs, M)) {
        fprintf(stderr, "pxq23: fused mt mmv unsupported (kslabs=%d M=%d)\n", kslabs, M);
        abort();
    }
    switch (tier) {
        case PXQ_TIER_PXQ2: pxq23_mmv_fused_mt_tpl<pxq2_pol>(slabs, anchor, x, part, ctr, out, M, panels, kslabs, vecx, stream); return;
        case PXQ_TIER_PXQ3: pxq23_mmv_fused_mt_tpl<pxq3_pol>(slabs, anchor, x, part, ctr, out, M, panels, kslabs, vecx, stream); return;
        default: pxq23_bad_tier(tier, "pxq23_launch_mmv_fused_mt_f16");
    }
}
...
// ============================================================== v16 SPLIT DEVICE DIFFERENTIAL
//
// The gate for the K-chunk-split family. It is a DEVICE-VS-DEVICE comparison and it demands
// BIT-EXACT equality -- max-abs-diff 0 in the fp16 encoding -- not a tolerance, because that is
// what the port actually claims: the fold order is preserved exactly and the atomic is an
// arrival counter rather than an accumulator, so every output must equal the monolithic
// k_pxq23_mmv's to the bit. Anything less than 0 here is a real defect, not rounding.
//
// It is also the ONLY arm that can observe the fused kernels' barrier race. A host replay runs
// blocks sequentially and is structurally incapable of seeing it, so this must run on a card
// before any window: the failure mode of a missing release is silently stale fp16 output, which
// nothing downstream can detect.
//
// SHAPES. Deliberately ragged: panels and kslabs are non-powers-of-two so the canonical chunk
// boundaries b0 = kslabs*c/nfix land unevenly, and kslabs is chosen to exercise nfix = 2, 4, 8
// and the CMAX-capped 16 (nfix = 1 cannot reach the fused path at all and is covered by the
// monolithic arm above). M sweeps 1..16, which spans the whole dispatchable decode range
// including the multi-token ceiling.
//
// REPEAT ARM. Every launch is run PXQ23_DIFF_REPEATS times back to back on one stream. A
// completed fused launch must leave its counter slot at zero; if the rearm is wrong the FIRST
// launch still passes and the second hangs on a barrier nobody wins, so a single-shot check
// would miss it. The counter buffer is read back afterwards and required to be all zero.
namespace {

constexpr int PXQ23_DIFF_REPEATS = 3;

struct DiffShape { int panels, kslabs; };

int pxq23_split_differential(int tier) {
    HostTier T;
    if (!host_tier(tier, T) || tier == PXQ_TIER_PXQ4) return 0;   // pxq4 keeps its own gate

    const DiffShape shapes[] = { {3, 9}, {5, 24}, {7, 40}, {2, 160} };
    const int       ms[]     = { 1, 2, 3, 5, 8, 16 };

    uint32_t s = 0x5EEDBEEFu + (uint32_t)tier;
    int rc = 0;

    for (size_t si = 0; si < sizeof(shapes) / sizeof(shapes[0]) && rc == 0; ++si) {
        const int panels = shapes[si].panels;
        const int kslabs = shapes[si].kslabs;
        const int N      = panels * PXQ4_BM;
        const int K      = kslabs * PXQ4_QK;
        const int nfix   = pxq23_mmv_nfix(kslabs);
        if (nfix < 2 || !pxq23_mmv_supported(kslabs)) continue;
        ...
        uint8_t * d_slabs = nullptr; __half * d_anchor = nullptr;
        ...
        for (size_t mi = 0; mi < sizeof(ms) / sizeof(ms[0]) && rc == 0; ++mi) {
            const int M = ms[mi];
            ...
            __half * d_x = nullptr; __half * d_ref = nullptr; __half * d_got = nullptr;
            float * d_part = nullptr; unsigned * d_ctr = nullptr;
            const size_t nout   = (size_t)M * N;
            const size_t npart  = (size_t)M * panels * nfix * (PXQ4_MMV_KSEG * PXQ4_BM);
            const size_t nctr   = (size_t)M * panels;
            PXQ23_CUDA_CHECK(cudaMalloc(&d_x, h_x.size() * 2));
            PXQ23_CUDA_CHECK(cudaMalloc(&d_ref, nout * 2));
            PXQ23_CUDA_CHECK(cudaMalloc(&d_got, nout * 2));
            PXQ23_CUDA_CHECK(cudaMalloc(&d_part, npart * sizeof(float)));
            PXQ23_CUDA_CHECK(cudaMalloc(&d_ctr, nctr * sizeof(unsigned)));
            PXQ23_CUDA_CHECK(cudaMemcpy(d_x, h_x.data(), h_x.size() * 2, cudaMemcpyHostToDevice));
            PXQ23_CUDA_CHECK(cudaMemset(d_ctr, 0, nctr * sizeof(unsigned)));

            // reference: the monolithic kernel this whole family must reproduce to the bit
            pxq23_launch_mmv_f16(tier, d_slabs, d_anchor, d_x, d_ref, M, panels, kslabs, true, 0);
            PXQ23_CUDA_CHECK(cudaDeviceSynchronize());
            std::vector<uint16_t> ref(nout), got(nout);
            PXQ23_CUDA_CHECK(cudaMemcpy(ref.data(), d_ref, nout * 2, cudaMemcpyDeviceToHost));

            struct Arm { const char * name; int kind; };   // 0 = split2, 1 = fused, 2 = fused_mt
            const Arm arms[] = { {"split2", 0}, {"fused", 1}, {"fused_mt", 2} };
            for (size_t ai = 0; ai < sizeof(arms) / sizeof(arms[0]) && rc == 0; ++ai) {
                if (arms[ai].kind == 2 && !pxq23_mmv_mt_supported(kslabs, M)) continue;
                // poison the output so a kernel that never writes cannot pass by accident
                PXQ23_CUDA_CHECK(cudaMemset(d_got, 0xA5, nout * 2));
                for (int rep = 0; rep < PXQ23_DIFF_REPEATS; ++rep) {
                    switch (arms[ai].kind) {
                        case 0: pxq23_launch_mmv_split_f16(tier, d_slabs, d_anchor, d_x, d_part,
                                                           d_got, M, panels, kslabs, true, 0); break;
                        case 1: pxq23_launch_mmv_fused_f16(tier, d_slabs, d_anchor, d_x, d_part,
                                                           d_ctr, d_got, M, panels, kslabs, true, 0); break;
                        default: pxq23_launch_mmv_fused_mt_f16(tier, d_slabs, d_anchor, d_x, d_part,
                                                               d_ctr, d_got, M, panels, kslabs, true, 0); break;
                    }
                }
                PXQ23_CUDA_CHECK(cudaDeviceSynchronize());
                PXQ23_CUDA_CHECK(cudaMemcpy(got.data(), d_got, nout * 2, cudaMemcpyDeviceToHost));
                for (size_t i = 0; i < nout; ++i) {
                    if (got[i] != ref[i]) {
                        fprintf(stderr, "pxq23_selftest(%s): SPLIT DIFFERENTIAL MISMATCH, arm %s, "
                                        "panels=%d kslabs=%d nfix=%d M=%d, token %zu row %zu: "
                                        "0x%04x vs monolithic 0x%04x. This family claims BIT-EXACT "
                                        "equality with k_pxq23_mmv; any difference is a defect, "
                                        "not rounding.\n",
                                pxq_tier_name(tier), arms[ai].name, panels, kslabs, nfix, M,
                                i / (size_t)N, i % (size_t)N, got[i], ref[i]);
                        rc = 4; break;
                    }
                }
                if (rc == 0 && arms[ai].kind != 0) {
                    // the zero-on-exit contract: a completed launch must leave every counter
                    // slot rearmed, or the NEXT launch hangs on a barrier nobody wins
                    std::vector<unsigned> ctr(nctr);
                    PXQ23_CUDA_CHECK(cudaMemcpy(ctr.data(), d_ctr, nctr * sizeof(unsigned),
                                                cudaMemcpyDeviceToHost));
                    for (size_t i = 0; i < nctr; ++i) {
                        if (ctr[i] != 0u) {
                            fprintf(stderr, "pxq23_selftest(%s): arm %s left counter slot %zu at "
                                            "%u after %d launches (must be 0 -- the winner rearms "
                                            "it). panels=%d kslabs=%d M=%d\n",
                                    pxq_tier_name(tier), arms[ai].name, i, ctr[i],
                                    PXQ23_DIFF_REPEATS, panels, kslabs, M);
                            rc = 5; break;
                        }
                    }
                }
            }
            cudaFree(d_x); cudaFree(d_ref); cudaFree(d_got); cudaFree(d_part); cudaFree(d_ctr);
        }
        cudaFree(d_slabs); cudaFree(d_anchor);
    }
    if (rc == 0) {
        fprintf(stderr, "pxq23_selftest(%s): SPLIT DIFFERENTIAL PASS (split2/fused/fused_mt "
                        "bit-identical to the monolithic mmv over 4 shapes x 6 M x %d repeats, "
                        "counters rearmed)\n", pxq_tier_name(tier), PXQ23_DIFF_REPEATS);
    }
    return rc;
}
...
int pxq23_selftest(int tier) {
    ...
    // v16 arm: the split/fused/multi-token family against the monolithic mmv, BIT-EXACT.
    // Runs last because it allocates its own, larger, ragged shapes; skipped for tier 252,
    // which keeps the frozen pxq4 gates.
    if (rc == 0) rc = pxq23_split_differential(tier);
        ...
        fprintf(stderr, "pxq23_selftest(%s): PASS (dequant bit-exact vs host oracle%s)\n",
                ...
                tier == PXQ_TIER_PXQ4 ? " and vs the shipped pxq4 kernel"
                                      : ", mmv within the 1 ULP contraction allowance, "
                                        "split family bit-exact vs the monolithic mmv");
    }
    return rc;
}
