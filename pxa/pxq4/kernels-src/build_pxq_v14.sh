#!/usr/bin/env bash
# build_pxq_v14.sh -- build libpxq_<arch>_v14.so: the frozen v12b PXQ4 kernels PLUS the new
...
#
#   ARCH=sm60 ./build_pxq_v14.sh          -> ../kernels/libpxq_sm60_v14.so
#   ARCH=sm70 ./build_pxq_v14.sh          -> ../kernels/libpxq_sm70_v14.so
#
...
BUILD_DIR="${PXQ_BUILD_DIR:-$SRC_DIR/../build-pxq-v14-$ARCH}"
JOBS="${MAX_JOBS:-6}"
...
g++ -shared -o "$OUT_DIR/libpxq_${ARCH}_v14.so" k.o m.o k23.o q8.o pop.o t.o t23.o tq8.o tpop.o tmf.o \
    -L$TORCH/lib -ltorch -ltorch_cpu -ltorch_cuda -lc10 -lc10_cuda -L/usr/local/cuda/lib64 -lcudart
echo BUILD_OK
ls -la "$OUT_DIR/libpxq_${ARCH}_v14.so"
md5sum "$OUT_DIR/libpxq_${ARCH}_v14.so"
...
strings -a "$OUT_DIR/libpxq_${ARCH}_v14.so" \
...
for op in $PXQ_EXPECT_OPS; do
  strings -a "$OUT_DIR/libpxq_${ARCH}_v14.so" | grep -qx "$op" || MISSING="$MISSING $op"
done
if [ -n "$MISSING" ]; then
...
cuobjdump --list-elf "$OUT_DIR/libpxq_${ARCH}_v14.so" | sed -n 's/.*\.\(sm_[0-9]*\)\..*/\1/p' | sort -u
