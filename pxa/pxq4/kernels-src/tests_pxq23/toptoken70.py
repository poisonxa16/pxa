...
name, which is right for an arm-vs-arm gate on one server and wrong here: the question this package
...
Usage:
    python3 toptoken70.py capture --port 8000 --model fusion2-35b --tag llama-pxq2
    python3 toptoken70.py capture --port 8001 --model fusion2-35b-pxq2 --tag vllm-pxq2
    python3 toptoken70.py compare llama-pxq2 vllm-pxq2
    python3 toptoken70.py needle  --port 8001 --model fusion2-35b-pxq2 --fill 8192
"""
...
def top2_gap(choice) -> "float | None":
    ...
    if not tops:
        ...
        # The reference is the authority on whether a prompt is decisive.
...
#: THE PRE-REGISTERED GATE (fixed 2026-09-05 BEFORE the re-capture).
#: Written here, in the code, so it cannot drift between registration and reporting.
#:
...
def main() -> int:
    ...
    g = sub.add_parser("gate", help="the pre-registered gate")
