...
#define GGML_RESTRICT restrict
    ...
    enum ggml_pxa_model_class {
        ...
        GGML_PXA_MODEL_DENSE_HYBRID_RECURRENT = 5, // dense (no experts) + linear/recurrent hybrid (Gated-DeltaNet class: Qwen3.5 / Qwable)
    };
    ...
    // ------------------------------------------------------------------------
    // PXA master config level (2026-09-03) -- ONE definition for the whole tree.
    //
    //   0 REFERENCE : every PXA lever forced OFF (the bit-exact audit baseline).
    //                 Selected by PXA_REFERENCE=1, which wins over everything else.
    //   1 DEFAULT   : the historical shipped default set. Selected by PXA_ENHANCE=0.
    //   2 ENHANCE   : DEFAULT plus every measured per-card lever. THIS IS THE DEFAULT
    //                 when neither variable is set (changed 2026-09-03; it used to be
    //                 level 1, so the measured levers only fired for operators who
    //                 knew to export PXA_ENHANCE=1).
    //
    // Explicit per-lever env vars ALWAYS override the level -- the level only moves the
    // default each resolver falls back to when its own variable is unset.
    //
    // This lives in ggml.h, not in the CUDA-only pxa-enhance.cuh, because the level is
    // pure environment logic and six translation units outside the CUDA backend
    // (common/sampling.cpp, examples/server/server.cpp, examples/server/server-context.cpp,
    // src/llama.cpp) used to keep hand-mirrored copies of it. Mirrored copies is how a
    // level change lands at some sites and is silently ignored at the rest.
    GGML_API int          ggml_pxa_config_level(void);       // 0 / 1 / 2 as above
    GGML_API const char * ggml_pxa_config_level_name(void);  // "REFERENCE" / "DEFAULT" / "ENHANCE"
