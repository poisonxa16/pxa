...
static __device__ __forceinline__ float pxq4_glu_apply(float g, float u, int unary, float alpha, float limit) {
    ...
    if (unary == 3) {                    // GELU-glu (tanh approximation)
        // The same expression, in the same order, as fused_mul_gelu_f32 in unary.cu, which is
        // what the decomposed graph computes for this activation: gelu(gate) * up. Written out
        // rather than called through a gelu() helper so the float operations pair up one for one.
        const float GELU_COEF_A    = 0.044715f;
        const float SQRT_2_OVER_PI = 0.79788456080286535587989211986876f;
        return 0.5f*g*u*(1.0f + tanhf(SQRT_2_OVER_PI*g*(1.0f + GELU_COEF_A*g*g)));
    }
    // SILU-swiglu
