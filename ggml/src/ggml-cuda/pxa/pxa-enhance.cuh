...
//
// 2026-09-20: a second admitted case, and this one HAS a measured basis. On sm_70 the ids
// readback blocks the host inside every fused PXQ expert op - 1,205 us of host submit per call
// against 21 us for a route that does not read back - so the card starves between launches, and
// the smaller the experts the more it shows. Gemma 4 26B-A4B (128 experts of width 704), our
// PXQ4 file, two V100s, same binary and prompts, lever the only difference: prefill 2,347 ->
// 3,172 / 2,116 -> 2,976 / 1,799 -> 2,534 tok/s at 2.4k / 6.4k / 12.7k prompt tokens, decode
// unchanged, greedy text byte-identical in all eleven reps. On ONE V100 (our PXQ3 file, -c 16384)
// the same lever is a wash - prefill +3..4%, decode -1%, text identical - because a single card
// is busy either way; it is the hand-off between cards that the stall breaks. So: a Gemma 4
// routed-expert model on a MULTI-card all-sm_70 topology arms mode 1 at ENHANCE. One card, and
// other routed-expert architectures on sm_70, stay env-only until they are measured; a file whose
// experts are not PXQ never reaches the mapping this lever moves, so it is inert there.
...
static inline void pxa_enhance_log_model_decisions() {
    // The generated lever report comes FIRST, above both early returns below: the levers it covers
    // used to announce themselves at every config level and with or without a registered model
    // profile, so a REFERENCE run (the arm every A/B is measured against) must still say what they are.
    pxa_core_lever_report();
    pxa_core_lever_check_unknown();
    ...
    // The value printed is the one in EFFECT: with the env set this line used to print the
    // topology default next to the words "explicit env override", so =1 on a V100 pair read "0".
    fprintf(stderr, "PXA_AUTO: MOE_DEVICE_MAP=%d (%s; override PXA_MOE_DEVICE_MAP)\n",
            getenv("PXA_MOE_DEVICE_MAP") ? atoi(getenv("PXA_MOE_DEVICE_MAP")) : pxa_moe_device_map_default(),
            getenv("PXA_MOE_DEVICE_MAP") ? "explicit env override" :
            pxa_moe_device_map_default() ? "routed-expert model x ENHANCE on a multi-card sm_60 topology, or a Gemma 4 "
                                           "routed-expert model on a multi-card sm_70 one — expert-row mapping on device, no D2H "
                                           "inside graph compute"
                                         : (!m.n_expert ? "OFF: dense model — no expert row mapping exists"
                                                        : "OFF: not a measured topology/architecture for this lever, or level below ENHANCE"));
    ...
    // The lines above are hand-written: each one re-reads the environment and re-derives the
    // default it is describing, which is how a topology default came to be printed beside the
    // words "explicit env override". The lines below are GENERATED from the lever registry, which
    // is the same table the engine reads the value out of, so they cannot say anything the engine
    // is not doing. The hand-written ones are converted a few at a time.
}
