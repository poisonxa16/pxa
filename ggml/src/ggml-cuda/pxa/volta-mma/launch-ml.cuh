#pragma once
// -----------------------------------------------------------------------------
// Vendored from ggml-org/llama.cpp (mainline), commit
// 9400c8946e4da5e7694f2c26d6d4e50e14b690fa (2026-09-02), file
// ggml/src/ggml-cuda/fattn-common.cuh -- the launch half only: the kernel ABI,
// the causal KV_max bound, the stream-k fixup, the result combine and
// launch_fattn itself. The vec-kernel dot products and the quantized-KV
// dequantizers (mainline lines 87-663) are NOT vendored: the MMA kernel does not
// reference them, and this fork already has its own.
//
// llama.cpp / ggml are MIT licensed:
//   MIT License, Copyright (c) 2023-2026 The ggml authors
//
// Everything lives in namespace pxa_volta_fa so it cannot collide with this
// fork's own launch_fattn / fattn_kernel_t, whose template signature and kernel
// ABI are different and incompatible. Local edits are marked "PXA:".
// -----------------------------------------------------------------------------

#include "volta-mma-compat.cuh"
#include <cstdint>

namespace pxa_volta_fa {

// PXA: mainline names this FATTN_KQ_STRIDE; renamed to avoid clashing with this
// fork's identically-named macro in ggml-cuda/fattn-common.cuh.
...
// PXA (2026-09-21): the staging hook that used to live here handed out addresses PAST the end of
// the destination tensor, on the assumption that the backend oversizes that buffer for exactly this
// purpose. This tree does not oversize it, so those addresses belonged to whatever was allocated
// next, and the dispatcher refused a non-f16 cache outright rather than write into them. The
// scratch now comes from the CUDA pool, in launch_fattn below, which needs no backend hook at all;
// the hook and its struct are gone rather than left unused.
...
void launch_fattn(
    ...
    // DECLARATION ORDER IS LOAD-BEARING. The pool is a stack and frees in strict reverse allocation
    // order. These two are allocated in the conversion block below, BEFORE KV_max/dst_tmp/
    // dst_tmp_meta are allocated further down, and C++ destroys locals in reverse declaration
    // order -- so they have to be declared FIRST for the two orders to agree. Declared after
    // KV_max, the frees come back out of order and the pool's stack discipline breaks.
    ...
    if (need_f16_K && K->type != GGML_TYPE_F16) {
        ...
        half * K_f16 = K_f16_pool.alloc(ggml_nelements(K));
        if (ggml_is_contiguously_allocated(K)) {
            to_fp16_cuda_t to_fp16 = ggml_get_to_fp16_cuda(K->type);
            to_fp16(K_data, K_f16, 1, ggml_nelements(K), main_stream); // PXA: nrows=1, n_per_row=k
    ...
    if (need_f16_V && V->type != GGML_TYPE_F16) {
        if (V_is_K_view) {
            ...
            half * V_f16 = V_f16_pool.alloc(ggml_nelements(V));
            if (ggml_is_contiguously_allocated(V)) {
                to_fp16_cuda_t to_fp16 = ggml_get_to_fp16_cuda(V->type);
                to_fp16(V_data, V_f16, 1, ggml_nelements(V), main_stream); // PXA: nrows=1, n_per_row=k
                V_data = (char *) V_f16;
...
// PXA: sparse flash attention is gated on turing_mma_available() in mainline's
// ggml_cuda_flash_attn_ext_mma_f16_shall_use_sparse(), so launch_fattn is never
// called with use_sparse=true on sm_70 -- the only arch this vendored path
// serves. The mask-compaction kernel (mainline fattn.cu:30-89) is therefore not
// vendored and this stub stands in for its declaration above.
inline void ggml_cuda_flash_attn_ext_compact_mask(
        const ggml_tensor * mask, int32_t * indices, int32_t n_kv_max, cudaStream_t stream) {
    GGML_UNUSED_VARS(mask, indices, n_kv_max, stream);
    GGML_ABORT("PXA: sparse flash attention is not vendored into the Volta MMA path");
}
