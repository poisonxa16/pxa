#pragma once
// -----------------------------------------------------------------------------
// Vendored from ggml-org/llama.cpp (mainline), commit 9400c8946e4da5e7694f2c26d6d4e50e14b690fa (2026-09-02),
// file ggml/src/ggml-cuda/fattn-swizzle.cuh -- shared-memory swizzle helpers for the MMA flash-attention kernel
//
// llama.cpp / ggml are MIT licensed:
//   MIT License, Copyright (c) 2023-2026 The ggml authors
//   (full text: the LICENSE file at the root of the llama.cpp repository)
//
// Vendored rather than merged because this fork's ggml-cuda is ik_llama-derived
// and its mma.cuh / fattn-common.cuh have diverged from mainline's: the two
// launch_fattn template signatures and the two fattn_kernel_t ABIs are not
// compatible. Symbols are renamed into pxa_volta_* namespaces so both
// implementations can coexist in one binary without ODR conflicts.
// Local edits are marked "PXA:".
// -----------------------------------------------------------------------------
#include "volta-mma-compat.cuh"

// PXA: common.cuh comes via volta-mma-compat.cuh
#include "mma-ml.cuh" // PXA: the vendored tiles, not this fork's own mma.cuh
...
static __device__ __forceinline__ void load_ldmatrix(
    ...
    if constexpr (swz) {
        static_assert(std::is_same_v<TileT, pxa_volta_mma::tile<16, 8, half2>>,
        ...
        pxa_volta_mma::load_ldmatrix(t, tile_base + base_row*stride_h2 + base_col_h2, stride_h2);
    }
}
...
static __device__ __forceinline__ void load_ldmatrix(TileT & t, const half2 * tile_base, const int off_h2) {
    if constexpr (swz) {
        ...
        pxa_volta_mma::load_ldmatrix(t, tile_base + off_h2, stride_h2);
    }
}
...
static __device__ __forceinline__ void load_ldmatrix_trans(
    ...
    if constexpr (swz) {
        ...
        pxa_volta_mma::load_ldmatrix_trans(t, tile_base + base_row*stride_h2 + base_col_h2, stride_h2);
    }
}
...
static __device__ __forceinline__ void load_ldmatrix_trans(TileT & t, const half2 * tile_base, const int off_h2) {
    if constexpr (swz) {
        ...
        pxa_volta_mma::load_ldmatrix_trans(t, tile_base + off_h2, stride_h2);
    }
}
