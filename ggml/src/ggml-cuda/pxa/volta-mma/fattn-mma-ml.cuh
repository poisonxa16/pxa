...
// ggml/src/ggml-cuda/fattn-mma-f16.cuh -- the MMA flash-attention kernel. On
// sm_70 this compiles to the mma.sync.aligned.m8n8k4 path (VOLTA_MMA_AVAILABLE)
// and packs gqa_ratio Q heads per K/V read via ncols2.
//
// llama.cpp / ggml are MIT licensed:
//   MIT License, Copyright (c) 2023-2026 The ggml authors
//
// Wrapped in namespace pxa_volta_fa alongside the vendored launch_fattn.
// Local edits are marked "PXA:".
// -----------------------------------------------------------------------------

#include "volta-mma-compat.cuh"
#include "../../cp-async.cuh"   // PXA: this fork's own; inert on sm_70 (no cp.async)
#include "mma-ml.cuh"
#include "launch-ml.cuh"
#include "swizzle-ml.cuh"
...
#pragma unroll
                for (int k0 = k0_start; k0 < k0_stop; k0 += stride_k) {
                    ...
                    if constexpr (swz) {
                        const int smem_offs_b = pxa_volta_swizzle::bytes_rc<stride_tile>(i, k*h2_per_chunk);
...
#pragma unroll
                for (int k0 = k0_start; k0 < k0_stop; k0 += stride_k) {
                    ...
                    if constexpr (swz) {
                        ggml_cuda_memcpy_1<16>((char *) tile_KV + pxa_volta_swizzle::bytes_rc<stride_tile>(i, k*h2_per_chunk), src);
                    } else {
...
static __device__ __forceinline__ void flash_attn_ext_f16_iter(
    ...
    constexpr int stride_tile_K = pxa_volta_swizzle::tile_stride(nbatch_K2);
    constexpr int stride_tile_V = V_is_K_view ? stride_tile_K : pxa_volta_swizzle::tile_stride(nbatch_V2);
    constexpr bool swz_K = pxa_volta_swizzle::enabled(nbatch_K2);
    constexpr bool swz_V = V_is_K_view ? swz_K : pxa_volta_swizzle::enabled(nbatch_V2);
...
#pragma unroll
                for (int k_KQ_0 = k0_start; k_KQ_0 < k0_stop; k_KQ_0 += T_A_KQ::J) {
                    T_A_KQ K_A;
                    pxa_volta_swizzle::load_ldmatrix<stride_tile_K, swz_K>(K_A, tile_K, i_KQ_0, k_KQ_0 - k0_start);
...
#pragma unroll
            for (int k00 = 0; k00 < nbatch_fa/2; k00 += np*T_A_VKQ::J) {
                ...
                pxa_volta_swizzle::load_ldmatrix_trans<stride_tile_V, swz_V>(A, tile_V, (int)(tile_V_i - tile_V) + 2*k0*stride_tile_V + (i_VKQ_0 - i0_start)/2);
...
#pragma unroll
            for (int k00 = 0; k00 < nbatch_fa; k00 += np*T_A_VKQ::I) {
                ...
                pxa_volta_swizzle::load_ldmatrix<stride_tile_V, swz_V>(A, tile_V, (int)(tile_V_i - tile_V) + k0*stride_tile_V + (i_VKQ_0 - i0_start)/2);
...
void ggml_cuda_flash_attn_ext_mma_f16_case(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    // PXA: this fork's cuda_device_info has no warp_size member (it is an
    // NVIDIA-only build, where the warp is always 32 lanes).
    const int warp_size_host = WARP_SIZE;
    GGML_UNUSED(ctx);
    ...
    const int stride_tile_K = pxa_volta_swizzle::tile_stride(nbatch_K2, cc);
    const int stride_tile_V = V_is_K_view ? stride_tile_K : pxa_volta_swizzle::tile_stride(nbatch_V2, cc);
