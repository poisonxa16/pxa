#pragma once
// -----------------------------------------------------------------------------
// Vendored from ggml-org/llama.cpp (mainline), file
// ggml/src/ggml-cuda/fattn-tile.cuh -- the no-tensor-core tile flash-attention
// kernel, kept for its 512/512 head family. llama.cpp / ggml are MIT licensed:
//   MIT License, Copyright (c) 2023-2026 The ggml authors
//
// WHY THIS FILE EXISTS. At head 512 on sm_70 the MMA kernel next door cannot be
// dispatched below 32 columns per block, so a decode step has to compute a
// four-token tile for one token. This kernel has no such floor: its 512/512
// config row runs at eight columns per block with two blocks per multiprocessor,
// which is the schedule upstream itself selects for this head size at decode on
// this architecture. It is the second candidate for PXA_FA_D512_VOLTA.
//
// Everything lives in namespace pxa_volta_fa, beside the vendored launch_fattn
// and the kernel ABI it shares, so it cannot collide with this fork's own
// identically-named tile kernels, whose template signatures are different.
// Local edits are marked "PXA:".
// -----------------------------------------------------------------------------

#include "../volta-mma/launch-ml.cuh"

namespace pxa_volta_fa {

// PXA: launch-ml.cuh #undefs FATTN_KQ_STRIDE at its end so the macro does not leak into this
// fork's own headers, which define it with a different value. This file needs it, and it needs
// the vendored value, so it is restored here for the duration of this header and dropped again
// at the bottom.
#define FATTN_KQ_STRIDE 256

// PXA: this fork's common.cuh has no RDNA predicate. Nothing here targets AMD -- the config
// selector below only reaches it under GGML_CUDA_CC_IS_AMD, which is false on every card this
// kernel is dispatched on -- so a constant false keeps the AMD branch compiling and unreachable.
#ifndef GGML_CUDA_CC_IS_RDNA
#define GGML_CUDA_CC_IS_RDNA(cc) (false)
#endif

// PXA: mainline's warp reductions are templated on the warp width; this fork's are not. A full
// 32-lane reduction goes to this fork's own; a narrower one (the pair reduction of the fp32
// schedule, which only an architecture without fast fp16 compiles) is mainline's loop verbatim.
// The half-precision forms stay 32-lane only: nothing narrower instantiates them. Declared in
// this namespace so the vendored call sites find them first.
template <int width>
...
template <int width>
static __device__ __forceinline__ float2 warp_reduce_sum(float2 x) {
    if constexpr (width == 32) {
        return ::warp_reduce_sum(x);
    } else {
#pragma unroll
        for (int offset = width/2; offset > 0; offset >>= 1) {
            x.x += __shfl_xor_sync(0xffffffff, x.x, offset, width);
            x.y += __shfl_xor_sync(0xffffffff, x.y, offset, width);
        }
        return x;
    }
}
...
static __device__ __forceinline__ half2 warp_reduce_max(half2 x) {
    static_assert(width == 32, "this fork's warp reductions are 32-lane only");
    return ::warp_reduce_max(x);
}
template <int width>
static __device__ __forceinline__ half2 warp_reduce_sum(half2 x) {
    static_assert(width == 32, "this fork's warp reductions are 32-lane only");
    return ::warp_reduce_sum(x);
}

// PXA: mainline takes this from fattn-common.cuh, which is not vendored here.
static __device__ __forceinline__ float ml_tile_get_alibi_slope(
...
#define get_alibi_slope ml_tile_get_alibi_slope
...
// PXA: only the head sizes this fork actually routes here are instantiated --
// building the rest would cost compile time for code no dispatcher can reach.
// Mainline's ggml_cuda_flash_attn_ext_tile() head-size switch is dropped with
// them; the entry points are the wrappers in pxa/fattn-volta-tile.cu.
//   512/512 -- the decode step of a 512-wide head, which has no other kernel
//              on sm_70 that can run one token's worth of work.
//   256/256 -- the SPECULATIVE VERIFY step (query width 2 and up) of a 256-wide
//              head, where this fork's vector kernel is NO_DEVICE_CODE above one
//              column and the node falls through to the legacy WMMA tile.
