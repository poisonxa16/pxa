...
// PXA_CPY_FASTDIV=1 enables the fastdiv index math in the float cpy kernel.
static bool pxa_cpy_fastdiv_enabled() {
    static const bool v = [] {
        // DEFAULT 2026-09-03: ON at ENHANCE (the default level), OFF at DEFAULT/REFERENCE. An
        // integer division replaced by the multiply-shift that computes the same quotient over
