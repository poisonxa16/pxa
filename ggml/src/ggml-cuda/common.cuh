...
// Strong definition in ggml-cuda.cu. 1 iff every CUDA device visible to THIS process is
// Volta-class (7.0 <= cc < 8.0). Lets a house lever that measured a win only on sm_70 default ON
// for the V100 seat while staying OFF on the P100 / 1080 Ti / mixed seats.
extern "C" int ggml_pxa_cuda_is_volta_only(void);

// The shared shape of a LEVEL-defaulted house lever: an explicit environment setting ALWAYS wins
// in both directions (so every A/B in the campaign log still reproduces bit-for-bit); with the
// variable unset the lever follows the PXA config level -- ON at ENHANCE (the default since
// 2026-09-03), OFF at DEFAULT and REFERENCE. Cache the result in a function-local static at the
// call site -- this itself does a getenv and must not sit on a per-token path.
//
// The four kernels gated this way (get_rows narrow, cpy fastdiv, concat flat, norm regcache) plus
// the topk-moe multi-row barrier are index-math and launch-geometry changes: no architecture
