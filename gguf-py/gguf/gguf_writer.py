    ...
    def get_total_parameter_count(self) -> tuple[int, int, int, int]:
        ...
        for tensors in self.tensors:
            for name, info in tensors.items():
                ...
                # "_exps." alone is not enough: a routed-expert WEIGHT stack is 3-D (and a bias
                # 2-D), but the sparse Gemma 4 also carries blk.N.ffn_down_exps.scale, one scalar
                # per expert and so 1-D. Indexing shape[-3] on that threw IndexError and killed
                # the conversion at the very end, after every tensor had been written. A 1-D
                # per-expert vector is a handful of floats; count it with the shared parameters.
                if "_exps." in name and len(shape) >= (2 if ".bias" in name else 3):
