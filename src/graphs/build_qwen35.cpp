...
struct ggml_tensor * llm_build_context::build_qwen35moe_mtp(
    ...
    // rows. Reached from the MTP graph only - the target's own head above keeps the full
    // vocabulary, which is what leaves the verified output distribution unchanged.
    cur = build_output(lctx, ctx0, cur,
            model.output_mtp_shortlist ? model.output_mtp_shortlist : model.output_mtp,
            mtp_layer.nextn.shared_head_norm, cb, false);
    cb(cur, "result_output", -1);
