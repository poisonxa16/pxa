...
// PXA_FA_TILE_512 mirrored here. The lever itself lives in the CUDA backend
// (ggml_cuda_fattn_tile_big_armed()); the graph builder only has to know whether a 512/512
// flash-attention node will be accepted, and the answer is "only when that lever is armed".
static bool pxa_gemma4_split_fa_tile512(void) {
    static const bool v = [] {
...
static ggml_cgraph * build_gemma4_graph_parallel(llm_build_context & llm, llama_context & lctx, ggml_context * ctx0,
    ...
    for (int il = 0; il < hparams.n_layer; ++il) {
        ...
        for (int id = 0; id < n_device; ++id) {
            ...
            // The V rows this device owns. On the five full-attention layers Gemma 4 has no wv at
            // all (K is V), so the K projection gives the shape. This USED TO ASSIGN wv = wk, and
            // wv is declared outside the device loop: after device 0 the next device saw a non-null
            // wv and built a second, redundant V projection from the K weights. Same numbers, one
            // extra matmul per full layer per device. A local keeps wv meaning what it says.
            const ggml_split_tensor_t * wv_shape = wv ? wv : wk;
            auto v_cache_view = ggml_view_1d(ctx0, vl->splits[id], n_tokens*wv_shape->splits[id]->ne[1],
                    llm.kv_head*ggml_row_size(vl->splits[id]->type, wv_shape->splits[id]->ne[1]));
            lctx.cache_copies[idx+1].step = ggml_row_size(vl->splits[id]->type, wv_shape->splits[id]->ne[1]);
            ...
            auto v = ggml_view_3d(ctx0, split_vl, n_embd_head_v, llm.n_kv, n_head_kv,
                    ggml_row_size(split_vl->type, wv_shape->splits[id]->ne[1]),
                    ggml_row_size(split_vl->type, n_embd_head_v), 0);
            cb(v, "v", il_cb);

            // D512 AND THE SCHEDULER. Gemma 4's five full-attention layers are 512/512, and no
            // CUDA backend accepts a 512/512 FLASH_ATTN_EXT node by default: sm_60 has no kernel
            // for that head size at all, and the sm_70 fused route is reached only through the
            // mark llm_build_kqv sets in op_params[5], which this builder deliberately does not
            // set (fa-route.cu: "an unmarked 512/512 node keeps the route it always had"). An
            // unsupported node is not an error - ggml_backend_sched silently places it on the CPU
            // backend, and every graph evaluation then makes that layer's whole K and V host
            // visible, a cost proportional to the CACHE rather than to the prompt. '-sm layer'
            // does not pay it: llm_build_kqv asks whether any GPU backend takes the node and
            // builds the unfused chain when none does (PXA_FA_GPU_FALLBACK, default on under
            // ENHANCE). The split builder never asked, so a tensor split of this model measured
            // five layers of host round trip and called it a split. Same question, same answer:
            // keep the layer on the card. PXA_FA_TILE_512 arms a real 512/512 kernel on both
            // sm_60 and sm_70, and when it is set the node is supported and the chain is skipped.
            const bool d512_chain =
                n_embd_head_k == 512 && n_embd_head_v == 512 && !pxa_gemma4_split_fa_tile512();
            if (d512_chain) {
                if (cparams.v_cache_hadamard && lctx.model.hadamard_size_v(il) > 0) {
                    throw std::runtime_error("gemma4 split: the V-cache Hadamard has no unfused "
                                             "attention path; run with PXA_FA_TILE_512=1 or without "
                                             "the V-cache Hadamard");
                }
                // The FA layout stores V untransposed; the chain multiplies by it the other way
                // round, exactly as llm_build_kqv's own chain does for a non-transposed V cache.
                ...
                if (hparams.attn_soft_cap) {
                    ...
                    kq = ggml_soft_max_ext(ctx0, kq, KQ_mask_l, hparams.f_attention_scale,
                            hparams.f_max_alibi_bias);
                }
                ...
                cur = ggml_cont_2d(ctx0, kqv_merged, n_embd_head_v*q->ne[2], n_tokens);
        ...
        for (int id = 0; id < n_device; ++id) {
            ...
            if (is_moe) {
                ...
                // The split builder builds the sparse FFN from the merged expert tensor and the
                // per-expert down scale only. Today it is unreachable for this arch (-sm graph and
                // -sm attn are demoted to -sm layer at load), so this is a fence for the day that
                // demotion is lifted: a file without either tensor must fail as a clean graph-build
                // error, not as a null dereference inside the MoE builder.
                // The released QAT GGUF carries the MERGED blk.N.ffn_gate_up_exps; a file quantised
                // here carries the separate blk.N.ffn_{gate,up}_exps. The loader already splits
                // both layouts and the single-device builder already accepts both - it passes the
                // separate pair AND the merged tensor and lets the MoE builder take whichever is
                // there. This one demanded the merged tensor and threw on everything else, which
                // is what refused every PXQ Gemma 4 file under '-sm tensor'. Hand it the same two
                // things the single-device call hands it.
                auto split_of = [&](ggml_tensor * t) -> ggml_tensor * {
                    return t ? ((const ggml_split_tensor_t *)t->extra)->splits[id] : nullptr;
                };
                if (!model.layers[il].ffn_up_gate_exps &&
                    !(model.layers[il].ffn_up_exps && model.layers[il].ffn_gate_exps)) {
                    throw std::runtime_error("gemma4: the split graph needs either the merged "
                                             "blk.N.ffn_gate_up_exps tensor or the separate "
                                             "blk.N.ffn_gate_exps and blk.N.ffn_up_exps; this file "
                                             "has neither - run it with -sm layer");
                }
                if (!model.layers[il].ffn_down_exps_s) {
                    throw std::runtime_error("gemma4: the split graph needs blk.N.ffn_down_exps.scale; "
                                             "this file lacks it - run it with -sm layer");
                }
                auto moe = llm. llm_build_moe_ffn(ctx0, lctx, cur,
                        nullptr,
                        split_of(model.layers[il].ffn_up_exps),
                        split_of(model.layers[il].ffn_gate_exps),
                        split_of(model.layers[il].ffn_down_exps), nullptr,
                        llm.n_expert, llm.n_expert_used,
                        LLM_FFN_GELU, true, false, 0.0f,
                        LLM_EXPERT_GATING_FUNC_SOFTMAX,
                        cb, il, gf, false,
                        split_of(model.layers[il].ffn_up_gate_exps),
                        nullptr, logits, split_of(model.layers[il].ffn_down_exps_s));
...
ggml_cgraph * llm_build_context::build_gemma4() {
    ...
    // '-sm tensor' resolves to LLAMA_SPLIT_MODE_ATTN (llama.cpp), so testing GRAPH alone sent a
    // tensor-split load through the single-device builder -- which then reads the split weights'
    // dummy base pointers. Every other arch with a split builder tests both values.
