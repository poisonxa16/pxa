...
// Defined further down this file, next to the split policy they belong to; declared here
// because the LM-head precondition check below needs the granularity before it creates
// anything.
...
struct create_tensors_helper : public create_tensors_helper_interface {
    ...
    // Which arches may take the vocab-parallel head. NOT every arch that reaches the helper below:
    // create_embd_output() is shared by ~16 arches and most of their graph builders project the
    // logits with a bare llm_build_lora_mm(model.output, cur) instead of going through
    // llm_build_context::build_output(). build_output is the ONLY consumer that resolves ->extra
    // into the per-device slices; a builder that does not use it reads the parent tensor, whose
    // data under the split buffer is the dummy 0x1000 base -- silent garbage, which is the exact
    // hazard rule #1 of this file is written about. The same goes for output_norm: a norm is not a
    // mul_mat, so the CUDA backend declines it and the scheduler runs it on the CPU over that dummy
    // pointer.
    //
    // So the list is arches whose builder routes the head through build_output AND that have been
    // measured through this lever. Adding one means checking both facts, in that order.
    static bool pxa_tsplit_lmhead_arch_ok(llm_arch arch) {
        switch (arch) {
            case LLM_ARCH_QWEN35:   // build_qwen35.cpp -> build_output(..., model.output_norm, ...);
                                    // measured 2026-09-20 on 2x P100, ledger
                                    // pxa-tsplit-lmhead-vocab-parallel
                return true;
            default:
                return false;
        }
    ...
    // PXA_TSPLIT_LMHEAD: where the LM head and its norm are created. `split` puts them in the split
    // context, which makes the head vocab-parallel; `plain` leaves them on one device, which is the
    // behaviour without the lever. Every precondition is checked here rather than at the call sites
    // so that an arch cannot half-adopt it: the mode has to be the PXA tensor split, there have to
    // be at least two participating devices, and the output buffer must not be a host buffer (a
    // host output buffer is never split -- see the end of create_tensors()).
    //
    // `head_name` is the name the LM head will be created under, and it is not decoration. Two
    // preconditions can only be answered from it, and both were found by reading rather than by
    // crashing:
    //   * -ot / --override-tensor is matched by std::regex_search on the tensor NAME, so a recipe
    //     like -ot "output\.weight=CPU" moves the head out of the split context while leaving
    //     output_norm in it. The norm would then sit in the split buffer with no ->extra: never
    //     uploaded, read as garbage. Deciding the head and its norm TOGETHER, from the head's own
    //     name, is what keeps them from being separated.
    //   * create_split() opens with GGML_ASSERT(nr % granularity == 0). A vocab that is not a
    //     multiple of the granularity (16 for a row-addressed head, the panel height for a PXQ one)
    //     would abort the process at load with a bare assert -- on a mode that promises a named
    //     reason and a named fix. Checked here instead, where the answer is a warning and a
    //     single-device head.
    inline ggml_context * pxa_tsplit_lmhead_head_ctx(ggml_context * plain, ggml_context * split,
            const std::string & head_name) {
        if (!model.pxa_tsplit || !pxa_tsplit_lmhead_split()) {
            return plain;
        }
        ...
        for (const auto & o : overrides) {
            if (std::regex_search(head_name, o.first)) {
                LLAMA_LOG_WARN("%s: PXA_TSPLIT_LMHEAD is set but '%s' is sent elsewhere by "
                        "--override-tensor: the head and its norm stay together, single-device\n",
                        __func__, head_name.c_str());
                return plain;
            }
        }
        if (model.mtp) {
        ...
        // warning here, not GGML_ASSERT(nr % granularity == 0) inside create_split() later.
        const ggml_tensor * meta = ml.get_tensor_meta(head_name.c_str());
        if (meta == nullptr) {
            // No head in the file: it is duplicated from the token embedding, which is created in
            // the input context and is not what this lever is about.
            return plain;
        }
        const int gran = pxa_type_is_pxq_panel(meta->type)
                       ? pxa_split_granularity(meta->type, 16) : 16;
        if (gran > 0 && (meta->ne[1] % gran) != 0) {
            LLAMA_LOG_WARN("%s: PXA_TSPLIT_LMHEAD is set but '%s' has %" PRId64 " rows, which is not "
                    "a multiple of the %d-row unit a %s head is cut on: the head stays "
                    "single-device\n", __func__, head_name.c_str(), meta->ne[1], gran,
...
void create_tensors_helper::create_embd_output(const LLM_TN & tn, int n_embd, int n_vocab, bool has_norm, bool use_ctx_split) {
    ...
    auto out_ctx = use_ctx_split ? ctx_output_split
                                 : pxa_tsplit_lmhead_head_ctx(ctx_output, ctx_output_split,
...
bool create_tensors_helper::create_qwen35_tensors(const LLM_TN & tn) {
    ...
    // output
    {
        // PXA_TSPLIT_LMHEAD. The LM head is the largest single tensor in the model and the last
        // matmul of every token. Left in ctx_output it is a SINGLE-DEVICE tensor under a tensor
        // split: one card holds it and does the final projection while the others have already
        // finished. Created in ctx_output_split instead, it lands in split_tensors and the policy
        // at the end of create_tensors() cuts it vocab-parallel (dim 1), with build_output
        // concatenating the per-device slices back into one logits vector for the sampler.
        // The output norm goes with it and is MIRRORED (split_dim -1), because each device has to
        // normalise the hidden state itself before its own slice of the projection; leaving the
        // norm single-device would make every other device read across the bus for it.
        // OFF BY DEFAULT: the concatenation is a different summation from one device's GEMV, so
        // this changes output, and it is measured before it is a default.
        auto out_ctx = pxa_tsplit_lmhead_head_ctx(ctx_output, ctx_output_split,
                                                  tn(LLM_TENSOR_OUTPUT, "weight"));
        model.output_norm = create_tensor(out_ctx, tn(LLM_TENSOR_OUTPUT_NORM, "weight"), {n_embd});
...
bool create_tensors_helper::create_gemma4_tensors(const LLM_TN & tn) {
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        if (has_expert) {
            ...
            // MoE FFN. Google's QAT 26B-A4B GGUF carries the merged blk.N.ffn_gate_up_exps (gate
            // rows first); a converter may also write the two halves as separate
            // blk.N.ffn_gate_exps / blk.N.ffn_up_exps tensors, so accept either. The separate
            // layout has not been run on any released file: supported, not measured.
            layer.ffn_up_gate_exps  = create_tensor(ctx_split, tn(LLM_TENSOR_FFN_GATE_UP_EXPS,  "weight", i), {n_embd, n_ff_exp * 2, n_expert}, llama_model_loader::TENSOR_NOT_REQUIRED);
            if (layer.ffn_up_gate_exps == nullptr) {
...
// Both predicates now live in src/pxa-pxq-split.h, so that the admission check in llama.cpp -- which
// has to answer the same question about a file BEFORE tensor creation starts -- cannot drift away
// from the backstop below.
static bool pxa_type_is_pxq_panel(enum ggml_type t) {
    return pxa_pxq_type_is_panel(t);
}

static bool pxa_type_pxq_k_split_ok(enum ggml_type t) {
    return pxa_pxq_k_split_ok(t);
}
    ...
    if (pxa_type_is_pxq_panel(tensor->type)) {
        ...
        if (split_dim == 0 && !pxa_type_pxq_k_split_ok(tensor->type)) {
            throw std::runtime_error(format(
                "a tensor split cannot cut the PXQ tensor '%s' (type %s) on dim 0 (K): the K-split "
                "ladder is turned off (PXA_TSPLIT_KSPLIT_LADDER=0), which leaves PXQ4 as the only "
                "K-splittable tier. Unset PXA_TSPLIT_KSPLIT_LADDER to use the whole ladder, or run "
                "'-sm layer'.",
...
bool create_tensors_helper::create_tensors() {
...

    {
        ...
        // PXA_TSPLIT_GEMMA4=1 opens this door too, and only for a request that already passed the
        // '-sm tensor' capability check. Both demotes have to read the same lever: the tensors
        // were created in the SPLIT contexts (use_split_ctx above keys on the same split_mode),
        // so demoting here and not there leaves CUDA_Split buffers that the split policy below
        // never fills, and the load dies at "Failed to allocate buffer type CUDA_Split".
    ...
    if (model.split_mode == LLAMA_SPLIT_MODE_GRAPH || model.split_mode == LLAMA_SPLIT_MODE_ATTN) {
        ...
        if (model.output) {
            if (auto it = split_tensors.find(model.output); it != split_tensors.end()) {
                if (ggml_backend_buft_is_host(model.buft_output.buft_matrix)) {
                    ...
                    // The granularity used to be a flat 16, which is a quant block and not a panel:
                    // a PXQ LM head cut at 16 rows lands inside a 64-row panel and prepare_split_tensors
                    // refuses it. Ask the codec what it admits instead, so a PXQ output head splits
                    // rather than throwing.
                    // Only PANEL types get the granularity raised. Leaving it at 16 for everything
                    // else keeps every arch that splits its head today splitting it exactly as
                    // before -- create_split asserts nr % granularity == 0, so raising it blindly
                    // would turn a working split into an abort on any vocab not divisible by 64.
                    auto out_gran = pxa_type_is_pxq_panel(model.output->type)
                                  ? pxa_split_granularity(model.output->type, 16) : 16;
                    auto split = create_split(model.output->ne[1], out_gran, model.splits, mem_used);
                    LLAMA_LOG_INFO("%s: splitting the LM head vocab-parallel: %" PRId64 " rows, granularity %d, %d devices\n",
                            __func__, model.output->ne[1], out_gran, (int)model.splits.size());
                    prepare_split_tensors(1, ctx_split, model.output, model.split_output, split, mem_used);
                    if (model.output_norm && split_tensors.find(model.output_norm) != split_tensors.end()) {
        ...
        // BELT, for the whole output block above. A tensor that is in the split context and gets no
        // ->extra is never uploaded (the split buffer's set_tensor returns on a null extra) and
        // reads the dummy base: silent garbage, the failure mode rule #1 of this file describes.
        // The head and its norm are kept together at creation time so this cannot happen; if it
        // ever does, it stops the load with a line naming the tensor instead of producing output
        // nobody can tell is wrong.
        if (!ggml_backend_buft_is_host(model.buft_output.buft_matrix)) {
            for (ggml_tensor * t : { model.output, model.output_norm }) {
                if (t && t->extra == nullptr && split_tensors.find(t) != split_tensors.end()) {
                    throw std::runtime_error(format(
                        "'%s' was created in the tensor-split context but was never split or "
                        "mirrored, so it would never be uploaded and would read as garbage. This is "
                        "an engine bug, not a user error -- report it with this line.", t->name));
                }
            }
        }
    ...
    // WHAT THE LOADER ACTUALLY CUT, counted after the fact. Every other line about a tensor split
    // states an intention: the mode that was asked for, the policy that was meant to apply. This
    // one walks the tensors that exist and reports what happened to them, because the quiet way a
    // tensor split disappoints is not an error -- it is a weight class that no rule matched,
    // sitting whole on one card while the model runs, the text reads fine and the number gets
    // quoted as a tensor-split number. A kind showing 0 cut is that case, named.
    if (model.pxa_tsplit) {
        struct kind_tally { int total = 0; int cut = 0; int mirrored = 0; };
        std::map<std::string, kind_tally> tally;
        for (ggml_context * c : model.ctxs) {
            for (ggml_tensor * t = ggml_get_first_tensor(c); t != nullptr; t = ggml_get_next_tensor(c, t)) {
                ...
                if (suffix != "weight" && suffix != "bias") {
                    // The per-device slices are the parent's name plus '.<device>'; counting them
                    // would count every cut twice and invent kinds that are not kinds.
                    continue;
                }
                const auto prev = name.rfind('.', last - 1);
                const std::string kind = prev == std::string::npos ? name : name.substr(prev + 1);
                auto & k = tally[kind];
                k.total++;
                if (t->extra) {
                    const auto * e = (const ggml_split_tensor_t *) t->extra;
                    if (e->split_dim < 0) {
                        k.mirrored++;
                    } else {
                        k.cut++;
        ...
        LLAMA_LOG_INFO("PXA_TSPLIT: cut per tensor kind, counted at load (kind: cut / mirrored / total)\n");
        for (const auto & [kind, k] : tally) {
            const char * verdict = k.cut > 0      ? ""
                                 : k.mirrored > 0 ? "   (replicated on every device)"
                                                  : "   <- NOT split: this kind stays whole on one card";
            LLAMA_LOG_INFO("PXA_TSPLIT:   %-28s %4d / %4d / %4d%s\n",
                    kind.c_str(), k.cut, k.mirrored, k.total, verdict);
        }
    }
    return use_mmap_buffer;
}
