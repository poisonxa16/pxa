...
struct llama_model {
    ...
    // PXA_MTP_SHORTLIST: a shortlisted draft head. output_mtp_shortlist holds the
    // mtp_shortlist_rows rows of the MTP head, gathered into a contiguous [n_embd, N] tensor of
    // the same type, so the DRAFT step scores N rows instead of the whole vocabulary. The draft
    // only proposes; the verify step keeps the full head, so the output distribution is
    // unchanged by construction. Null unless the lever is armed.
    ...
    // True when the user asked for '-sm tensor' and the capability check admitted it. split_mode
    // itself resolves to the split path the engine already runs, so this is what tells the boot
    // banner, and anything else that wants to name the mode, which mode the user actually asked
    // for. Never set by '-sm attn' or '-sm graph'.
    bool pxa_tsplit = false;
    int main_gpu;
