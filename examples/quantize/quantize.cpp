...
static void usage(const char * executable) {
    ...
    printf("pxq-quantize — the PXA quantizer that writes PXQ files. Same command line as\n"
           ...
           "PXQ4-HQ (the 4-bit quality tier), PXQ6 (5-bit LM32), and PXQ_UNIVERSAL (a per-tensor\n"
           "mix of PXQ1/2/3/4 from a tier map). Every stock ggml type still works too.\n\n");
#endif
...
int main(int argc, char ** argv) {
    ...
    // Q6_K output head can legitimately consume an imatrix on a PXQ target.
    {
        ...
        // non-PXQ target below is exactly the build it always was.
        {
            auto is_pxq_type = [](ggml_type t) {
