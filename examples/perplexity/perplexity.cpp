...
// PXA_PPL_PARSE_SPECIAL: score a corpus that carries the model's own control tokens as text.
// The corpus is tokenized with parse_special false, so a chat-shaped file's <start_of_turn> and
// friends are scored as ordinary character runs and the measurement never enters the regime the
// instruction-tuned weights were trained in. Off by default: turning it on changes the token
// stream, so a run with it is comparable only with other runs with it.
static bool pxa_ppl_parse_special() {
    static const bool v = [] {
        const char * e = getenv("PXA_PPL_PARSE_SPECIAL");
        return e && atoi(e) != 0;
    }();
    return v;
}
...
static results_perplexity perplexity_v2(llama_context * ctx, const gpt_params & params) {
    ...
    std::vector<llama_token> tokens = ::common_tokenize(ctx, params.prompt, true, pxa_ppl_parse_special());
