...
struct server_slot {
    ...
    // PXA_CKPT_PROMPT_END: armed for the one checkpoint taken at the end of a prompt, consumed by
    // create_checkpoint. Label only; it changes no policy, only what the log and the saved file
    // say about that checkpoint.
    bool ckpt_mark_prompt_end = false;
    ...
    std::vector<std::vector<pxa_spec_cand>> drafted_q;
    ...
    bool has_mtp = false;
    // PXA_SPEC_MTP_LAZY_v1 (2026-09-15): the chain CARRIES an MTP stage. has_mtp
    // above now means the strictly narrower "that stage's companion exists and this slot must pay
    // for it" -- it starts false under lazy arming and flips true on promotion. Everything that
    // asks "was this server started with MTP support" asks this flag instead.
    bool mtp_stage_armed = false;
    ...
    // PXA_MTP_ZERO_BUDGET_SKIP: this request asked for a zero draft budget, so none of the
    // drafting-side prompt work (per-token output flags, companion warmup) can ever be read.
    bool draft_budget_is_zero() const;
...
struct server_context {
    ...
    // PXA_INTERLEAVE_DECODE_MS_v1: end of the decode window opened by the last prompt chunk.
    int64_t pxa_decode_window_until_us = 0;
    ...
    // PXA_CACHE_DISK_v1: read the parked prompts back at start-up / write them out at shutdown.
    // Both are no-ops unless PXA_CACHE_DISK names a file.
    void prompt_cache_disk_init();
    void prompt_cache_disk_save();
    ...
    void create_checkpoint_at_interval(server_slot & slot, const gpt_params & params_base, bool in_prompt = false);
