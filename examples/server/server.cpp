...
static void log_prompt(const gpt_params & params_base, const json & body) {
    if (params_base.minilog) {
        LOG_TEE("Prompt:\n%s\n", body.dump(4, ' ', false, json::error_handler_t::replace).c_str());
    }
}
...
static int pxa_posture_mode() {
    if (ggml_pxa_config_level() == 0) return -1;
...
// Same estimate pxa_adaptive_ubatch uses (uniform split, tensor-split not modelled) and the same
...
static bool pxa_device_shares(const gpt_params & params, int ndev, size_t model_bytes, std::vector<size_t> & share) {
    ...
    double sum = 0.0;
    for (int i = 0; i < ndev; ++i) sum += params.tensor_split[i] > 0 ? params.tensor_split[i] : 0.0;
    if (sum <= 0.0) return false;
    for (int i = 0; i < ndev; ++i) {
        const double f = params.tensor_split[i] > 0 ? params.tensor_split[i] / sum : 0.0;
        share[i] = (size_t) ((double) model_bytes * f);
    }
    return true;
}
// -ot overrides that pin tensors to the CPU (e.g. Flash-Next's 54 GB per-layer embedding table) make the
// file size an overestimate of what lands on the devices. The estimators cannot know by how much, so
// they only say so; a negative headroom under an -ot=CPU override is an estimator artefact, not a fact.
static bool pxa_has_cpu_override(const gpt_params & params) {
    for (const auto & o : params.tensor_buft_overrides) {
        if (o.buft && ggml_backend_buft_name(o.buft) && strstr(ggml_backend_buft_name(o.buft), "CPU")) return true;
    }
    return false;
}
...
// True when every visible CUDA device is sm_61 and there is exactly one of them -- the 11 GB
...
static int pxa_adaptive_ubatch(const gpt_params & params, std::string & why) {
    ...
    long long headroom_ll = LLONG_MAX; size_t share = 0; int worst = 0;
    for (int i = 0; i < ndev; ++i) {
        size_t free = 0, total = 0;
        ggml_backend_cuda_get_device_memory(i, &free, &total);
        const long long h = (long long) free - (long long) shares[i];
        if (h < headroom_ll) { headroom_ll = h; share = shares[i]; worst = i; }
    }
    if (headroom_ll < 0) {
        // A negative headroom says the model would not load at all -- which is not what happens on
    ...
    const size_t headroom = (size_t) headroom_ll;
...
int main(int argc, char ** argv) {
    ...
    // explicit flags always win; PXA_REFERENCE=1 stands this down. See the block above main.
    {
        ...
        if (pmode < 0) {
            ...
            // PXA PER-CARD BATCH DEFAULTS (2026-09-03): on a topology the campaign actually
            // measured, -b/-ub the user left unset are filled from the measured cell instead of
            // from the generic VRAM ladder below. The ladder is a heuristic ("what plausibly
            // fits"); this is a measurement ("what was fastest on these exact cards"), so it
            // takes precedence -- but only where a measurement exists, and only for a flag that
            // was NOT given. See ggml_backend_cuda_pxa_suggest_batch() for the table.
            const bool b_explicit = pxa_argv_has(argc, argv, {"-b", "--batch-size"})
                                    || getenv("LLAMA_ARG_BATCH") != nullptr;
            bool card_b = false, card_ub = false;
#if defined(GGML_USE_CUDA)
            if (!ub_explicit || !b_explicit) {
                ...
                if (ggml_backend_cuda_pxa_suggest_batch(n_expert, &sb, &sub, &cell)) {
                    ...
                    fprintf(stderr,
                            "PXA_AUTO: batch defaults for %s -> -b %d%s -ub %d%s "
                            "(measured cell; pass -b/-ub to override, PXA_ENHANCE=0 for the "
                            "stock defaults)\n",
    ...
    // PXA_AUTO_TS=0 forces off, =1 forces on at any level (never under PXA_REFERENCE=1).
    {
        ...
        const bool  ref2 = ggml_pxa_config_level() == 0;
        const bool  enh2 = ggml_pxa_config_level() >= 2;
    ...
    // 6-card cell, 2026-07-04). The guard clamps an unset-or-0 top_k to 100.
    {
        ...
        const bool  ref = ggml_pxa_config_level() == 0;
        const bool  enh = ggml_pxa_config_level() >= 2;
    ...
    // overridden, only absence is filled.
    {
        ...
        const bool   ref3 = ggml_pxa_config_level() == 0;
        const bool   enh3 = ggml_pxa_config_level() >= 2;
        ...
        // VRAM GATE (2026-09-03). Arming a speculation stage costs device memory: the draft
        // context, its KV, and the per-slot checkpoint buffers the verify path keeps. On a card
        // whose weights already fill it -- the 11 GB 1080 Ti seat is the measured case, PXQ2
        // weights are 9.9 GiB of 11.0 -- that turns a working server into one that OOMs mid
        // prefill. It never did so before because AUTO_SPEC only armed for people who exported
        // PXA_ENHANCE=1; with ENHANCE as the default it would become a default OOM for every
        // 11-12 GB card. So the auto-arm now declines when the estimated post-weights headroom
        // is under 2 GiB, and declines outright on a single-card sm_61 fleet. This is an
        // AUTO-ARM gate only: an explicit --spec-type, -md, or PXA_AUTO_SPEC=1 still gets what
        // it asked for, because a user who names the flag has accepted the cost.
        bool spec_vram_ok = true;
        std::string spec_vram_why;
        if (active && sp == nullptr) {
            const long long head = pxa_min_vram_headroom(params, spec_vram_why);
            const long long need = 2LL*1024*1024*1024;   // 2 GiB
            if (pxa_single_sm61()) {
                spec_vram_ok  = false;
                spec_vram_why = "single-card sm_61 (11 GB class): the weights fill the card and a "
                                "draft context OOMs mid-prefill -- measured on the 1080 Ti PXQ2 tier";
            } else if (head < 0) {
                spec_vram_ok  = false;
                spec_vram_why = "cannot estimate free VRAM (" + spec_vram_why + ")";
            } else if (head < need) {
                spec_vram_ok  = false;
                spec_vram_why = "only " + spec_vram_why + ", under the 2048 MiB a draft context needs";
            }
            if (!spec_vram_ok) {
        ...
        if (active && spec_vram_ok && !spec_explicit && !draft_explicit) {
            const std::string arch = pxa_gguf_arch(params.model);
            ...
            if (srow) {
                ...
                // The shipped default above arms the n-gram table ALONE on every cc >= 600 fleet,
                // and on fresh text that table never fires: measured 2026-09-20 on the V100 pair,
                // Qwen3.8-27B PXQ4 with a distinct prompt per rep, the armed default reads 37.99
                // t/s against 37.69 with no speculation and draws no draft at all. Every gain the
                // table has ever shown was re-generation of text the same server boot had already
                // produced, which is a real feature for a client that revisits its own text and is not a first-answer
                // speed. The measurements that DO move a first answer on this model come from the
                // trained head behind the table -- and the head's best shape depends on the
                // acceptance rule the request runs under, which is a per-request fact.
                //
                // So with the policy on, an MTP file takes the CASCADE (table in front, head
                // behind) and the head's depth and floor are chosen per request further down in
                // the server. The table keeps its place at the front of the chain, so the
                // repetition class it wins keeps winning. Default OFF: unset, this whole rule is
                ...
                if (ngram_alone_chain) {
                    ...
                    // named it.
                    // PXA_SPEC_CASCADE_NGRAM_FULL_v1 (2026-09-15). THE CASCADE'S
                    // N-GRAM STAGE WAS NOT THE MEASURED N-GRAM STAGE. This branch built it BARE,
                    // so it took the family defaults for every knob the alone-chain branch twelve
                    // lines above sets explicitly: ngram_size_n fell back to 16 instead of 24, and
                    // -- the expensive one -- n_max fell back to the CHAIN-WIDE params.n_max, which
                    // the MTP auto layer below sets to 3. The boot banner proves it on the shipped
                    // bytes: `initialized ngram_mod with n=16` and `the chain drafts up to 4 tokens
                    // (capacity 5) -> effective n_max = 4`, against n=24 / effective n_max = 29 for
                    // the identical table in the alone chain. Measured consequence on the V100 pair
                    // 2026-09-15 (one binary, np1, REPS 3): the table's mean accepted draft is 4.0
                    // tokens in the cascade against 26.3 alone, and doc16k-greedy reads 64.72 t/s
                    // against 93.15. Every measurement ever taken of "the cascade" measured a
                    // 16-wide 4-deep table, not this engine's n-gram stage. A stage override wins
                    // over params.n_max in common_speculative_get_runtime_params(), so naming the
                    // constants here also stops the MTP stage's depth from deciding the table's.
                    static const char * const ngram_stage = "ngram:n_max=64,n_min=2,ngram_size_n=24";
    ...
    // keeps today's defaults, and PXA_REFERENCE=1 stands the whole thing down.
    {
        ...
        if (mtp_stage_present) {
            ...
            // PXA_SPEC_POLICY (2026-09-20): the row above ships ONE (n_max, p_min) for the whole
            // seat. The policy resolves TWO -- one per acceptance rule -- here, where the arch is
            // known, and the per-request chooser in the server picks between them. The startup
            // stage reserves the deeper of the two, because a request may not ask for more draft
            // capacity than the boot reserved (the verify batch and the checkpoint budget are
            // sized from it). Default off: unset, nothing below this line changes.
            const bool policy_tensor_split = params.split_mode == LLAMA_SPLIT_MODE_ATTN ||
                                             params.split_mode == LLAMA_SPLIT_MODE_GRAPH;
            const bool policy_on = !ref_level && pxa_spec_policy_enabled() &&
                                   pxa_spec_policy_resolve(arch, policy_tensor_split);
            ...
            if (crow == nullptr) {
                ...
                // With the policy live the startup values are the CEILING the per-request chooser
                ...
                for (auto & stage : params.speculative.stages) {
                    ...
                    if (!stage.has_n_max_override()) { stage.n_max = row_n_max; set_n_max = true; }
                    if (!stage.has_p_min_override()) { stage.p_min = row_p_min; set_p_min = true; }
                    eff_n_max = stage.n_max;
                    eff_p_min = stage.p_min;
                }
                // A field the operator named on the command line is the operator's, per request
                // as well as at boot: set_n_max / set_p_min are false exactly when the CLI named
                // one, and the per-request chooser reads that back through the policy.
                if (policy_on) {
                    pxa_spec_policy_set_cli_named(!set_n_max, !set_p_min);
                }
                ...
                fprintf(stderr, "PXA_AUTO: spec mtp arch=%s -> n_max=%d%s, p_min=%.3f%s, "
                                ...
                                "or PXA_MTP_PMIN / PXA_MTP_ZERO_OUTPUT_COMMIT)%s\n",
                        arch.c_str(),
                        ...
                        policy_on ? "the per-request policy's ceiling, not this row" : crow->why,
                        policy_on ? " -- PXA_SPEC_POLICY is live, so these are the startup CEILING "
                                    "and each request's own depth and floor are chosen from its "
                                    "acceptance rule" : "");
            }
        }
    }
    ...
    if (!params.speculative.get_resolved_stages().empty()) {
        ...
        if (common_sampler_spec_sampled_active()) {
            fprintf(stderr, "PXA_SPEC_SAMPLED: ON -- lossless speculative sampling at temp>0: the drafter DRAWS each "
                            "draft token from its own distribution under this request's filter chain, the verifier "
                            "accepts it with probability min(1, p/q) and resamples normalise(max(p-q,0)) on a "
                            "rejection, so the emitted token is distributed exactly as an unspeculated request's "
    ...
    svr->set_default_headers({{"Server", "PXA"}});
            ...
            case SERVER_STATE_READY:
                {
                    ...
                    if (ctx_server.pxa_decode_wedged(pxa_stall_ms)) {
                        ...
                        res.status = 503;
                        res.set_content(safe_json_to_str(health), "application/json");
                        break;
                    }
    ...
    const auto handle_slots = [&](const httplib::Request &, httplib::Response & res) {
        ...
        res.set_content(safe_json_to_str(result.data.at("slots")), "application/json");
        res.status = 200; // HTTP OK
    };
    ...
    const auto handle_models = [&params, &model_meta](const httplib::Request & req, httplib::Response & res) {
        ...
        res.set_content(safe_json_to_str(models), "application/json; charset=utf-8");
    };
    ...
    const auto handle_lora_adapters_list = [&](const httplib::Request & req, httplib::Response & res) {
        ...
        res.set_content(safe_json_to_str(result), "application/json");
        res.status = 200; // HTTP OK
    };
    ...
    const auto handle_control_vectors_apply = [&](const httplib::Request & req, httplib::Response & res) {
        ...
        for (auto entry : body) {
            ...
            if (0 <= id && id < max_idx) {
                ...
                res.set_content(safe_json_to_str(json{{ "success", false }, { "error", "Invalid control vector id" }}), "application/json");
                res.status = 400;
                return;
            }
        }
    ...
    const auto handle_version = [&params, sqlite_extension_loaded](const httplib::Request&, httplib::Response& res) {
        res.set_content(safe_json_to_str(
            json{{"version", 4},
            {"features", {{"sql", !params.sql_save_file.empty()}, {"zstd_compression", sqlite_extension_loaded}}}}),
            "application/json"
        );
    };
#else
    const auto handle_version = [](const httplib::Request&, httplib::Response& res)-> void {
        res.set_content(safe_json_to_str(
             json{{"version", 4},
             {"features", {{"sql", false}, {"zstd_compression", false}}}}),
             "application/json"
        );
    };
#endif
    ...
    auto db_handler = [db_handle](auto func) {
        return [func, db_handle](const httplib::Request& req, httplib::Response& res) {
            ...
            res.status = 500;
            res.set_content(safe_json_to_str(
                json{{"ok", false}, {"message", "Sqlite3 support was not enabled. Recompile with '-DLLAMA_SERVER_SQLITE3=ON'"}}),
                "application/json"
            );
        };
    };
    ...
    const auto get_key_string = [](const json& j) {
        return j.is_string() ? j.get<std::string>() :safe_json_to_str( j);
    };
	...
	if(data.empty()) {
            res.status = 404;
            res.set_content(safe_json_to_str(json{{"ok", false}, {"message", "Key not found"}}), "application/json");
        } else {
            json response{{"ok", true}};
	    response["result"] = (store == "names") ? json(data) : json::parse(data);
            res.set_content(safe_json_to_str(response), "application/json");
        }
    });

    const auto handle_save = db_handler([normalize_store_name, get_key_string](auto& db, const json& body, auto&, auto& res) {
        const std::string store = normalize_store_name(body["storeName"]);
        const std::string data = (store == "names") ? body["data"].get<std::string>() :safe_json_to_str( body["data"]);
        db.db << "INSERT OR REPLACE INTO " + store + " (key, data) VALUES (?, ?)" << get_key_string(body["key"]) << data;
        res.set_content(safe_json_to_str(json{{"ok", true}, {"result", "Data saved successfully"}}), "application/json");
    });
    ...
    const auto handle_rename = db_handler([get_key_string](auto& db, const json& body, auto&, auto& res) {
        ...
        res.set_content(safe_json_to_str(json{{"ok", true}, {"result", "Session renamed successfully"}}), "application/json");
    });
    ...
    const auto handle_all = db_handler([normalize_store_name](auto& db, const json& body, auto&, auto& res) {
        ...
        res.set_content(safe_json_to_str(json{{"ok", true}, {"result", result}}), "application/json");
    });
    ...
    const auto handle_delete = db_handler([normalize_store_name, get_key_string](auto& db, const json& body, auto&, auto& res) {
        db.db << "DELETE FROM " + normalize_store_name(body["storeName"]) + " WHERE key = ?"
            << get_key_string(body["key"]);
        res.set_content(safe_json_to_str(json{{"ok", true}, {"result", "Session deleted successfully"}}), "application/json");
    });

    const auto handle_vacuum = db_handler([](auto& db, const json& body, auto&, auto& res) {
        json result = json::object();
        db.db << "VACUUM";
        res.set_content(safe_json_to_str(json{"ok", true}), "application/json");
    });
    ...
    const auto handle_zstd_get_configs = db_handler([](auto& db, const json& body, auto&, auto& res) {
        ...
        res.set_content(safe_json_to_str(json{{"ok", true}, {"configs", result}}), "application/json");
    });
    ...
    ctx_server.prompt_cache_disk_save();
