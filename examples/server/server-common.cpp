    ...
    std::vector<raw_buffer>& out_files)
{
        ...
        for (auto& p : content) {
            ...
            if (type == "image_url") {
                ...
                if (string_starts_with(url, "http")) {
                    ...
                    params.headers.push_back("User-Agent: PXA/");
    ...
    inputs.json_schema = json_schema.is_null() ? "" :safe_json_to_str( json_schema);
    ...
    for (const auto& item : chat_template_kwargs_object.items()) {
        inputs.chat_template_kwargs[item.key()] =safe_json_to_str( item.value());
    }
