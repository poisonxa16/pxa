...
json server_chat_convert_responses_to_chatcmpl(const json& response_body) {
    ...
    if (!json_value(response_body, "previous_response_id", std::string{}).empty()) {
        throw std::runtime_error("pxa does not support 'previous_response_id'.");
    }
            ...
            if (exists_and_is_array(item, "content") &&
                ...
                for (const json& input_item : item.at("content")) {
                    ...
                    if (type == "input_text") {
                        ...
                        throw std::runtime_error("'input_file' is not supported by pxa at this moment");
                    }
                    else {
                ...
                if (type == "text") {
                    ...
                    tool_calls.push_back({
                        ...
                        {"function", {
                            ...
                            {"arguments",safe_json_to_str( json_value(block, "input", json::object()))}
                        }}
                        });
                    has_tool_calls = true;
