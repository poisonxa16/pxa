...
public:
    ...
    // Whether THIS token list actually holds media chunks. Distinct from `has_mtmd`, which is the
    // "this server was started with a projector" flag and is true on every list a projector build
    // makes, media or not.
    bool has_mtmd_data() const {
