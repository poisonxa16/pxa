...
struct server_prompt_checkpoint {
    ...
    // PXA_CKPT_PROMPT_END: this checkpoint was taken at the end of a prompt, before the first
    // token of the answer was decoded, so its pos_max is the prompt's last position. Measured
    // 2026-09-20: that is one position too HIGH to be restored for the turn it was taken for --
    // apply_checkpoint needs pos_max strictly below the re-entry point, and a next turn that
    // keeps the whole prompt re-enters at the position just above it. The flag is a label for
    // the log and for the file format; nothing gives it priority in the ring, because a
    // checkpoint that cannot be chosen is not worth protecting. Unset, nothing sets it.
    ...
    void from_json(const json & j) {
        ...
        // absent in checkpoints written before PXA_CKPT_PROMPT_END: treat as an ordinary one
        prompt_end = j.value<bool>("prompt_end", false);
    }
};
...
struct server_prompt {
    ...
    // PXA_CACHE_PARK_SPEC_v1 -- the speculative half of a parked conversation.
    //
    // A parked entry that carries only the target KV is half a conversation: when the client comes
    // back, the drafter has to be re-seeded from the restored tokens before it can draft again, and
    // until it is, the first steps run unaccelerated. These two payloads carry the rest of it:
    //   data_draft  -- the draft (companion) context's sequence state, same format as the target's
    //   spec_carry  -- the drafter's own opaque per-sequence carry (hidden row + cached draft)
    // Both are OPTIONAL by construction: empty when the lever is off, when the run is not
    // speculative, or when the drafter held nothing worth carrying. An entry with empty payloads
    // restores exactly as it did before this field existed.
    std::vector<uint8_t> data_draft;
    std::vector<uint8_t> spec_carry;
...
struct server_prompt_cache {
    ...
    // out_restored (optional) reports whether an entry was actually taken out of the cache and
    // written into the context. The return value only says the operation did not fail, so a caller
    // that has to undo or complete something on a real restore cannot use it.
    bool load(server_prompt& prompt, const server_tokens& tokens_new, llama_context* ctx, int32_t id_slot, float min_reusable_fraction,
              bool* out_restored = nullptr);

    void update();

    // PXA_CACHE_DISK_v1 -- the whole cache, written to one file and read back at start-up.
    //
    // The RAM cache is where a parked conversation waits for its client to come back; without this
    // it waits only until the process does. `fingerprint` is checked on load: a file written by a
    // different model, a different build of the same model file, a different context size, KV type
    // or rope schedule describes state this process cannot install -- or, worse, state it CAN
    // install and would then be wrong about. Refused whole rather than per entry.
    uint64_t fingerprint = 0;

    size_t save_file(const std::string& filepath) const;
    // `has_mtmd_now` is this process's own projector state. The serialized token list carries the
    // writing process's flag, which must never be installed here: it decides which asserts in
    // server_tokens hold, not what the bytes contain.
    size_t load_file(const std::string& filepath, uint64_t fingerprint_expect, bool has_mtmd_now);
};
