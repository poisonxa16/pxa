...
extern "C" {
    ...
    enum llama_split_mode {
        ...
        LLAMA_SPLIT_MODE_TENSOR  = 4, // PXA tensor split: every splittable weight and the KV cache
                                      // divided across GPUs, admitted by a capability check on the
                                      // file rather than demoted silently. Value 4 and not 3
                                      // deliberately: 3 is already GRAPH here.
    };
    ...
    // True when the most recent llama_init_from_model() returned NULL because it could
    // not find memory for the KV cache, the compute buffers or the output buffer -- as
    // opposed to rejecting a parameter, which asking again would not change. A caller
    // that chose the context size itself can use this to ask for a smaller one.
    LLAMA_API bool llama_context_creation_failed_on_alloc(void);
...
#define LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY 1
    ...
    // PXA_CKPT_LEAN_SYNC (default OFF): true when the redundant checkpoint drains are gated off,
    // so the server can drop its own explicit pre-read llama_synchronize as well.
    LLAMA_API bool llama_pxa_ckpt_lean_sync(void);

    // PXA_CKPT_PROMPT_POLICY (default OFF): true when mid-prompt context checkpoints follow
    // mainline's policy (only near the end of the prompt) instead of a fixed token interval.
    LLAMA_API bool llama_pxa_ckpt_prompt_policy(void);
    ...
    // rows; rows keep the n_vocab stride either way, so a reader must scan this many and no more.
    LLAMA_API int32_t llama_n_logits(const struct llama_context * ctx);

    // The shortlist row table, or 0/NULL when the lever is not armed. Index i of a shortlisted
    // draft-head logit row belongs to token llama_mtp_shortlist_rows(model)[i].
    LLAMA_API int32_t         llama_mtp_shortlist_n_rows(const struct llama_model * model);
    LLAMA_API const int32_t * llama_mtp_shortlist_rows  (const struct llama_model * model);
    LLAMA_API void            llama_mtp_shortlist_count_draw(void);
