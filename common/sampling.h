...
// PXA_SPEC_SAMPLED: `draft_q[i]` is the proposal distribution the drafter DREW draft[i] from, as a
// normalised (id, logit, p) list over its post-filter support. When it is present (and the lever and
// the request allow it) position i is verified with the lossless sampled rule -- accept with
...
// draft and the exact rule). `out_prob`, when asked for, is q(drawn token) -- the confidence of the
// token that was actually proposed, which is the quantity a p_min floor means under this rule.
llama_token common_sampler_draft_sample_dist(struct common_sampler * gsmpl, struct llama_context * ctx, int idx,
        std::vector<pxa_spec_cand> & q_out, float * out_prob = nullptr);

// True when the lever is armed AND this request's sampler chain can be mirrored onto the drafter
// (temp > 0, no grammar, no mirostat, no adaptive-p, no dynamic temperature, no DRY, and only
// samplers whose effect on the draft head is reproducible). Refusal is logged once.
...
// the target's own sample. Exact-match semantics always hold at temp<=0. Read-only; for ledgers.
bool  common_sampler_spec_relaxed_active();
float common_sampler_spec_relaxed_pmin();

// PXA_SPEC_ACCEPT_RULE_v1: the ONE place that answers "does this request's speculative
// verification behave as exact match?". common_sampler_sample_and_accept_n and every caller that
// reports or plans for the rule must go through this, so the reported rule and the executed rule
// cannot drift apart. `why`, when given, is filled with a short public-safe reason.
// Returns false when the request can keep a draft token the target did not sample.
bool common_sampler_acceptance_is_exact(const struct common_params_sampling & params, const char ** why = nullptr);
