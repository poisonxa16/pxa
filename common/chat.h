...
// PXA_AUTO_REASONING_v1. A chat template can carry its own default for `enable_thinking`, and
// some models ship it OFF. The engine's own default is ON, so for such a model every chat request
// becomes a reasoning request: the thought is extracted into `reasoning_content` and `content`
// stays empty until the thought closes, which on a short answer budget means the caller gets an
// empty string back from a perfectly healthy server. That is the model's authors' choice being
// overridden by ours.
//
// So: when the caller gave neither --reasoning nor a chat_template_kwargs enable_thinking, and the
// model is one of the architectures whose template is known to default thinking off, follow the
// template and SAY SO. --reasoning on still decides; PXA_AUTO_REASONING=0 declines and says why.
// Returns true when it turned reasoning off.
