...
bool gpt_params_find_arg(int argc, char ** argv, const std::string & arg, gpt_params & params, int & i, bool & invalid_param) {
    ...
    if (arg == "--split-mode" || arg == "-sm") {
        ...
        else if (arg_next == "tensor" || arg_next == "pxa") {
...
void gpt_params_print_usage(int /*argc*/, char ** argv, const gpt_params & params) {
    ...
    if (llama_supports_gpu_offload()) {
        ...
        options.push_back({ "*",           "-sm,   --split-mode SPLIT_MODE",
                                                                        ...
                                                                        "  - tensor: PXA tensor split - split every splittable weight and the KV\n"
                                                                        "            cache across GPUs. Admitted by a capability check on the file;\n"
                                                                        "            if it cannot be had you get one line saying why, not a silent\n"
                                                                        "            fall back to layer. Needs >= 2 GPUs and flash attention.\n"
                                                                        "  - attn: the attention/head split as it shipped before 'tensor'\n"
...
// PXA_AUTO_CTX_v1 (2026-09-20). A boot with no '-c' asks for the model's whole
// trained window, because that is what a context size of zero has always meant.
// On a model trained at 262,144 tokens that is tens of gigabytes of KV cache, so
// the obvious first command -- 'llama-server -m model.gguf -ngl 99' -- dies at the
// allocation with a wall of CUDA errors, and the reader has no way of knowing that
// the file, the cards and the driver are all fine and the only wrong number is one
// they never typed.
//
// So: when the caller named no context size, and the context failed to be created
// because there was no memory for it, ask again for half as much, and keep halving
// down to a floor of 4,096. Then say in one line what was asked for, what fit, and
// how to choose for yourself.
//
// This is deliberately a RETRY and not an estimate. An estimate has to model the KV
// cache, the compute buffers, every other process on the card and each architecture's
// own attention geometry, and it is wrong the day a new architecture lands. A retry
// asks the allocator, which is the only thing that actually knows.
//
// It cannot change a boot that works today: it is reached only after a context
// creation has already failed, and only when the caller passed no '-c'. An explicit
// '-c' that does not fit still fails exactly as before -- the number came from the
// user, and silently serving a different one would be worse than stopping. Decline
// it entirely with PXA_AUTO_CTX=0.
static const int32_t PXA_AUTO_CTX_FLOOR = 4096;

// Where the retry starts when the trained window is larger than this. Two reasons for
// a ceiling rather than plain halving from the trained window:
//
//   COST. Every doomed attempt reprints the whole context-creation block - sixty-odd
//   lines including the allocator's "cudaMalloc failed: out of memory" - and the first
//   thing a new user sees should not be three copies of it. A 262,144-token window
//   halves to 131,072 and 65,536 before anything fits on a 32 GiB pair; starting at
//   32,768 gets there in one. (A failed attempt is CHEAP in time - measured at about a
//   second, because the allocator refuses an impossible size immediately - so this is
//   about what is printed, not about the clock.)
//
//   HEADROOM. The largest context that fits is not the one to hand somebody who has
//   not asked for a size. Halving from the trained window landed a 2x P100 pair on
//   65,536 tokens with 600 MiB left on each card: it runs, and then the first '-np 4'
//   or draft model fails at runtime. 32,768 tokens of Gemma 4 is 7 GiB of KV and still
//   longer than most people's chats.
//
// Anyone who wants the biggest context their cards will hold asks for it with -c N,
// which is passed through untouched and fails honestly if it does not fit.
static const int32_t PXA_AUTO_CTX_CEILING = 32768;

static llama_context * pxa_auto_ctx_retry(llama_model * model, gpt_params & params,
                                          llama_context_params cparams) {
    const int32_t asked = llama_n_ctx_train(model);

    if (const char * e = getenv("PXA_AUTO_CTX"); e && atoi(e) == 0) {
        fprintf(stderr,
                "PXA_AUTO: CTX=off (PXA_AUTO_CTX=0; no -c was given, so the whole %d-token "
                "trained window was asked for and there was no memory for it - pass '-c N')\n",
                asked);
        return nullptr;
    }

    if (asked <= PXA_AUTO_CTX_FLOOR) {
        return nullptr;
    }

    // The trained window has already been tried and failed, so the first thing worth
    // trying is the ceiling - or half the window, when the window is under the ceiling.
    const int32_t first = asked > PXA_AUTO_CTX_CEILING ? PXA_AUTO_CTX_CEILING : asked / 2;

    for (int32_t n_ctx = first; n_ctx >= PXA_AUTO_CTX_FLOOR; n_ctx /= 2) {
        fprintf(stderr, "PXA_AUTO: ctx %d did not fit, trying %d\n",
                n_ctx == first ? asked : n_ctx * 2, n_ctx);

        cparams.n_ctx = n_ctx;
        llama_context * lctx = llama_init_from_model(model, cparams);
        if (lctx == nullptr) {
            if (!llama_context_creation_failed_on_alloc()) {
                // It stopped for a reason a smaller context cannot fix; the engine has
                // already said what it was. Trying again would only repeat it.
                return nullptr;
            }
            continue;
        }

        // Everything downstream reads params.n_ctx rather than the context: the server's
        // per-slot ceiling, the draft context of a speculative companion, the size it
        // reports to a client. They all have to follow the context that actually booted.
        params.n_ctx = n_ctx;

        // What -np does to this number is the server's arithmetic, not ours: with one
        // shared KV ring every slot may address the whole context, and without it the
        // context is divided. Saying the wrong one here would be worse than saying
        // nothing, so each case says its own.
        char per_slot[96] = "";
        if (params.n_parallel > 1) {
            if (params.kv_unified) {
                snprintf(per_slot, sizeof(per_slot),
                         ", shared by all %d slots (--kv-unified)", params.n_parallel);
            } else {
                snprintf(per_slot, sizeof(per_slot), ", divided by '-np %d' into %d per slot",
                         params.n_parallel, n_ctx / params.n_parallel);
            }
        }
        fprintf(stderr,
                "PXA_AUTO: CTX=%d (no -c given, so the model's whole %d-token trained window "
                "was asked for and there was no memory for its KV cache; %d fits%s - choose "
                "your own with '-c N', or PXA_AUTO_CTX=0 to fail instead)\n",
                n_ctx, asked, n_ctx, per_slot);
        return lctx;
    }

    fprintf(stderr,
            "PXA_AUTO: CTX gave up - nothing from %d down to %d tokens fit on these cards. "
            "The model itself is too large for the memory that is left, not the context: "
            "use fewer layers on the GPU (-ngl), more cards, or a smaller file.\n",
            first, PXA_AUTO_CTX_FLOOR);
    return nullptr;
}
...
struct llama_init_result llama_init_from_gpt_params(gpt_params & params) {
    ...
    if (lctx == NULL && params.n_ctx == 0 && llama_context_creation_failed_on_alloc()) {
        lctx = pxa_auto_ctx_retry(model, params, cparams);
    }

    if (lctx == NULL) {
        fprintf(stderr, "%s: error: failed to create context with model '%s'\n", __func__, params.model.c_str());
        // Only when it was the MEMORY that ran out. A model the engine declines to build,
        // or a parameter it rejects, is not answered by asking for less context, and
        // sending somebody off to try '-c 2048' after a refusal that already told them
        // exactly which variable to set wastes the one sentence they needed.
        if (params.n_ctx != 0 && llama_context_creation_failed_on_alloc()) {
            fprintf(stderr, "%s: error: '-c %d' was asked for; a smaller one may fit\n",
