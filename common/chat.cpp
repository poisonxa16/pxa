...
bool common_chat_auto_jinja(const struct llama_model * model, bool & use_jinja,
    ...
    //   gemma4 / gemma4_mtp : the built-in map has no row for the template at all.
    //   qwen35              : the built-in chatml row renders the turns, but the model answers
    //                         inside <think> tags and only the jinja chat path separates the thought
    ...
    if (is_qwen35) {
        fprintf(stderr,
                "PXA_AUTO: JINJA=on ('%s' answers inside <think> tags, and only the model's own chat "
                "template path returns the thought in `reasoning_content` and the reply in `content`; "
                "override with --chat-template, or PXA_AUTO_JINJA=0)\n",
                arch);
        return true;
    }
    fprintf(stderr,
...
bool common_chat_auto_reasoning(const struct llama_model * model, bool use_jinja,
                                int & enable_reasoning,
                                std::map<std::string, std::string> & template_kwargs) {
    // An explicit choice always wins: --reasoning on|off|auto sets enable_reasoning, and
    // --chat-template-kwargs enable_thinking=... says it the other way round.
    if (model == nullptr || !use_jinja || enable_reasoning != -1) {
        return false;
    }
    if (template_kwargs.find("enable_thinking") != template_kwargs.end()) {
        return false;
    }
    ...
    // Same rule as the jinja list above: an explicit row, added only when someone has read the
    // template and can say what its own default is. Gemma 4's canonical template opens with
    //   {%- set enable_thinking = enable_thinking | default(false) -%}
    // so the model's authors ship it with thinking OFF, and the two renderings really do differ:
    ...
    if (const char * e = getenv("PXA_AUTO_REASONING"); e && atoi(e) == 0) {
        fprintf(stderr,
                "PXA_AUTO: REASONING=on (PXA_AUTO_REASONING=0; '%s' ships its template with thinking "
                "off, so a short answer budget can be spent entirely on the thought and the content "
    ...
    fprintf(stderr,
            "PXA_AUTO: REASONING=off ('%s' ships its own chat template with enable_thinking defaulting "
            "to false, and turning it on costs the whole answer budget before the content field is "
            "written; ask for it with --reasoning on, or PXA_AUTO_REASONING=0)\n",
            arch);
    return true;
}
